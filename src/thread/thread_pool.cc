...
struct WorkerThread {
  ...
  // An SIM_Task for shutting down this worker.
  SIM_Task shutdown_task_ {
    &ShutdownFunction,
    nullptr,
    SIM_TASK_NEW
  };
...
// Concrete C++ class definition for SIM_ThreadPool.
// (The public SIM_ThreadPool C struct is an opaque one.)
class ThreadPoolImpl : public SIM_ThreadPool {
 public:
  ThreadPoolImpl(int num_worker) : SIM_ThreadPool{num_worker} {
    // initialize worker threads
    for (int i = 0; i < std::min(num_worker, SIM_MAXTHREAD); ++i) {
      WorkerThread worker{
  ...
  void Enqueue(SIM_Task* task) {
    if (SIM_UNLIKELY(GetAtomicTaskStatus(task).exchange(SIM_TASK_QUEUED) !=
                   SIM_TASK_NEW)) {
      SIM_ERROR("task->status is not SIM_TASK_NEW");
    }
  ...
  void Shutdown() {
    ...
    shutdown_ = true;
    std::vector<SIM_Task> shutdown_tasks(workers_.size());
 ...
 private:
  ...
  static void ThreadPoolWorker(
    ...
    while (!thread_pool->shutdown_) {
      auto task = static_cast<SIM_Task*>(thread_pool->lockless_queue_.pop());
      task->args = task->func(task->args);
      GetAtomicTaskStatus(task).store(SIM_TASK_COMPLETED);
    }
  }
  ...
  simcore::LocklessQueue<void*, kThreadPoolQueueSize> lockless_queue_;

  // Mutex to protect arena allocations.
  std::mutex alloc_mutex_;

  // Whether or not a ThreadPool was bound using sim_math_bindThreadPool.
  bool thread_pool_bound_ = false;
};

// create a thread pool
SIM_ThreadPool* sim_math_threadPoolCreate(size_t number_of_threads) {
  return reinterpret_cast<SIM_ThreadPool*>(new ThreadPoolImpl(number_of_threads));
}

// gets the number of shards the stack is currently broken into
static size_t GetNumberOfShards(sim_data_t* d) {
  if (!d->threadpool) {
    return 1;
  }
  return sim_math_threadPoolNumberOfThreads((SIM_ThreadPool*)d->threadpool) + 1;
}
...
SIM_StackInfo* sim_math_getStackInfoForThread(sim_data_t* d, size_t thread_id) {
  ...
  if (!thread_pool || !thread_pool->IsThreadPoolBound()) {
    sim_error("Thread Pool not bound, use sim_math_bindThreadPool to add an SIM_ThreadPool to sim_data_t");
  }
  ...
  size_t misalignment = bytes_per_shard % sim_math_getDestructiveInterferenceSize();

  if (misalignment != 0) {
    bytes_per_shard += sim_math_getDestructiveInterferenceSize() - misalignment;
  }

  if (bytes_per_shard * number_of_shards > total_arena_size_bytes) {
    sim_error("Arena is not large enough for %zu shards", number_of_shards);
  }

  uintptr_t result = (end_of_arena_ptr - (thread_id + 1) * bytes_per_shard);

  // align the end of the shard to be SIM_StackInfo.
  misalignment = result % alignof(SIM_StackInfo);
  result -= misalignment;
#ifdef ADDRESS_SANITIZER
  // Ensure StackInfo is always accessible
  ASAN_UNPOISON_MEMORY_REGION((void*)result, sizeof(SIM_StackInfo));
#endif
  return (SIM_StackInfo*) result;
}

// shards the stack for each thread
static void  ConfigureMultiThreadedStack(sim_data_t* d) {
  if (!d->threadpool) {
    sim_error("No thread pool specified for multithreaded operation");
  }
  ...
  for (size_t shard_index = 0; shard_index < number_of_shards; ++shard_index) {
    SIM_StackInfo* end_shard_cursor_ptr = sim_math_getStackInfoForThread(d, shard_index);
#ifdef ADDRESS_SANITIZER
  // unpoison stack info
  ASAN_UNPOISON_MEMORY_REGION((void*)end_shard_cursor_ptr, sizeof(SIM_StackInfo));
#endif
    ...
    if (shard_index == 0) {
      ...
      if ((uintptr_t)end_shard_cursor_ptr > current_limit) {
        sim_error("sim_bindThreadPool: sharding stack - existing stack larger than shard size: current_size = %zu, "
    ...
    end_shard_cursor_ptr->limit = (uintptr_t)end_shard_cursor_ptr + sizeof(SIM_StackInfo);
...
void sim_math_bindThreadPool(sim_data_t* d, void* thread_pool) {
  if (d->threadpool) {
    sim_error("Thread Pool already bound to sim_data_t");
  }
...
size_t sim_math_threadPoolNumberOfThreads(SIM_ThreadPool* thread_pool) {
  auto thread_pool_impl = static_cast<ThreadPoolImpl*>(thread_pool);
  return thread_pool_impl->NumberOfThreads();
}

size_t sim_math_threadPoolCurrentWorkerId(SIM_ThreadPool* thread_pool) {
  auto thread_pool_impl = static_cast<ThreadPoolImpl*>(thread_pool);
  return thread_pool_impl->GetWorkerId();
}

// start a task in the threadpool
void sim_math_threadPoolEnqueue(SIM_ThreadPool* thread_pool, SIM_Task* task) {
  auto thread_pool_impl = static_cast<ThreadPoolImpl*>(thread_pool);
  thread_pool_impl->Enqueue(task);
}

// shutdown the threadpool and free the memory
void sim_math_threadPoolDestroy(SIM_ThreadPool* thread_pool) {
  auto thread_pool_impl = static_cast<ThreadPoolImpl*>(thread_pool);
...
void sim_math_threadPoolLockAllocMutex(SIM_ThreadPool* thread_pool) {
  auto thread_pool_impl = static_cast<ThreadPoolImpl*>(thread_pool);
  thread_pool_impl->LockAlloc();
}

// unlocks the allocation mutex to protect Stack and Arena allocations
void sim_math_threadPoolUnlockAllocMutex(SIM_ThreadPool* thread_pool) {
  auto thread_pool_impl = static_cast<ThreadPoolImpl*>(thread_pool);
  thread_pool_impl->UnlockAlloc();
}

// Get the destructive interference size for the architecture.
size_t sim_math_getDestructiveInterferenceSize(void) {
