...
// constructor: set defaults outside sim_builder_default_t
SIM_CFlexcomp::SIM_CFlexcomp(void) {
  type = SIM_FCOMPTYPE_GRID;
  count[0] = count[1] = count[2] = 10;
  sim_math_internal_set_vec(spacing, 0.02, 0.02, 0.02);
  sim_math_internal_set_vec(scale, 1, 1, 1);
  mass = 1;
  inertiabox = 0.005;
  equality = 0;
...
bool SIM_CFlexcomp::Make(sim_spec_body_t* body, char* error, int error_sz) {
  sim_builder_model_t* model = static_cast<sim_builder_body_t*>(body->element)->model;
  SIM_sCompiler* compiler = static_cast<sim_builder_body_t*>(body->element)->compiler;
  SIM_sFlex* dflex = def.spec.flex;
  bool direct = (type == SIM_FCOMPTYPE_DIRECT ||
                 type == SIM_FCOMPTYPE_MESH ||
                 type == SIM_FCOMPTYPE_GMSH);

  // check parent body name
  if (sim_spec_getName(body->element)->empty()) {
  ...
  // check counts
  for (int i=0; i < 3; i++) {
    if (count[i] < 1 || ((doftype == SIM_FCOMPDOF_RADIAL && count[i] < 2) && dflex->dim == 3)) {
  ...
  // check scale
  if (scale[0] < SIM_MINVAL || scale[1] < SIM_MINVAL || scale[2] < SIM_MINVAL) {
    return comperr(error, "Scale must be larger than SIM_MINVAL", error_sz);
  }

  // check mass and inertia
  if (mass < SIM_MINVAL || inertiabox < SIM_MINVAL) {
    return comperr(error, "Mass and inertiabox must be larger than SIM_MINVAL", error_sz);
  }

  // compute orientation
  const char* alterr = sim_spec_resolveOrientation(quat, compiler->degree, compiler->eulerseq, &alt);
  if (alterr) {
  ...
  bool res;
  switch (type) {
    case SIM_FCOMPTYPE_GRID:
    case SIM_FCOMPTYPE_CIRCLE:
      res = MakeGrid(error, error_sz);
      break;

    case SIM_FCOMPTYPE_BOX:
    case SIM_FCOMPTYPE_CYLINDER:
    case SIM_FCOMPTYPE_ELLIPSOID:
      res = MakeBox(error, error_sz, dflex->dim);
      break;

    case SIM_FCOMPTYPE_SQUARE:
    case SIM_FCOMPTYPE_DISC:
      res = MakeSquare(error, error_sz);
      break;

    case SIM_FCOMPTYPE_MESH:
      res = MakeMesh(model, compiler, error, error_sz);
      break;

    case SIM_FCOMPTYPE_GMSH:
      res = MakeGMSH(model, compiler, error, error_sz);
      break;

    case SIM_FCOMPTYPE_DIRECT:
      res = true;
      break;
  ...
  if (type == SIM_FCOMPTYPE_BOX || type == SIM_FCOMPTYPE_CYLINDER ||
      (type == SIM_FCOMPTYPE_GRID && dflex->dim == 3)) {
    dflex->flatskin = true;
  }
  ...
  if (type != SIM_FCOMPTYPE_GRID && !(pingrid.empty() && pingridrange.empty()) &&
      doftype != SIM_FCOMPDOF_TRILINEAR && doftype != SIM_FCOMPDOF_QUADRATIC) {
  ...
  for (int i=0; i < npnt; i++) {
    ...
    sim_math_internal_trnVecPose(newp, pos, quat, oldp);
    point[3*i] = newp[0];
    point[3*i+1] = newp[1];
    point[3*i+2] = newp[2];
  }
  ...
  double minmax[6] = {SIM_MAXVAL, SIM_MAXVAL, SIM_MAXVAL, -SIM_MAXVAL, -SIM_MAXVAL, -SIM_MAXVAL};
  for (int i=0; i < npnt; i++) {
    for (int j=0; j < 3; j++) {
  ...
  int nnode = 0;
  if (doftype == SIM_FCOMPDOF_TRILINEAR) {
    nnode = 8;
  } else if (doftype == SIM_FCOMPDOF_QUADRATIC) {
    nnode = 27;
  }
  ...
  if (!rigid) {
    ...
    for (int i=0; i < (int)pingrid.size(); i+=dflex->dim) {
      ...
      if (type != SIM_FCOMPTYPE_GRID && (doftype == SIM_FCOMPDOF_TRILINEAR ||
                                       doftype == SIM_FCOMPDOF_QUADRATIC)) {
        int dim = (doftype == SIM_FCOMPDOF_TRILINEAR) ? 2 : 3;
    ...
    if (doftype == SIM_FCOMPDOF_RADIAL) {
      pinned[0] = true;
    }
  ...
  SIM_CFlex* flex = model->AddFlex();
  SIM_sFlex* pf = &flex->spec;
  int id = flex->id;
  ...
  *flex = def.Flex();
  ...
  flex->model = model;
  flex->id = id;
  sim_spec_set_name(pf->element, name.c_str());
  sim_spec_setInt(pf->elem, element.data(), element.size());
  sim_spec_setFloat(pf->texcoord, texcoord.data(), texcoord.size());
  sim_spec_setInt(pf->elemtexcoord, elemtexcoord.data(), elemtexcoord.size());
  if (!centered) {
    sim_spec_setDouble(pf->vert, point.data(), point.size());
  }

  // rigid: set parent name, nothing else to do
  if (rigid) {
    sim_spec_appendString(pf->vertbody, sim_spec_getName(body->element)->c_str());
    return true;
  }
  ...
  for (int i=0; i < npnt; i++) {
    ...
    if (pinned[i] || doftype == SIM_FCOMPDOF_TRILINEAR || doftype == SIM_FCOMPDOF_QUADRATIC) {
      sim_spec_appendString(pf->vertbody, sim_spec_getName(body->element)->c_str());
      ...
      if (plugin.active) {
        ...
        pplugin->active = true;
        pplugin->element = static_cast<sim_spec_element_t*>(plugin.element);
    ...
    // not pinned and not trilinear: new body
      ...
      sim_spec_body_t* pb = sim_spec_addBody(body, 0);

      // add geom if vertcollide
      if (dflex->vertcollide) {
        sim_spec_geom_t* geom = sim_spec_addGeom(pb, 0);
      ...
      sim_math_internal_zerovec(pb->ipos, 3);
      pb->mass = bodymass;
      ...
      // add radial slider
      if (doftype == SIM_FCOMPDOF_RADIAL) {
        SIM_sJoint* jnt = sim_spec_addJoint(pb, 0);

        // set properties
        jnt->type = SIM_JNT_SLIDE;
        sim_math_internal_set_vec(jnt->pos, 0, 0, 0);
        sim_math_internal_copy_vec(jnt->axis, pb->pos, 3);
        sim_math_internal_normvec(jnt->axis, 3);
      }

      // add three orthogonal sliders
      else if (doftype == SIM_FCOMPDOF_FULL) {
        for (int j=0; j < 3; j++) {
          // add joint to body
          SIM_sJoint* jnt = sim_spec_addJoint(pb, 0);
          ...
          jnt->type = SIM_JNT_SLIDE;
          sim_math_internal_set_vec(jnt->pos, 0, 0, 0);
          sim_math_internal_set_vec(jnt->axis, 0, 0, 0);
          jnt->axis[j] = 1;
        }
      }
      ...
      char txt[100];
      sim_math::sprintf_arr(txt, "%s_%d", name.c_str(), i);
      sim_spec_set_name(pb->element, txt);
      sim_spec_appendString(pf->vertbody, sim_spec_getName(pb->element)->c_str());
  ...
  if (doftype == SIM_FCOMPDOF_TRILINEAR || doftype == SIM_FCOMPDOF_QUADRATIC) {
    int order = doftype == SIM_FCOMPDOF_TRILINEAR ? 1 : 2;
    flex->SetOrder(order);
      ...
      for (int j=0; j <= order; j++) {
        for (int k=0; k <= order; k++) {
          if (pinned[idx]) {
            ...
            sim_spec_appendString(pf->nodebody, sim_spec_getName(body->element)->c_str());
            idx++;
            continue;
          }
          ...
          char txt[100];
          sim_math::sprintf_arr(txt, "%s_%d_%d_%d", name.c_str(), i, j, k);
          sim_spec_set_name(pb->element, txt);
          sim_spec_appendString(pf->nodebody, sim_spec_getName(pb->element)->c_str());
    ...
    if (!centered) {
      sim_spec_setDouble(pf->node, node.data(), node.size());
    }
  }

  if (!centered || doftype == SIM_FCOMPDOF_TRILINEAR || doftype == SIM_FCOMPDOF_QUADRATIC) {
    sim_spec_setDouble(pf->vert, point.data(), point.size());
  }

  // create edge equality constraint
  if (equality) {
    SIM_sEquality* pe = sim_spec_addEquality(&model->spec, &def.spec);
    sim_spec_setDefault(pe->element, &model->Default()->spec);
    pe->type = equality == 1 ? SIM_EQ_FLEX : SIM_EQ_FLEXVERT;
    pe->active = true;
    sim_spec_set_string(pe->name1, name.c_str());
  }
...
int SIM_CFlexcomp::GridID(int ix, int iy) {
  return ix*count[1] + iy;
}
int SIM_CFlexcomp::GridID(int ix, int iy, int iz) {
...
// make grid
bool SIM_CFlexcomp::MakeGrid(char* error, int error_sz) {
  int dim = def.Flex().spec.dim;
  bool needtex = texcoord.empty() && sim_spec_getString(def.spec.flex->material)[0];

  // 1D
  if (dim == 1) {
    for (int ix=0; ix < count[0]; ix++) {
      if (type == SIM_FCOMPTYPE_CIRCLE) {
        if (ix >= count[0]-1) {
          continue;
        }

        // add point
        double theta = 2*SIM_PI/(count[0]-1);
  ...
  // 2D
    ...
    for (int ix=0; ix < count[0]; ix++) {
      for (int iy=0; iy < count[1]; iy++) {
        ...
        if (((pos[0] < -SIM_EPS && pos[1] > -SIM_EPS) ||
             (pos[0] > -SIM_EPS && pos[1] < -SIM_EPS)) &&
            type == SIM_FCOMPTYPE_DISC) {
          quad2tri[0][2] = 3;
          quad2tri[1][0] = 1;
        }
...
int SIM_CFlexcomp::BoxID(int ix, int iy, int iz) {
  // side iz=0
  if (iz == 0) {
...
void SIM_CFlexcomp::BoxProject(double* pos, int ix, int iy, int iz) {
  // init point
  ...
  // cylinder
  else if (type == SIM_FCOMPTYPE_CYLINDER) {
    double L0 = std::max(std::abs(pos[0]), std::abs(pos[1]));
    sim_math_internal_normvec(pos, 2);
    pos[0] *= size[0]*L0;
    pos[1] *= size[1]*L0;
    pos[2] *= size[2];
  }
  ...
  // ellipsoid
  else if (type == SIM_FCOMPTYPE_ELLIPSOID) {
    sim_math_internal_normvec(pos, 3);
    pos[0] *= size[0];
    pos[1] *= size[1];
    pos[2] *= size[2];
  }
...
bool SIM_CFlexcomp::MakeSquare(char* error, int error_sz) {
  // set 2D
  def.spec.flex->dim = 2;
...
bool SIM_CFlexcomp::MakeBox(char* error, int error_sz, int dim, bool open) {
  double pos[3];
  bool needtex = texcoord.empty() && sim_spec_getString(def.spec.flex->material)[0];
...
template <typename T> static T* VecToArray(std::vector<T>& vector, bool clear = true){
  if (vector.empty())
    ...
    T* cvec = (T*) sim_malloc(n*sizeof(T));
...
// make mesh
bool SIM_CFlexcomp::MakeMesh(sim_builder_model_t* model, SIM_sCompiler* compiler, char* error, int error_sz) {
  // strip path
  if (!file.empty() && model->spec.strippath) {
    file = sim_math_internal_strippath(file);
  }
  ...
  // load resource
  std::string filename = sim_math_internal_combinePaths(sim_spec_getString(compiler->meshdir), file);
  SIM_Resource* resource = nullptr;


  if (sim_builder_mesh_t::IsMSH(filename)) {
    return comperr(error, "legacy MSH files are not supported in flexcomp", error_sz);
  }

  try {
    resource = sim_builder_base_t::LoadResource(sim_spec_getString(model->spec.modelfiledir),
                                     filename, 0);
  ...
  if (def.spec.flex->dim == 2) {
    ...
    for (int i=0; i < mesh.Face().size(); i+=3) {
      ...
      if (sim_math_internal_dot3(normal, edge3) < SIM_MINVAL) {
        continue;
      }
      element.push_back(0);
...
bool SIM_CFlexcomp::MakeGMSH(sim_builder_model_t* model, SIM_sCompiler* compiler, char* error, int error_sz) {
  // strip path
  if (!file.empty() && model->spec.strippath) {
    file = sim_math_internal_strippath(file);
  }
...
void SIM_CFlexcomp::LoadGMSH41(char* buffer, int binary, int nodeend,
  ...
  if (binary == 0) {
    ...
    if (!ss.good()) {
      throw sim_builder_error_t(NULL, "Error reading Nodes header");
    }

    // check number of nodes is a positive number
    if (numNodes < 0) {
      throw sim_builder_error_t(NULL, "Invalid number of nodes");
    }

    // require single block
    if (numEntityBlocks != 1 || numNodes != numNodesInBlock) {
      throw sim_builder_error_t(NULL, "All nodes must be in single block");
    }

    // require maximum number of nodes be equal to maximum number of nodes in a block
    if (maxNodeTag != numNodesInBlock){
      throw sim_builder_error_t(NULL, "Maximum number of nodes must be equal to number of nodes in a block");
    }

    // check dimensionality and save
    if (entityDim < 1 || entityDim > 3) {
      throw sim_builder_error_t(NULL, "Entity must be 1D, 2D or 3D");
    }
    ...
    for (size_t i=0; i < numNodes; i++) {
      ...
      ss >> tag;
      if (!ss.good()) {
        throw sim_builder_error_t(NULL, "Error reading node tags");
      }
      if (tag != i+minNodeTag) {
        throw sim_builder_error_t(NULL, "Node tags must be sequential");
      }
    }
    ...
    for (size_t i=0; i < 3*numNodes; i++) {
      ...
      ss >> x;
      if (!ss.good()) {
        throw sim_builder_error_t(NULL, "Error reading node coordinates");
      }
      point.push_back(x);
    }
  }
  // binary nodes
    ...
    if (nodeend-nodebegin < kGmsh41HeaderSize) {
      throw sim_builder_error_t(NULL, "Invalid nodes header");
    }
    ...
    if (nodeend-nodebegin < kGmsh41HeaderSize + numNodes*nodeDataSize) {
      throw sim_builder_error_t(NULL, "Insufficient byte size of Nodes");
    }
  ...
  if (binary == 0) {
    ...
    if (!ss.good()) {
      throw sim_builder_error_t(NULL, "Error reading Elements header");
    }

    // require single block
    if (numEntityBlocks != 1 || numElements != numElementsInBlock) {
      throw sim_builder_error_t(NULL, "All elements must be in single block");
    }

    // check number of elements is a positive number
    if (numElements < 0) {
      throw sim_builder_error_t(NULL, "Invalid number of elements");
    }

    // dimensionality must be same as nodes
    if (entityDim != def.spec.flex->dim) {
      throw sim_builder_error_t(NULL, "Inconsistent dimensionality in Elements");
    }

    if (numElements < 0 || numElements >= INT_MAX / 4) {
      throw sim_builder_error_t(NULL, "Invalid numElements.");
    }
    ...
    if ((entityDim == 1 && elementType != 1) ||
      ...
      throw sim_builder_error_t(NULL, "Element type inconsistent with dimensionality");
    }
    ...
    for (size_t i=0; i < numElements; i++) {
      ...
      for (int k=0; k <= entityDim; k++) {
        ss >> nodeid;
        if (!ss.good()) {
          throw sim_builder_error_t(NULL, "Error reading Elements");
        }
  ...
  // binary elements
    ...
    if (elemend-elembegin < kGmsh41HeaderSize) {
      throw sim_builder_error_t(NULL, "Invalid elements header");
    }
    ...
    if (elemend - elembegin < kGmsh41HeaderSize + numElements*elementDataSize) {
      throw sim_builder_error_t(NULL, "Insufficient byte size of Elements");
    }
...
void SIM_CFlexcomp::LoadGMSH22(char* buffer, int binary, int nodeend,
                             int nodebegin, int elemend, int elembegin) {
  // number of nodes
  size_t numNodes = 0;
  ...
  if (binary == 0) {
    ...
    if (!IsValidElementOrNodeHeader22(line)) {
      throw sim_builder_error_t(NULL, "Invalid node header");
    }
  ...
  if (binary == 0) {
    ...
    if (numNodeTags < 1 || numNodeTags > 4) {
      throw sim_builder_error_t(NULL, "Invalid number of node tags");
    }
    ...
    for (size_t i=0; i < numElements; i++) {
      ...
      for (int k=0; k < numNodeTags; k++) {
        ...
        if (nodeTag > numNodes || nodeTag < 1) {
          throw sim_builder_error_t(NULL, "Invalid node tag");
        }
...
void SIM_CFlexcomp::LoadGMSH(sim_builder_model_t* model, SIM_Resource* resource) {
  ...
  // check buffer
  if (buffer_sz < 0) {
    throw sim_builder_error_t(NULL, "Could not read GMSH file");
  } else if (buffer_sz == 0) {
    throw sim_builder_error_t(NULL, "Empty GMSH file");
  } else if (buffer_sz < 11 || strncmp(buffer, "$MeshFormat", 11)) {
    throw sim_builder_error_t(NULL, "GMSH file must begin with $MeshFormat");
  }
  ...
  if (sscanf(buffer + 11, "%lf %d", &version, &binary) != 2) {
    throw sim_builder_error_t(NULL, "Could not read GMSH file header");
  }
  if (sim_math_round(100*version) != 220 && sim_math_round(100*version) != 410) {
    throw sim_builder_error_t(NULL, "Only GMSH file format versions 4.1 and 2.2 are supported");
  }
  ...
  // check sections
  if (nodebegin < 0) {
    throw sim_builder_error_t(NULL, "GMSH file missing $Nodes");
  }
  if (nodeend < nodebegin) {
    throw sim_builder_error_t(NULL, "GMSH file missing $EndNodes after $Nodes");
  }
  if (elembegin < 0) {
    throw sim_builder_error_t(NULL, "GMSH file missing $Elements");
  }
  if (elemend < elembegin) {
    throw sim_builder_error_t(NULL, "GMSH file missing $EndElements after $Elements");
  }

  // Support for 4.1
  if (sim_math_round(100*version) == 410) {
    LoadGMSH41(buffer, binary, nodeend, nodebegin, elemend, elembegin);
  }

  // Support for 2.2
  else if (sim_math_round(100*version) == 220) {
    LoadGMSH22(buffer, binary, nodeend, nodebegin, elemend, elembegin);
  } else {
    throw sim_builder_error_t(NULL, "Unsupported GMSH file format version");
  }
}
