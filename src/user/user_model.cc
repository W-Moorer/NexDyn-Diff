...
int GetBodyIdFromWrap(const SIM_CWrap* wrap) {
  if (!wrap || !wrap->obj) return -1;
  switch (wrap->Type()) {
    case SIM_WRAP_SITE:
      return static_cast<sim_builder_site_t*>(wrap->obj)->Body()->id;
    case SIM_WRAP_CYLINDER:
    case SIM_WRAP_SPHERE:
      return static_cast<sim_builder_geom_t*>(wrap->obj)->GetParent()->id;
    default:
      return -1;
  }
}
...
// constructor
sim_builder_model_t::sim_builder_model_t() {
  sim_spec_defaultSpec(&spec);
  elemtype = SIM_OBJ_MODEL;
  spec_comment_.clear();
  spec_modelfiledir_.clear();
  meshdir_.clear();
  texturedir_.clear();
  spec_modelname_ = "SimCore Model";
  ...
  //------------------------ master default set
  defaults_.push_back(new sim_builder_default_t(this));
  ...
  // world body
  sim_builder_body_t* world = new sim_builder_body_t(this);
  sim_math_internal_zerovec(world->pos, 3);
  sim_math_internal_set_vec(world->quat, 1, 0, 0, 0);
  world->mass = 0;
  sim_math_internal_zerovec(world->inertia, 3);
  world->id = 0;
...
sim_builder_model_t::sim_builder_model_t(const sim_builder_model_t& other) {
  CreateObjectLists();
  *this = other;
}
...
sim_builder_model_t& sim_builder_model_t::operator=(const sim_builder_model_t& other) {
  deepcopy_ = true;
  if (this != &other) {
    this->spec = other.spec;
    *static_cast<SIM_CModel_*>(this) = static_cast<const SIM_CModel_&>(other);
    *static_cast<sim_spec_t*>(this) = static_cast<const sim_spec_t&>(other);
    PointToLocal();

    // copy attached specs first so that we can resolve references to them
    for (auto* s : other.specs_) {
      specs_.push_back(s);
      static_cast<sim_builder_model_t*>(s->element)->AddRef();
      compiler2spec_[&s->compiler] = specs_.back();
    }

    // the world copy constructor takes care of copying the tree
    sim_builder_body_t* world = new sim_builder_body_t(*other.bodies_[0], this);
    bodies_.push_back(world);
    ...
    *this += other;
    ...
    sim_builder_default_t* subtree = new sim_builder_default_t(*other.defaults_[0]);
    *this += *subtree;

    // copy name maps
    for (int i=0; i < SIM_NOBJECT; i++) {
      ids[i] = other.ids[i];
    }
...
template <class T>
void sim_builder_model_t::CopyList(std::vector<T*>& dest,
  ...
  for (int i = 0; i < nsource; i++) {
    ...
    try {
      ...
      sim_builder_model_t* source_model = source[i]->model;
    ...
    sim_spec_t* origin = FindSpec(source[i]->compiler);
...
void sim_builder_model_t::ResetTreeLists() {
  sim_builder_body_t *world = bodies_[0];
  resetlist(bodies_);
  resetlist(joints_);
  resetlist(geoms_);
  resetlist(sites_);
...
void sim_builder_model_t::SaveDofOffsets(bool computesize) {
  int qposadr = 0;
  int dofadr = 0;
  int actadr = 0;
  int mocapadr = 0;
  ...
  for (auto actuator : actuators_) {
    if (actuator->spec.actdim > 0) {
      ...
      actuator->actdim_ = (actuator->spec.dyntype != SIM_DYN_NONE);
    }
    actuator->actadr_ = actuator->actdim_ ? actadr : -1;
    actadr += actuator->actdim_;
  }

  for (sim_builder_body_t* body : bodies_) {
    if (body->spec.mocap) {
...
template <class T>
void sim_builder_model_t::CopyExplicitPlugin(T* obj) {
  if (!obj->plugin.active || !obj->plugin_instance_name.empty() || !obj->spec.plugin.element) {
    return;
  }
  sim_builder_plugin_t* origin = static_cast<sim_builder_plugin_t*>(obj->spec.plugin.element);
  sim_builder_plugin_t* candidate = deepcopy_ ? new sim_builder_plugin_t(*origin) : origin;
...
template void sim_builder_model_t::CopyExplicitPlugin<sim_builder_body_t>(sim_builder_body_t* obj);
template void sim_builder_model_t::CopyExplicitPlugin<sim_builder_geom_t>(sim_builder_geom_t* obj);
template void sim_builder_model_t::CopyExplicitPlugin<sim_builder_mesh_t>(sim_builder_mesh_t* obj);
template void sim_builder_model_t::CopyExplicitPlugin<SIM_CActuator>(SIM_CActuator* obj);
template void sim_builder_model_t::CopyExplicitPlugin<SIM_CSensor>(SIM_CSensor* obj);



template <class T>
void sim_builder_model_t::CopyPlugin(const std::vector<sim_builder_plugin_t*>& source,
  ...
  for (const auto& plugin : source) {
    ...
    sim_builder_plugin_t* candidate = new sim_builder_plugin_t(*plugin);
    candidate->model = this;
    candidate->NameSpace(plugin->model);
    bool referenced = instances.find(candidate->name) != instances.end();
    auto same_name = [candidate](const sim_builder_plugin_t* dest) {
...
static bool IsPluginActive(
  ...
  const std::vector<std::pair<const SIM_pPlugin*, int> >& active_plugins) {
  return std::find_if(
    active_plugins.begin(), active_plugins.end(),
    [&plugin](const std::pair<const SIM_pPlugin*, int>& element) {
...
template <class T>
void sim_builder_model_t::RemoveFromList(std::vector<T*>& list, const sim_builder_model_t& other) {
...
template <>
void sim_builder_model_t::DeleteAll<SIM_CKey>(std::vector<SIM_CKey*>& elements) {
  for (SIM_CKey* element : elements) {
    element->Release();
  }
  elements.clear();
}
...
template <class T>
void sim_builder_model_t::MarkPluginInstance(std::unordered_map<std::string, bool>& instances,
                                  const std::vector<T*>& list) {
  for (const auto& element : list) {
    if (!element->plugin_instance_name.empty()) {
...
void sim_builder_model_t::RemovePlugins() {
  // store elements that reference a plugin instance
...
sim_builder_model_t& sim_builder_model_t::operator-=(const sim_builder_body_t& subtree) {
  sim_builder_model_t oldmodel(*this);
...
sim_builder_model_t& sim_builder_model_t::operator+=(sim_builder_default_t& subtree) {
...
sim_builder_model_t& sim_builder_model_t::operator-=(const sim_builder_default_t& subtree) {

  // check we aren't trying to remove the 'main' default
  if (subtree.id == 0) {
    throw sim_builder_error_t(0, "cannot remove the global default ('main')");
  }

  // remove this default from parent's child list
  sim_builder_default_t* parent = subtree.parent;
  if (parent) {
...
template <class T>
void deletefromlist(std::vector<T*>* list, sim_spec_element_t* element) {
  if (!list) {
    return;
  }
...
void sim_builder_model_t::DeleteSubtreePlugin(sim_builder_body_t* subtree) {
  SIM_sPlugin* plugin = &(subtree->spec.plugin);
...
void sim_builder_model_t::operator-=(sim_spec_element_t* el) {
  if (el->elemtype == SIM_OBJ_BODY) {
    sim_builder_body_t* body = static_cast<sim_builder_body_t*>(el);
    *this -= *body;
  }

  detached_.push_back(static_cast<sim_builder_base_t*>(el));
  ResetTreeLists();

  if (el->elemtype != SIM_OBJ_DEFAULT) {
    if (static_cast<sim_builder_base_t*>(el)->model != this) {
      throw sim_builder_error_t(nullptr, "element is not in this model");
    }
  } else {
    if (static_cast<sim_builder_default_t*>(el)->model != this) {
      throw sim_builder_error_t(nullptr, "default is not in this model");
    }
  }
  ...
  switch (el->elemtype) {
    case SIM_OBJ_BODY:
    {
      ...
      sim_builder_body_t* subtree = static_cast<sim_builder_body_t*>(el);
    ...
    case SIM_OBJ_DEFAULT:
      MakeTreeLists();  // rebuild lists that were reset at the beginning of the function
      throw sim_builder_error_t(nullptr, "defaults cannot be deleted, use detach instead");
      break;

    case SIM_OBJ_GEOM:
    {
      sim_builder_geom_t* geom = static_cast<sim_builder_geom_t*>(el);
    ...
    case SIM_OBJ_SITE:
      deletefromlist(&(static_cast<sim_builder_site_t*>(el)->body->sites), el);
      break;

    case SIM_OBJ_JOINT:
      deletefromlist(&(static_cast<sim_builder_joint_t*>(el)->body->joints), el);
      break;

    case SIM_OBJ_LIGHT:
      deletefromlist(&(static_cast<SIM_CLight*>(el)->body->lights), el);
      break;

    case SIM_OBJ_CAMERA:
      deletefromlist(&(static_cast<SIM_CCamera*>(el)->body->cameras), el);
      break;

    case SIM_OBJ_MESH:
    {
      sim_builder_mesh_t* mesh = static_cast<sim_builder_mesh_t*>(el);
      if (mesh->plugin.active && mesh->plugin.name->empty()) {
        *this -= mesh->plugin.element;
      }
      deletefromlist(object_lists_[SIM_OBJ_MESH], el);
      break;
    }

    case SIM_OBJ_ACTUATOR:
    {
      SIM_CActuator* actuator = static_cast<SIM_CActuator*>(el);
      if (actuator->plugin.active && actuator->plugin.name->empty()) {
        *this -= actuator->plugin.element;
      }
      deletefromlist(object_lists_[SIM_OBJ_ACTUATOR], el);
      break;
    }

    case SIM_OBJ_SENSOR:
    {
      SIM_CSensor* sensor = static_cast<SIM_CSensor*>(el);
      if (sensor->plugin.active && sensor->plugin.name->empty()) {
        *this -= sensor->plugin.element;
      }
      deletefromlist(object_lists_[SIM_OBJ_SENSOR], el);
      break;
    }
...
void sim_builder_model_t::CreateObjectLists() {
  for (int i = 0; i < SIM_NOBJECT; ++i) {
    object_lists_[i] = nullptr;
  }

  object_lists_[SIM_OBJ_BODY]     = (std::vector<sim_builder_base_t*>*) &bodies_;
  object_lists_[SIM_OBJ_XBODY]    = (std::vector<sim_builder_base_t*>*) &bodies_;
  object_lists_[SIM_OBJ_JOINT]    = (std::vector<sim_builder_base_t*>*) &joints_;
  object_lists_[SIM_OBJ_GEOM]     = (std::vector<sim_builder_base_t*>*) &geoms_;
  object_lists_[SIM_OBJ_SITE]     = (std::vector<sim_builder_base_t*>*) &sites_;
  object_lists_[SIM_OBJ_CAMERA]   = (std::vector<sim_builder_base_t*>*) &cameras_;
  object_lists_[SIM_OBJ_LIGHT]    = (std::vector<sim_builder_base_t*>*) &lights_;
  object_lists_[SIM_OBJ_FLEX]     = (std::vector<sim_builder_base_t*>*) &flexes_;
  object_lists_[SIM_OBJ_MESH]     = (std::vector<sim_builder_base_t*>*) &meshes_;
  object_lists_[SIM_OBJ_SKIN]     = (std::vector<sim_builder_base_t*>*) &skins_;
  object_lists_[SIM_OBJ_HFIELD]   = (std::vector<sim_builder_base_t*>*) &hfields_;
  object_lists_[SIM_OBJ_TEXTURE]  = (std::vector<sim_builder_base_t*>*) &textures_;
  object_lists_[SIM_OBJ_MATERIAL] = (std::vector<sim_builder_base_t*>*) &materials_;
  object_lists_[SIM_OBJ_PAIR]     = (std::vector<sim_builder_base_t*>*) &pairs_;
  object_lists_[SIM_OBJ_EXCLUDE]  = (std::vector<sim_builder_base_t*>*) &excludes_;
  object_lists_[SIM_OBJ_EQUALITY] = (std::vector<sim_builder_base_t*>*) &equalities_;
  object_lists_[SIM_OBJ_TENDON]   = (std::vector<sim_builder_base_t*>*) &tendons_;
  object_lists_[SIM_OBJ_ACTUATOR] = (std::vector<sim_builder_base_t*>*) &actuators_;
  object_lists_[SIM_OBJ_SENSOR]   = (std::vector<sim_builder_base_t*>*) &sensors_;
  object_lists_[SIM_OBJ_NUMERIC]  = (std::vector<sim_builder_base_t*>*) &numerics_;
  object_lists_[SIM_OBJ_TEXT]     = (std::vector<sim_builder_base_t*>*) &texts_;
  object_lists_[SIM_OBJ_TUPLE]    = (std::vector<sim_builder_base_t*>*) &tuples_;
  object_lists_[SIM_OBJ_KEY]      = (std::vector<sim_builder_base_t*>*) &keys_;
  object_lists_[SIM_OBJ_PLUGIN]   = (std::vector<sim_builder_base_t*>*) &plugins_;
}



void sim_builder_model_t::PointToLocal() {
...
void sim_builder_model_t::CopyFromSpec() {
  *static_cast<sim_spec_t*>(this) = spec;
...
void sim_builder_model_t::ComputeSparseSizes() {
  ...
  for (int i = 0; i < nbody; ++i) {
    sim_builder_body_t* pb = bodies_[i];
    sim_builder_body_t* par = pb->parent;
    ...
    for (const auto* jnt : pb->joints) {
      for (int j1 = 0; j1 < jnt->nv(); ++j1) {
        ...
        if (dofadr < 0 || dofadr >= nv) {
          throw sim_builder_error_t(jnt, "dofadr out of bounds: dofadr=%d, nv=%d", nullptr, dofadr, nv);
        }
  ...
  for (int i = 0; i < nbody; ++i) {
    ...
    sim_builder_body_t* parent = bodies_[i]->parent;
  ...
  if (bodies_[0]->subtreedofs != nv) {
    throw sim_builder_error_t(0, "all DOFs should be in world subtree");
  }
  ...
  for (int i = 1; i < nbody; ++i) {
    ...
    for (const auto* pj : pb->joints) {
      bool axis_aligned = ((std::abs(pj->axis[0]) > SIM_EPS) +
                           (std::abs(pj->axis[1]) > SIM_EPS) +
                           (std::abs(pj->axis[2]) > SIM_EPS)) == 1;
      if (rotfound || !IsNullPose(pj->pos, nulldouble) ||
          ((pj->type == SIM_JNT_HINGE || pj->type == SIM_JNT_SLIDE) && !axis_aligned)) {
        body_simple_pre[i] = 0;
        break;
      }
      if (pj->type == SIM_JNT_BALL || pj->type == SIM_JNT_HINGE) {
        rotfound = 1;
      }
    }
    if (!body_simple_pre[i]) continue;
  ...
  for (int i = nv - 1; i >= 0; --i) {
    if (dof_bodyid_pre[i] < 0 || dof_bodyid_pre[i] >= nbody) {
      throw sim_builder_error_t(0, "dof_bodyid out of bounds: dof=%d, body=%d", nullptr, i, dof_bodyid_pre[i]);
    }
...
sim_builder_model_t::~sim_builder_model_t() {
  ...
  // delete objects allocated in sim_builder_model_t
  ...
  for (int i=0; i < specs_.size(); i++) sim_deleteSpec(specs_[i]);
...
void sim_builder_model_t::Clear() {
...
template <class T>
T* sim_builder_model_t::AddObject(vector<T*>& list, string type) {
  T* obj = new T(this);
...
template <class T>
T* sim_builder_model_t::AddObjectDefault(vector<T*>& list, string type, sim_builder_default_t* def) {
...
// add flex
SIM_CFlex* sim_builder_model_t::AddFlex() {
  return AddObject(flexes_, "flex");
}


// add mesh
sim_builder_mesh_t* sim_builder_model_t::AddMesh(sim_builder_default_t* def) {
  return AddObjectDefault(meshes_, "mesh", def);
}


// add skin
SIM_CSkin* sim_builder_model_t::AddSkin() {
  return AddObject(skins_, "skin");
}


// add hfield
SIM_CHField* sim_builder_model_t::AddHField() {
  return AddObject(hfields_, "hfield");
}


// add texture
sim_builder_texture_t* sim_builder_model_t::AddTexture() {
  return AddObject(textures_, "texture");
}


// add material
SIM_CMaterial* sim_builder_model_t::AddMaterial(sim_builder_default_t* def) {
  return AddObjectDefault(materials_, "material", def);
}


// add geom pair to include in collisions
SIM_CPair* sim_builder_model_t::AddPair(sim_builder_default_t* def) {
  return AddObjectDefault(pairs_, "pair", def);
}


// add body pair to exclude from collisions
SIM_CBodyPair* sim_builder_model_t::AddExclude() {
  return AddObject(excludes_, "exclude");
}


// add constraint
SIM_CEquality* sim_builder_model_t::AddEquality(sim_builder_default_t* def) {
  return AddObjectDefault(equalities_, "equality", def);
}


// add tendon
sim_builder_tendon_t* sim_builder_model_t::AddTendon(sim_builder_default_t* def) {
  return AddObjectDefault(tendons_, "tendon", def);
}


// add actuator
SIM_CActuator* sim_builder_model_t::AddActuator(sim_builder_default_t* def) {
  return AddObjectDefault(actuators_, "actuator", def);
}


// add sensor
SIM_CSensor* sim_builder_model_t::AddSensor() {
...
// add custom
SIM_CNumeric* sim_builder_model_t::AddNumeric() {
  return AddObject(numerics_, "numeric");
}


// add text
SIM_CText* sim_builder_model_t::AddText() {
  return AddObject(texts_, "text");
}


// add tuple
SIM_CTuple* sim_builder_model_t::AddTuple() {
  return AddObject(tuples_, "tuple");
}


// add keyframe
SIM_CKey* sim_builder_model_t::AddKey() {
  return AddObject(keys_, "key");
}


// add plugin instance
sim_builder_plugin_t* sim_builder_model_t::AddPlugin() {
  return AddObject(plugins_, "plugin");
}


// append spec to spec
void sim_builder_model_t::AppendSpec(sim_spec_t* spec, const SIM_sCompiler* compiler_) {
...
int sim_builder_model_t::NumObjects(sim_obj_t type) {
...
sim_builder_base_t* sim_builder_model_t::GetObject(sim_obj_t type, int id) {
...
template <class T>
static sim_spec_element_t* GetNext(std::vector<T*>& list, sim_spec_element_t* child) {
  if (!child) {
    if (list.empty()) {
      return nullptr;
    }
...
sim_spec_element_t* sim_builder_model_t::NextObject(sim_spec_element_t* object, sim_obj_t type) {
  if (type == SIM_OBJ_UNKNOWN) {
    if (!object) {
      throw sim_builder_error_t(nullptr, "type must be specified if no element is given");
    } else {
      type = object->elemtype;
    }
  } else if (object && object->elemtype != type) {
    throw sim_builder_error_t(nullptr, "element is not of requested type");
  }
...
// compiled flag
bool sim_builder_model_t::IsCompiled() const {
  return compiled;
}
...
const sim_builder_error_t& sim_builder_model_t::GetError() const {
  return errInfo;
}
...
sim_builder_body_t* sim_builder_model_t::GetWorld() {
  return bodies_[0];
}
...
sim_builder_default_t* sim_builder_model_t::FindDefault(string name) {
...
sim_builder_default_t* sim_builder_model_t::AddDefault(string name, sim_builder_default_t* parent) {
  ...
  // create new object
  sim_builder_default_t* def = new sim_builder_default_t(parent->model);
...
template <class T>
static T* findobject(std::string_view name, const vector<T*>& list, const SIM_KeyMap& ids) {
  ...
  if (id->second > (int)list.size() - 1) {
    throw sim_builder_error_t(0, "object not found");
  }
...
template <class T>
sim_builder_base_t* sim_builder_model_t::FindAsset(std::string_view name, const std::vector<T*>& list) const {
  for (unsigned int i=0; i < list.size(); i++) {
    if (list[i]->name == name) {
      return list[i];
    }
...
template sim_builder_base_t* sim_builder_model_t::FindAsset<sim_builder_texture_t>(
    std::string_view name, const std::vector<sim_builder_texture_t*>& list) const;
template sim_builder_base_t* sim_builder_model_t::FindAsset<sim_builder_mesh_t>(
    std::string_view name, const std::vector<sim_builder_mesh_t*>& list) const;



// find object in global lists given string type and name
sim_builder_base_t* sim_builder_model_t::FindObject(sim_obj_t type, string name) const {
  if (!object_lists_[type]) {
    return nullptr;
  }
...
// find body by name
sim_builder_base_t* sim_builder_model_t::FindTree(sim_builder_body_t* body, sim_obj_t type, std::string name) {
  switch (type) {
    case SIM_OBJ_BODY:
      if (body->name == name) {
        return body;
...
// find spec by name
sim_spec_t* sim_builder_model_t::FindSpec(std::string name) const {
  for (auto spec : specs_) {
    if (sim_spec_getString(spec->modelname) == name) {
      return spec;
    }
  }
  return nullptr;
...
// find spec by SIM_sCompiler pointer
sim_spec_t* sim_builder_model_t::FindSpec(const SIM_sCompiler* compiler_) {
  ...
  for (auto s : specs_) {
    sim_spec_t* source = static_cast<sim_builder_model_t*>(s->element)->FindSpec(compiler_);
    if (source) {
      return source;
    }
  }
...
void sim_builder_model_t::MakeTreeLists(sim_builder_body_t* body) {
  if (body == nullptr) {
    body = bodies_[0];
  }
  ...
  for (sim_builder_geom_t *geom : body->geoms) geoms_.push_back(geom);
  for (sim_builder_joint_t *joint : body->joints) joints_.push_back(joint);
  for (sim_builder_site_t *site : body->sites) sites_.push_back(site);
  for (SIM_CCamera *camera : body->cameras) cameras_.push_back(camera);
  for (SIM_CLight *light : body->lights) lights_.push_back(light);
  for (sim_builder_frame_t *frame : body->frames) frames_.push_back(frame);

  // recursive call to all child bodies
  for (sim_builder_body_t* body : body->bodies) MakeTreeLists(body);
}


// delete material with given name or all materials if the name is omitted
template <class T>
void sim_builder_model_t::DeleteMaterial(std::vector<T*>& list, std::string_view name) {
  for (T* plist : list) {
...
template <>
void sim_builder_model_t::Delete<sim_builder_geom_t>(std::vector<sim_builder_geom_t*>& elements,
                               const std::vector<bool>& discard) {
  // update bodies
  for (sim_builder_body_t* body : bodies_) {
    body->geoms.erase(
      std::remove_if(body->geoms.begin(), body->geoms.end(),
                     [&discard](sim_builder_geom_t* geom) {
...
template <>
void sim_builder_model_t::Delete<sim_builder_mesh_t>(std::vector<sim_builder_mesh_t*>& elements,
                               const std::vector<bool>& discard) {
  DeleteElements(elements, discard);
}
...
template <>
void sim_builder_model_t::DeleteAll<SIM_CMaterial>(std::vector<SIM_CMaterial*>& elements) {
  DeleteMaterial(geoms_);
  DeleteMaterial(skins_);
  DeleteMaterial(sites_);
  DeleteMaterial(tendons_);
  for (SIM_CMaterial* element : elements) {
    element->Release();
  }
  elements.clear();
}
...
template <>
void sim_builder_model_t::DeleteAll<sim_builder_texture_t>(std::vector<sim_builder_texture_t*>& elements) {
  DeleteAllTextures(materials_);
  for (sim_builder_texture_t* element : elements) {
    element->Release();
  }
  elements.clear();
}
...
// set nuser fields
void sim_builder_model_t::SetNuser() {
  if (nuser_body == -1) {
    nuser_body = 0;
    for (int i = 0; i < bodies_.size(); i++) {
      nuser_body = SIM_MAX(nuser_body, bodies_[i]->spec_userdata_.size());
    }
  }
  if (nuser_jnt == -1) {
    nuser_jnt = 0;
    for (int i = 0; i < joints_.size(); i++) {
      nuser_jnt = SIM_MAX(nuser_jnt, joints_[i]->spec_userdata_.size());
    }
  }
  if (nuser_geom == -1) {
    nuser_geom = 0;
    for (int i = 0; i < geoms_.size(); i++) {
      nuser_geom = SIM_MAX(nuser_geom, geoms_[i]->spec_userdata_.size());
    }
  }
  if (nuser_site == -1) {
    nuser_site = 0;
    for (int i = 0; i < sites_.size(); i++) {
      nuser_site = SIM_MAX(nuser_site, sites_[i]->spec_userdata_.size());
    }
  }
  if (nuser_cam == -1) {
    nuser_cam = 0;
    for (int i = 0; i < cameras_.size(); i++) {
      nuser_cam = SIM_MAX(nuser_cam, cameras_[i]->spec_userdata_.size());
    }
  }
  if (nuser_tendon == -1) {
    nuser_tendon = 0;
    for (int i = 0; i < tendons_.size(); i++) {
      nuser_tendon = SIM_MAX(nuser_tendon, tendons_[i]->spec_userdata_.size());
    }
  }
  if (nuser_actuator == -1) {
    nuser_actuator = 0;
    for (int i = 0; i < actuators_.size(); i++) {
      nuser_actuator = SIM_MAX(nuser_actuator, actuators_[i]->spec_userdata_.size());
    }
  }
  if (nuser_sensor == -1) {
    nuser_sensor = 0;
    for (int i = 0; i < sensors_.size(); i++) {
      nuser_sensor = SIM_MAX(nuser_sensor, sensors_[i]->spec_userdata_.size());
    }
  }
}

// index assets
void sim_builder_model_t::IndexAssets(bool discard) {
  // assets referenced in geoms
  for (int i=0; i < geoms_.size(); i++) {
    sim_builder_geom_t* geom = geoms_[i];


    // find mesh by name
    if (!geom->get_meshname().empty()) {
      sim_builder_mesh_t* mesh = static_cast<sim_builder_mesh_t*>(FindObject(SIM_OBJ_MESH, geom->get_meshname()));
      if (mesh) {
        if (!geom->visual_) {
          mesh->SetNotVisual();  // reset to true by mesh->Compile()
        }
        geom->mesh = (discard && geom->visual_) ? nullptr : mesh;
        mesh->spec.needsdf |= geom->spec.type == SIM_GEOM_SDF;
      } else {
        throw sim_builder_error_t(geom, "mesh '%s' not found in geom %d", geom->get_meshname().c_str(), i);
      }
    }
    ...
    if (!geom->get_material().empty()) {
      sim_builder_base_t* material = FindObject(SIM_OBJ_MATERIAL, geom->get_material());
      if (material) {
        geom->matid = material->id;
      } else {
        throw sim_builder_error_t(geom, "material '%s' not found in geom %d", geom->get_material().c_str(), i);
      }
    }

    // find hfield by name
    if (!geom->get_hfieldname().empty()) {
      sim_builder_base_t* hfield = FindObject(SIM_OBJ_HFIELD, geom->get_hfieldname());
      if (hfield) {
        geom->hfield = (SIM_CHField*)hfield;
      } else {
        throw sim_builder_error_t(geom, "hfield '%s' not found in geom %d", geom->get_hfieldname().c_str(), i);
      }
    }
  }
  ...
  for (int i=0; i < skins_.size(); i++) {
    SIM_CSkin* skin = skins_[i];

    // find material by name
    if (!skin->material_.empty()) {
      sim_builder_base_t* material = FindObject(SIM_OBJ_MATERIAL, skin->material_);
      if (material) {
        skin->matid = material->id;
      } else {
        throw sim_builder_error_t(skin, "material '%s' not found in skin %d", skin->material_.c_str(), i);
      }
    }
  }
  ...
  for (int i=0; i < sites_.size(); i++) {
    sim_builder_site_t* site = sites_[i];

    // find material by name
    if (!site->material_.empty()) {
      sim_builder_base_t* material = FindObject(SIM_OBJ_MATERIAL, site->get_material());
      if (material) {
        site->matid = material->id;
      } else {
        throw sim_builder_error_t(site, "material '%s' not found in site %d", site->material_.c_str(), i);
      }
    }
  }
  ...
  for (int i=0; i < tendons_.size(); i++) {
    sim_builder_tendon_t* tendon = tendons_[i];

    // find material by name
    if (!tendon->material_.empty()) {
      sim_builder_base_t* material = FindObject(SIM_OBJ_MATERIAL, tendon->material_);
      if (material) {
        tendon->matid = material->id;
      } else {
        throw sim_builder_error_t(tendon, "material '%s' not found in tendon %d", tendon->material_.c_str(), i);
      }
    }
  }
  ...
  for (int i=0; i < materials_.size(); i++) {
    SIM_CMaterial* material = materials_[i];

    // find textures by name
    for (int j=0; j < SIM_NTEXROLE; j++) {
      if (!material->textures_[j].empty()) {
        sim_builder_base_t* texture = FindObject(SIM_OBJ_TEXTURE, material->textures_[j]);
        if (texture) {
          material->texid[j] = texture->id;
        } else {
          throw sim_builder_error_t(material, "texture '%s' not found in material %d", material->textures_[j].c_str(), i);
        }
      }
    }
  }
  ...
  if (discard) {
    ...
    for (auto body : bodies_) {
      ...
      for (auto geom : body->geoms) {
        if (geom->IsVisual()) {
          if (compiler.inertiafromgeom == SIM_INERTIAFROMGEOM_TRUE) {
            compiler.inertiafromgeom = SIM_INERTIAFROMGEOM_AUTO;
          }
          body->explicitinertial = true;  // for XML writer
          body->spec.explicitinertial = true;
          body->spec.mass = body->mass;
          sim_math_internal_copy_vec(body->spec.ipos, body->ipos, 3);
          sim_math_internal_copy_vec(body->spec.iquat, body->iquat, 4);
          sim_math_internal_copy_vec(body->spec.inertia, body->inertia, 3);
          break;
        }
      }
    }
...
void sim_builder_model_t::CheckEmptyNames(void) {
  // meshes
  for (int i=0; i < meshes_.size(); i++) {
    if (meshes_[i]->name.empty()) {
      throw sim_builder_error_t(meshes_[i], "empty name in mesh");
    }
  }
  ...
  for (int i=0; i < hfields_.size(); i++) {
    if (hfields_[i]->name.empty()) {
      throw sim_builder_error_t(hfields_[i], "empty name in height field");
    }
  }

  // textures
  for (int i=0; i < textures_.size(); i++) {
    if (textures_[i]->name.empty() && textures_[i]->type != SIM_TEXTURE_SKYBOX) {
      throw sim_builder_error_t(textures_[i], "empty name in texture");
    }
  }
  ...
  for (int i=0; i < materials_.size(); i++) {
    if (materials_[i]->name.empty()) {
      throw sim_builder_error_t(materials_[i], "empty name in material");
    }
  }
}
...
// set array sizes
void sim_builder_model_t::SetSizes() {
  ...
  for (int i=0; i < nflex; i++) {
    ...
    nflexelemedge += flexes_[i]->nelem * SIM_CFlex::kNumEdges[flexes_[i]->dim - 1];
    ...
    for (const auto& edge : flexes_[i]->edge) {
      sim_builder_body_t* b1 = bodies_[flexes_[i]->vertbodyid[edge.first]];
      sim_builder_body_t* b2 = bodies_[flexes_[i]->vertbodyid[edge.second]];
      std::unordered_set<sim_builder_body_t*> bodies_in_jac;
      while (b1 || b2) {
        if (b1) {
      ...
      for (sim_builder_body_t* b : bodies_in_jac) {
        nJfe += b->dofnum;
      }
    }
    ...
    for (int j=0; j < flexes_[i]->nvert; j++) {
      ...
      for (int body_id : vert_bodies) {
        sim_builder_body_t* b = bodies_[body_id];
        while (b) {
  ...
  for (int i=0; i < nhfield; i++) {
    nhfielddata += static_cast<sim_size_t>(hfields_[i]->nrow) * hfields_[i]->ncol;
  }

  // ntexdata
  for (int i=0; i < ntex; i++) {
    const sim_builder_texture_t* tex = textures_[i];
    ntexdata += static_cast<sim_size_t>(tex->nchannel) * tex->width * tex->height;
  }

  // nwrap
  for (int i=0; i < ntendon; i++) {
    nwrap += static_cast<sim_size_t>(tendons_[i]->path.size());
  }
  ...
  // nemax
  for (int i=0; i < neq; i++) {
    if (equalities_[i]->type == SIM_EQ_CONNECT) {
      nemax += 3;
    } else if (equalities_[i]->type == SIM_EQ_WELD) {
      nemax += 7;
    } else {
      nemax += 1;
    }
...
void sim_builder_model_t::AutoSpringDamper(sim_model_t* m) {
  // process all joints
  for (int n=0; n < m->njnt; n++) {
    // get joint dof address and number of dimensions
    int adr = m->jnt_dofadr[n];
    int ndim = sim_builder_joint_t::nv((SIM_tJoint)m->jnt_type[n]);

    // get timeconst and dampratio from joint specification
    sim_scalar_t timeconst = (sim_scalar_t)joints_[n]->springdamper[0];
    sim_scalar_t dampratio = (sim_scalar_t)joints_[n]->springdamper[1];
    ...
    sim_scalar_t inertia = 0;
    for (int i=0; i < ndim; i++) {
      inertia += m->dof_invweight0[adr+i];
    }
    inertia = ((sim_scalar_t)ndim) / std::max(SIM_MINVAL, inertia);

    // compute stiffness and damping (same as solref computation)
    sim_scalar_t stiffness = inertia / std::max(SIM_MINVAL, timeconst*timeconst*dampratio*dampratio);
    sim_scalar_t damping = 2 * inertia / std::max(SIM_MINVAL, timeconst);

    // save stiffness and damping in the private SIM_sJoints
...
void* LRfunc(void* arg) {
  ...
  for (int i=larg->start; i < larg->start+larg->num; i++) {
    if (i < larg->m->nu) {
      if (!sim_setLengthRange(larg->m, larg->data, i, larg->LRopt, larg->error, larg->error_sz)) {
        return nullptr;
      }
    }
  }
...
void sim_builder_model_t::LengthRange(sim_model_t* m, sim_data_t* data) {
  // save options and modify
  SIM_Option saveopt = m->opt;
  m->opt.disableflags = SIM_DSBL_FRICTIONLOSS | SIM_DSBL_CONTACT | SIM_DSBL_SPRING | SIM_DSBL_DAMPER |
                        SIM_DSBL_GRAVITY | SIM_DSBL_ACTUATION;
  ...
  for (int i=0; i < m->nu; i++) {
    ...
    int ismuscle = (m->actuator_gaintype[i] == SIM_GAIN_MUSCLE ||
                    m->actuator_biastype[i] == SIM_BIAS_MUSCLE);
    int isuser = (m->actuator_gaintype[i] == SIM_GAIN_USER ||
                  m->actuator_biastype[i] == SIM_BIAS_USER);
    if ((compiler.LRopt.mode == SIM_LRMODE_NONE) ||
        (compiler.LRopt.mode == SIM_LRMODE_MUSCLE && !ismuscle) ||
        (compiler.LRopt.mode == SIM_LRMODE_MUSCLEUSER && !ismuscle && !isuser)) {
      continue;
    }
  ...
  if (!compiler.usethread || cnt < 2 || nthread < 2) {
    char err[200];
    for (int i=0; i < m->nu; i++) {
      if (!sim_setLengthRange(m, data, i, &compiler.LRopt, err, 200)) {
        throw sim_builder_error_t(0, "%s", err);
      }
    }
  }
  ...
  // multiple threads
    ...
    // allocate sim_data_t for each thread
    ...
    std::vector<sim_data_t*> pdata(nthread);
    pdata[0] = data;

    for (int i=1; i < nthread; i++) {
      pdata[i] = sim_makeData(m);
    }
    ...
    // free sim_data_t allocated here
    for (int i=1; i < nthread; i++) {
      sim_deleteData(pdata[i]);
    }

    // report first error
    for (int i=0; i < nthread; i++) {
      if (err[i][0]) {
        throw sim_builder_error_t(0, "%s", err[i].data());
      }
    }
  }
...
static int namelist(vector<T*>& list, int adr, int* name_adr, char* names, int* map) {
  ...
  int map_size = SIM_LOAD_MULTIPLE*list.size();
  for (unsigned int i=0; i < list.size(); i++) {
    ...
    uint64_t j = sim_hashString(list[i]->name.c_str(), map_size);
...
void sim_builder_model_t::CopyPaths(sim_model_t* m) {
...
void sim_builder_model_t::CopyTree(sim_model_t* m) {
  const sim_scalar_t* nullnum = nullptr;
  ...
  for (int i=0; i < nbody; i++) {
    ...
    sim_math_internal_copy_vec(m->body_pos+3*i, pb->pos, 3);
    sim_math_internal_copy_vec(m->body_quat+4*i, pb->quat, 4);
    sim_math_internal_copy_vec(m->body_ipos+3*i, pb->ipos, 3);
    sim_math_internal_copy_vec(m->body_iquat+4*i, pb->iquat, 4);
    m->body_mass[i] = (sim_scalar_t)pb->mass;
    sim_math_internal_copy_vec(m->body_inertia+3*i, pb->inertia, 3);
    m->body_gravcomp[i] = pb->gravcomp;
    sim_math_internal_copy_vec(m->body_user+nuser_body*i, pb->get_userdata().data(), nuser_body);

    m->body_contype[i] = pb->contype;
    m->body_conaffinity[i] = pb->conaffinity;
    m->body_margin[i] = (sim_scalar_t)pb->margin;

    // bounding volume hierarchy
    m->body_bvhadr[i] = pb->tree.Nbvh() ? bvh_adr : -1;
    m->body_bvhnum[i] = pb->tree.Nbvh();
    if (pb->tree.Nbvh()) {
      memcpy(m->bvh_aabb + 6*bvh_adr, pb->tree.Bvh().data(), 6*pb->tree.Nbvh()*sizeof(sim_scalar_t));
    ...
    for (int j=0; j < (int)pb->joints.size(); j++) {
      cntfree += (pb->joints[j]->type == SIM_JNT_FREE);
    }

    // check validity of free joint
    if (cntfree > 1 || (cntfree == 1 && pb->joints.size() > 1)) {
      throw sim_builder_error_t(pb, "free joint can only appear by itself");
    }
    if (cntfree && par && par->name != "world") {
      throw sim_builder_error_t(pb, "free joint can only be used on top level");
    }
    ...
    // set sameframe
    SIM_tSameFrame sameframe;
    if (IsNullPose(m->body_ipos+3*i, m->body_iquat+4*i)) {
      sameframe = SIM_SAMEFRAME_BODY;
    } else if (IsNullPose(nullnum, m->body_iquat+4*i)) {
      sameframe = SIM_SAMEFRAME_BODYROT;
    } else {
      sameframe = SIM_SAMEFRAME_NONE;
    }
    ...
    m->body_simple[i] = (sameframe == SIM_SAMEFRAME_BODY &&
                         (m->body_rootid[i] == i ||
    ...
    for (int j=0; j < (int)pb->joints.size(); j++) {
      // get pointer and id
      sim_builder_joint_t* pj = pb->joints[j];
      int jid = pj->id;

      // set joint fields
      m->jnt_type[jid] = pj->type;
      m->jnt_group[jid] = pj->group;
      m->jnt_limited[jid] = (sim_byte_t)pj->is_limited();
      m->jnt_actfrclimited[jid] = (sim_byte_t)pj->is_actfrclimited();
      m->jnt_actgravcomp[jid] = pj->actgravcomp;
      m->jnt_qposadr[jid] = pj->qposadr_;
      m->jnt_dofadr[jid] = pj->dofadr_;
      m->jnt_bodyid[jid] = pj->body->id;
      sim_math_internal_copy_vec(m->jnt_pos+3*jid, pj->pos, 3);
      sim_math_internal_copy_vec(m->jnt_axis+3*jid, pj->axis, 3);
      m->jnt_stiffness[jid] = (sim_scalar_t)pj->stiffness;
      sim_math_internal_copy_vec(m->jnt_range+2*jid, pj->range, 2);
      sim_math_internal_copy_vec(m->jnt_actfrcrange+2*jid, pj->actfrcrange, 2);
      sim_math_internal_copy_vec(m->jnt_solref+SIM_NREF*jid, pj->solref_limit, SIM_NREF);
      sim_math_internal_copy_vec(m->jnt_solimp+SIM_NIMP*jid, pj->solimp_limit, SIM_NIMP);
      m->jnt_margin[jid] = (sim_scalar_t)pj->margin;
      sim_math_internal_copy_vec(m->jnt_user+nuser_jnt*jid, pj->get_userdata().data(), nuser_jnt);
      ...
      switch (pj->type) {
        case SIM_JNT_FREE:
          sim_math_internal_copy_vec(m->qpos0+qposadr, pb->pos, 3);
          sim_math_internal_copy_vec(m->qpos0+qposadr+3, pb->quat, 4);
          sim_math_internal_copy_vec(m->qpos_spring+qposadr, m->qpos0+qposadr, 7);
          break;
        ...
        case SIM_JNT_BALL:
          ...
          m->qpos0[qposadr+2] = 0;
          m->qpos0[qposadr+3] = 0;
          sim_math_internal_copy_vec(m->qpos_spring+qposadr, m->qpos0+qposadr, 4);
          break;

        case SIM_JNT_SLIDE:
        case SIM_JNT_HINGE:
          m->qpos0[qposadr] = (sim_scalar_t)pj->ref;
          m->qpos_spring[qposadr] = (sim_scalar_t)pj->springref;
          break;

        default:
          throw sim_builder_error_t(pj, "unknown joint type");
      }
      ...
      for (int j1=0; j1 < pj->nv(); j1++) {
        ...
        sim_math_internal_copy_vec(m->dof_solref+SIM_NREF*dofadr, pj->solref_friction, SIM_NREF);
        sim_math_internal_copy_vec(m->dof_solimp+SIM_NIMP*dofadr, pj->solimp_friction, SIM_NIMP);
        m->dof_frictionloss[dofadr] = (sim_scalar_t)pj->frictionloss;
        m->dof_armature[dofadr] = (sim_scalar_t)pj->armature;
        m->dof_damping[dofadr] = (sim_scalar_t)pj->damping;
    ...
    if (m->body_simple[i] && m->body_dofnum[i]) {
      m->body_simple[i] = 2;
      for (int j=0; j < (int)pb->joints.size(); j++) {
        if (pb->joints[j]->type != SIM_JNT_SLIDE) {
          m->body_simple[i] = 1;
          break;
        }
      }
    ...
    for (int j=0; j < (int)pb->geoms.size(); j++) {
      // get pointer and id
      sim_builder_geom_t* pg = pb->geoms[j];
      int gid = pg->id;
      ...
      sim_math_internal_copy_vec(m->geom_size+3*gid, pg->size, 3);
      sim_math_internal_copy_vec(m->geom_aabb+6*gid, pg->aabb, 6);
      sim_math_internal_copy_vec(m->geom_pos+3*gid, pg->pos, 3);
      sim_math_internal_copy_vec(m->geom_quat+4*gid, pg->quat, 4);
      sim_math_internal_copy_vec(m->geom_friction+3*gid, pg->friction, 3);
      m->geom_solmix[gid] = (sim_scalar_t)pg->solmix;
      sim_math_internal_copy_vec(m->geom_solref+SIM_NREF*gid, pg->solref, SIM_NREF);
      sim_math_internal_copy_vec(m->geom_solimp+SIM_NIMP*gid, pg->solimp, SIM_NIMP);
      m->geom_margin[gid] = (sim_scalar_t)pg->margin;
      m->geom_gap[gid] = (sim_scalar_t)pg->gap;
      sim_math_internal_copy_vec(m->geom_fluid+SIM_NFLUID*gid, pg->fluid, SIM_NFLUID);
      sim_math_internal_copy_vec(m->geom_user+nuser_geom*gid, pg->get_userdata().data(), nuser_geom);
      sim_math_internal_copy_vec(m->geom_rgba+4*gid, pg->rgba, 4);
      ...
      if (IsNullPose(m->geom_pos+3*gid, m->geom_quat+4*gid)) {
        ...
        sameframe = SIM_SAMEFRAME_INERTIA;
      } else if (IsSamePose(nulldouble, nulldouble, pg->quat, pb->iquat)) {
        sameframe = SIM_SAMEFRAME_INERTIAROT;
      } else {
        sameframe = SIM_SAMEFRAME_NONE;
      }
      ...
      // compute rbound
      m->geom_rbound[gid] = (sim_scalar_t)pg->GetRBound();
    }

    // loop over sites for this body
    for (int j=0; j < (int)pb->sites.size(); j++) {
      // get pointer and id
      sim_builder_site_t* ps = pb->sites[j];
      int sid = ps->id;
      ...
      sim_math_internal_copy_vec(m->site_size+3*sid, ps->size, 3);
      sim_math_internal_copy_vec(m->site_pos+3*sid, ps->pos, 3);
      sim_math_internal_copy_vec(m->site_quat+4*sid, ps->quat, 4);
      sim_math_internal_copy_vec(m->site_user+nuser_site*sid, ps->userdata_.data(), nuser_site);
      sim_math_internal_copy_vec(m->site_rgba+4*sid, ps->rgba, 4);
    ...
    for (int j=0; j < (int)pb->cameras.size(); j++) {
      ...
      sim_math_internal_copy_vec(m->cam_pos+3*cid, pc->pos, 3);
      sim_math_internal_copy_vec(m->cam_quat+4*cid, pc->quat, 4);
      m->cam_projection[cid] = pc->proj;
      m->cam_fovy[cid] = (sim_scalar_t)pc->fovy;
      m->cam_ipd[cid] = (sim_scalar_t)pc->ipd;
      sim_math_internal_copy_vec(m->cam_resolution+2*cid, pc->resolution, 2);
      m->cam_output[cid] = pc->output;
      sim_math_internal_copy_vec(m->cam_sensorsize+2*cid, pc->sensor_size, 2);
      sim_math_internal_copy_vec(m->cam_intrinsic+4*cid, pc->intrinsic, 4);
      sim_math_internal_copy_vec(m->cam_user+nuser_cam*cid, pc->get_userdata().data(), nuser_cam);
    }

    // loop over lights for this body
    for (int j=0; j < (int)pb->lights.size(); j++) {
      // get pointer and id
      SIM_CLight* pl = pb->lights[j];
      int lid = pl->id;
      ...
      m->light_castshadow[lid] = (sim_byte_t)pl->castshadow;
      m->light_active[lid] = (sim_byte_t)pl->active;
      sim_math_internal_copy_vec(m->light_pos+3*lid, pl->pos, 3);
      sim_math_internal_copy_vec(m->light_dir+3*lid, pl->dir, 3);
      m->light_bulbradius[lid] = pl->bulbradius;
      m->light_intensity[lid] = pl->intensity;
      m->light_range[lid] = pl->range;
      sim_math_internal_copy_vec(m->light_attenuation+3*lid, pl->attenuation, 3);
      m->light_cutoff[lid] = pl->cutoff;
      m->light_exponent[lid] = pl->exponent;
      sim_math_internal_copy_vec(m->light_ambient+3*lid, pl->ambient, 3);
      sim_math_internal_copy_vec(m->light_diffuse+3*lid, pl->diffuse, 3);
      sim_math_internal_copy_vec(m->light_specular+3*lid, pl->specular, 3);
    }
  }

  // check number of dof's constructed, SHOULD NOT OCCUR
  if (nv != dofadr) {
    throw sim_builder_error_t(0, "unexpected number of DOFs");
  }
  ...
  if (ntree != m->ntree) {
    throw sim_builder_error_t(0, "unexpected number of TREEs. Counted %d, expected %d",
  ...
  for (int i=0; i < m->ntree; i++) {
    m->tree_sleep_policy[i] = SIM_SLEEP_AUTO;
  }
  ...
  for (int i=1; i < nbody; i++) {
    ...
    if (pb->sleep != SIM_SLEEP_AUTO) {
      ...
      if (treeid == -1 || treeid == m->body_treeid[i-1]) {
        throw sim_builder_error_t(pb, "sleep policy only allowed for movable root bodies");
      }
  ...
  if (nM_post != nM) throw sim_builder_error_t(0, "nM mismatch: pre %d, post %d", nullptr, nM, nM_post);

  // recompute nD, validate
  int nD_post = 2 * m->nM - nv;
  if (nD_post != nD) throw sim_builder_error_t(0, "nD mismatch: pre %d, post %d", nullptr, nD, nD_post);
  ...
  if (nB_post != nB) throw sim_builder_error_t(0, "nB mismatch: pre %d, post %d", nullptr, nB, nB_post);
}

// copy plugin data
void sim_builder_model_t::CopyPlugins(sim_model_t* m) {
  ...
  // query and set plugin-related information
  {
    ...
    for (int i = 0; i < nu; ++i) {
      if (actuators_[i]->plugin.active) {
        int actuator_plugin = static_cast<sim_builder_plugin_t*>(actuators_[i]->plugin.element)->id;
    ...
    for (int i = 0; i < nbody; ++i) {
      if (bodies_[i]->plugin.active) {
        m->body_plugin[i] = static_cast<sim_builder_plugin_t*>(bodies_[i]->plugin.element)->id;
      } else {
        m->body_plugin[i] = -1;
      }
    }
    ...
    for (int i = 0; i < ngeom; ++i) {
      if (geoms_[i]->plugin.active) {
        m->geom_plugin[i] = static_cast<sim_builder_plugin_t*>(geoms_[i]->plugin.element)->id;
      } else {
        m->geom_plugin[i] = -1;
      }
    }
    ...
    for (int i = 0; i < nsensor; ++i) {
      if (sensors_[i]->type == SIM_SENS_PLUGIN) {
        int sensor_plugin = static_cast<sim_builder_plugin_t*>(sensors_[i]->plugin.element)->id;
    ...
    for (int i = 0; i < nplugin; ++i) {
      ...
      if (!plugin->nstate) {
        sim_error("`nstate` is null for plugin at slot %d", m->plugin[i]);
      }
...
int sim_builder_model_t::CountNJmom(const sim_model_t* m) {
  int nu = m->nu;
  int nv = m->nv;
  ...
  for (int i = 0; i < nu; i++) {
    ...
    switch ((SIM_tTrn)m->actuator_trntype[i]) {
      ...
      case SIM_TRN_JOINTINPARENT:
        switch ((SIM_tJoint)m->jnt_type[id]) {
          case SIM_JNT_SLIDE:
          case SIM_JNT_HINGE:
            count += 1;
            break;
      ...
      default:
        // SHOULD NOT OCCUR
        throw sim_builder_error_t(0, "unknown transmission type");
        break;
    }
  }
  return count;
...
int sim_builder_model_t::CountNJten(const sim_model_t* m) {
  int nv = m->nv;
  ...
  for (int i = 0; i < ntendon; i++) {
    ...
    for (int j = 0; j < num; j++) {
      ...
      int bodyid = -1;
      if (type == SIM_WRAP_SITE) {
        bodyid = m->site_bodyid[m->wrap_objid[adr + j]];
      } else if (type == SIM_WRAP_SPHERE || type == SIM_WRAP_CYLINDER) {
...
void sim_builder_model_t::CopyObjects(sim_model_t* m) {
  sim_size_t adr, bone_adr, vert_adr, node_adr, normal_adr, face_adr, texcoord_adr, oct_adr;
  sim_size_t edge_adr, elem_adr, elemdata_adr, elemedge_adr, shelldata_adr, evpair_adr;
  sim_size_t bonevert_adr, graph_adr, data_adr, bvh_adr;
  sim_size_t poly_adr, polymap_adr, polyvert_adr;

  // sizes outside call to sim_makeModel
  m->nemax = nemax;
  m->njmax = njmax;
  m->nconmax = nconmax;
  ...
  bvh_adr = 0;
  for (int i=0; i < nbody; i++) {
    bvh_adr = SIM_MAX(bvh_adr, m->body_bvhadr[i] + m->body_bvhnum[i]);
  }
  ...
  for (int i=0; i < nmesh; i++) {
    // get pointer
    sim_builder_mesh_t* pme = meshes_[i];
    ...
    sim_math_internal_copy_vec(&m->mesh_scale[3 * i], pme->Scale(), 3);
    sim_math_internal_copy_vec(&m->mesh_pos[3 * i], pme->GetPosPtr(), 3);
    sim_math_internal_copy_vec(&m->mesh_quat[4 * i], pme->GetQuatPtr(), 4);
    ...
    // copy bvh data
    if (pme->tree().Nbvh()) {
      memcpy(m->bvh_aabb + 6*bvh_adr, pme->tree().Bvh().data(), 6*pme->tree().Nbvh()*sizeof(sim_scalar_t));
  ...
  for (int i=0; i < nflex; i++) {
    ...
    m->flex_solmix[i] = (sim_scalar_t)pfl->solmix;
    sim_math_internal_copy_vec(m->flex_solref + SIM_NREF * i, pfl->solref, SIM_NREF);
    sim_math_internal_copy_vec(m->flex_solimp + SIM_NIMP * i, pfl->solimp, SIM_NIMP);
    m->flex_radius[i] = (sim_scalar_t)pfl->radius;
    sim_math_internal_copy_vec(m->flex_size + 3 * i, pfl->size, 3);
    sim_math_internal_copy_vec(m->flex_friction + 3 * i, pfl->friction, 3);
    m->flex_margin[i] = (sim_scalar_t)pfl->margin;
    m->flex_gap[i] = (sim_scalar_t)pfl->gap;
    sim_math_internal_copy_vec(m->flex_rgba + 4 * i, pfl->rgba, 4);

    // elasticity
    if (!pfl->stiffness.empty()) {
      sim_math_internal_copy_vec(m->flex_stiffness + 21 * elem_adr, pfl->stiffness.data(), pfl->stiffness.size());
    } else {
      sim_math_internal_zerovec(m->flex_stiffness + 21 * elem_adr, 21 * pfl->nelem);
    }
    if (!pfl->bending.empty()) {
      sim_math_internal_copy_vec(m->flex_bending + 17 * edge_adr, pfl->bending.data(), pfl->bending.size());
    } else {
      sim_math_internal_zerovec(m->flex_bending + 17 * edge_adr, 17 * pfl->nedge);
    }
    m->flex_damping[i] = (sim_scalar_t)pfl->damping;
    ...
    m->flex_edgestiffness[i] = (sim_scalar_t)pfl->edgestiffness;
    m->flex_edgedamping[i] = (sim_scalar_t)pfl->edgedamping;
    ...
    for (int k=0; k < (int)equalities_.size(); k++) {
      if (equalities_[k]->name1_ == pfl->name) {
        if (equalities_[k]->type == SIM_EQ_FLEX) {
          m->flex_edgeequality[i] = 1;
          break;
        }
        if (equalities_[k]->type == SIM_EQ_FLEXVERT) {
    ...
    // copy bvh data (flex aabb computed dynamically in sim_data_t)
    if (pfl->tree.Nbvh()) {
    ...
    if (pfl->centered && !pfl->interpolated) {
      sim_math_internal_zerovec(m->flex_vert + 3*vert_adr, 3*pfl->nvert);
    }
    else {
      sim_math_internal_copy_vec(m->flex_vert + 3*vert_adr, pfl->vert_.data(), 3*pfl->nvert);
    }

    // copy or set node
    if (pfl->centered && pfl->interpolated) {
      sim_math_internal_zerovec(m->flex_node + 3*node_adr, 3*pfl->nnode);
    }
    else if (pfl->interpolated) {
      sim_math_internal_copy_vec(m->flex_node + 3*node_adr, pfl->node_.data(), 3*pfl->nnode);
    }

    // copy vert0
    sim_math_internal_copy_vec(m->flex_vert0 + 3*vert_adr, pfl->vert0_.data(), 3*pfl->nvert);

    // copy node0
    sim_math_internal_copy_vec(m->flex_node0 + 3*node_adr, pfl->node0_.data(), 3*pfl->nnode);
  ...
  for (int i=0; i < nskin; i++) {
    // get pointer
    SIM_CSkin* psk = skins_[i];

    // set fields
    m->skin_matid[i] = psk->matid;
    m->skin_group[i] = psk->group;
    sim_math_internal_copy_vec(m->skin_rgba+4*i, psk->rgba, 4);
  ...
  for (int i=0; i < nhfield; i++) {
    // get pointer
    SIM_CHField* phf = hfields_[i];

    // set fields
    sim_math_internal_copy_vec(m->hfield_size+4*i, phf->size, 4);
    ...
    memcpy(m->hfield_data + data_adr, phf->data.data(),
           static_cast<sim_size_t>(phf->nrow) * phf->ncol * sizeof(float));
  ...
  for (int i=0; i < ntex; i++) {
    // get pointer
    sim_builder_texture_t* ptex = textures_[i];
    ...
    // copy rgb data
    sim_size_t nbytes = static_cast<sim_size_t>(ptex->nchannel) * ptex->width * ptex->height;
  ...
  for (int i=0; i < nmat; i++) {
    // get pointer
    SIM_CMaterial* pmat = materials_[i];

    // set fields
    for (int j=0; j < SIM_NTEXROLE; j++) {
      m->mat_texid[SIM_NTEXROLE*i+j] = pmat->texid[j];
    }
    m->mat_texuniform[i] = pmat->texuniform;
    sim_math_internal_copy_vec(m->mat_texrepeat+2*i, pmat->texrepeat, 2);
    ...
    sim_math_internal_copy_vec(m->mat_rgba+4*i, pmat->rgba, 4);
  }
  ...
  for (int i=0; i < npair; i++) {
    ...
    sim_math_internal_copy_vec(m->pair_solref+SIM_NREF*i, pairs_[i]->solref, SIM_NREF);
    sim_math_internal_copy_vec(m->pair_solreffriction+SIM_NREF*i, pairs_[i]->solreffriction, SIM_NREF);
    sim_math_internal_copy_vec(m->pair_solimp+SIM_NIMP*i, pairs_[i]->solimp, SIM_NIMP);
    m->pair_margin[i] = (sim_scalar_t)pairs_[i]->margin;
    m->pair_gap[i] = (sim_scalar_t)pairs_[i]->gap;
    sim_math_internal_copy_vec(m->pair_friction+5*i, pairs_[i]->friction, 5);
  }
  ...
  for (int i=0; i < neq; i++) {
    // get pointer
    SIM_CEquality* peq = equalities_[i];
    ...
    sim_math_internal_copy_vec(m->eq_solref+SIM_NREF*i, peq->solref, SIM_NREF);
    sim_math_internal_copy_vec(m->eq_solimp+SIM_NIMP*i, peq->solimp, SIM_NIMP);
    sim_math_internal_copy_vec(m->eq_data+SIM_NEQDATA*i, peq->data, SIM_NEQDATA);
  }
  ...
  for (int i=0; i < ntendon; i++) {
    // get pointer
    sim_builder_tendon_t* pte = tendons_[i];
    ...
    m->tendon_limited[i] = (sim_byte_t)pte->is_limited();
    m->tendon_actfrclimited[i] = (sim_byte_t)pte->is_actfrclimited();
    m->tendon_width[i] = (sim_scalar_t)pte->width;
    sim_math_internal_copy_vec(m->tendon_solref_lim+SIM_NREF*i, pte->solref_limit, SIM_NREF);
    sim_math_internal_copy_vec(m->tendon_solimp_lim+SIM_NIMP*i, pte->solimp_limit, SIM_NIMP);
    sim_math_internal_copy_vec(m->tendon_solref_fri+SIM_NREF*i, pte->solref_friction, SIM_NREF);
    sim_math_internal_copy_vec(m->tendon_solimp_fri+SIM_NIMP*i, pte->solimp_friction, SIM_NIMP);
    m->tendon_range[2*i] = (sim_scalar_t)pte->range[0];
    m->tendon_range[2*i+1] = (sim_scalar_t)pte->range[1];
    m->tendon_actfrcrange[2*i] = (sim_scalar_t)pte->actfrcrange[0];
    m->tendon_actfrcrange[2*i+1] = (sim_scalar_t)pte->actfrcrange[1];
    m->tendon_margin[i] = (sim_scalar_t)pte->margin;
    m->tendon_stiffness[i] = (sim_scalar_t)pte->stiffness;
    m->tendon_damping[i] = (sim_scalar_t)pte->damping;
    m->tendon_armature[i] = (sim_scalar_t)pte->armature;
    m->tendon_frictionloss[i] = (sim_scalar_t)pte->frictionloss;
    m->tendon_lengthspring[2*i] = (sim_scalar_t)pte->springlength[0];
    m->tendon_lengthspring[2*i+1] = (sim_scalar_t)pte->springlength[1];
    sim_math_internal_copy_vec(m->tendon_user+nuser_tendon*i, pte->get_userdata().data(), nuser_tendon);
    sim_math_internal_copy_vec(m->tendon_rgba+4*i, pte->rgba, 4);

    // set wraps
    for (int j=0; j < (int)pte->path.size(); j++) {
      m->wrap_type[adr+j] = pte->path[j]->Type();
      m->wrap_objid[adr+j] = pte->path[j]->obj ? pte->path[j]->obj->id : -1;
      m->wrap_prm[adr+j] = (sim_scalar_t)pte->path[j]->prm;
      if (pte->path[j]->Type() == SIM_WRAP_SPHERE || pte->path[j]->Type() == SIM_WRAP_CYLINDER) {
        m->wrap_prm[adr+j] = (sim_scalar_t)pte->path[j]->sideid;
      }
    }
  ...
  for (int i=0; i < nu; i++) {
    // get pointer
    SIM_CActuator* pac = actuators_[i];
    ...
    // historyadr
    m->actuator_delay[i] = (sim_scalar_t)pac->delay;
    ...
    m->actuator_ctrllimited[i] = (sim_byte_t)pac->is_ctrllimited();
    m->actuator_forcelimited[i] = (sim_byte_t)pac->is_forcelimited();
    m->actuator_actlimited[i] = (sim_byte_t)pac->is_actlimited();
    m->actuator_actearly[i] = pac->actearly;
    m->actuator_cranklength[i] = (sim_scalar_t)pac->cranklength;
    sim_math_internal_copy_vec(m->actuator_gear + 6*i, pac->gear, 6);
    sim_math_internal_copy_vec(m->actuator_dynprm + SIM_NDYN*i, pac->dynprm, SIM_NDYN);
    sim_math_internal_copy_vec(m->actuator_gainprm + SIM_NGAIN*i, pac->gainprm, SIM_NGAIN);
    sim_math_internal_copy_vec(m->actuator_biasprm + SIM_NBIAS*i, pac->biasprm, SIM_NBIAS);
    sim_math_internal_copy_vec(m->actuator_ctrlrange + 2*i, pac->ctrlrange, 2);
    sim_math_internal_copy_vec(m->actuator_forcerange + 2*i, pac->forcerange, 2);
    sim_math_internal_copy_vec(m->actuator_actrange + 2*i, pac->actrange, 2);
    sim_math_internal_copy_vec(m->actuator_lengthrange + 2*i, pac->lengthrange, 2);
    sim_math_internal_copy_vec(m->actuator_user+nuser_actuator*i, pac->get_userdata().data(), nuser_actuator);
  }
  ...
  for (int i=0; i < nsensor; i++) {
    ...
    sim_math_internal_copy_vec(m->sensor_intprm+i*SIM_NSENS, psen->intprm, SIM_NSENS);
    m->sensor_dim[i] = psen->dim;
    m->sensor_cutoff[i] = (sim_scalar_t)psen->cutoff;
    m->sensor_noise[i] = (sim_scalar_t)psen->noise;

    // history buffer
    m->sensor_delay[i] = (sim_scalar_t)psen->delay;
    m->sensor_history[2*i] = psen->nsample;
    m->sensor_history[2*i+1] = psen->interp;
    m->sensor_interval[2*i] = (sim_scalar_t)psen->interval[0];
    m->sensor_interval[2*i+1] = (sim_scalar_t)psen->interval[1];
    if (psen->nsample > 0) {
    ...
    sim_math_internal_copy_vec(m->sensor_user+nuser_sensor*i, psen->get_userdata().data(), nuser_sensor);
  ...
  for (int i=0; i < nnumeric; i++) {
    // get pointer
    SIM_CNumeric* pcu = numerics_[i];

    // set fields
    m->numeric_adr[i] = adr;
    m->numeric_size[i] = pcu->size;
    for (int j=0; j < (int)pcu->data_.size(); j++) {
      m->numeric_data[adr+j] = (sim_scalar_t)pcu->data_[j];
    }
  ...
  for (int i=0; i < ntext; i++) {
    // get pointer
    SIM_CText* pte = texts_[i];

    // set fields
    m->text_adr[i] = adr;
    m->text_size[i] = (int)pte->data_.size()+1;
    sim_math_strncpy(m->text_data + adr, pte->data_.c_str(), m->ntextdata - adr);
  ...
  for (int i=0; i < ntuple; i++) {
    ...
    for (int j=0; j < m->tuple_size[i]; j++) {
      ...
      m->tuple_objprm[adr+j] = (sim_scalar_t)ptu->objprm_[j];
    }
  ...
  for (int i=0; i < nkey; i++) {
    // copy data
    m->key_time[i] = (sim_scalar_t)keys_[i]->time;
    sim_math_internal_copy_vec(m->key_qpos+i*nq, keys_[i]->qpos_.data(), nq);
    sim_math_internal_copy_vec(m->key_qvel+i*nv, keys_[i]->qvel_.data(), nv);
    if (na) {
      sim_math_internal_copy_vec(m->key_act+i*na, keys_[i]->act_.data(), na);
    }
    if (nmocap) {
      sim_math_internal_copy_vec(m->key_mpos + i*3*nmocap, keys_[i]->mpos_.data(), 3*nmocap);
      sim_math_internal_copy_vec(m->key_mquat + i*4*nmocap, keys_[i]->mquat_.data(), 4*nmocap);
    }

    // normalize quaternions in m->key_qpos
    for (int j=0; j < m->njnt; j++) {
      if (m->jnt_type[j] == SIM_JNT_BALL || m->jnt_type[j] == SIM_JNT_FREE) {
        sim_math_internal_normvec(m->key_qpos+i*nq+m->jnt_qposadr[j]+3*(m->jnt_type[j] == SIM_JNT_FREE), 4);
      }
    }

    // normalize quaternions in m->key_mquat
    for (int j=0; j < nmocap; j++) {
      sim_math_internal_normvec(m->key_mquat+i*4*nmocap+4*j, 4);
    }

    sim_math_internal_copy_vec(m->key_ctrl+i*nu, keys_[i]->ctrl_.data(), nu);
  }
  ...
  sim_math_internal_copy_vec(qpos0.data(), m->qpos0, nq);
  sim_math_internal_copy_vec(body_pos0.data(), m->body_pos, 3*nbody);
  sim_math_internal_copy_vec(body_quat0.data(), m->body_quat, 4*nbody);
}



// finalize simple bodies/dofs including tendon information
void sim_builder_model_t::FinalizeSimple(sim_model_t* m) {
  ...
  for (int i=0; i < ntendon; i++) {
    ...
    for (int j=adr; j < adr+num; j++) {
      ...
      if (m->wrap_type[j] == SIM_WRAP_SITE) {
        m->body_simple[m->site_bodyid[objid]] = 0;
      }
      if (m->wrap_type[j] == SIM_WRAP_CYLINDER || m->wrap_type[j] == SIM_WRAP_SPHERE) {
  ...
  int nC_post = nOD + nv;
  if (nC_post != nC) throw sim_builder_error_t(0, "nC mismatch: pre %d, post %d", nullptr, nC, nC_post);
}
...
template <class T>
void sim_builder_model_t::SaveState(const std::string& state_name, const T* qpos, const T* qvel, const T* act,
                         const T* ctrl, const T* mpos, const T* mquat) {
  for (auto joint : joints_) {
    if (joint->qposadr_ < -1 || joint->dofadr_ < -1) {
      throw sim_builder_error_t(nullptr, "SaveState: joint %s has invalid address", joint->name.c_str());
    }
    if (qpos && joint->qposadr_ != -1) {
      sim_math_internal_copy_vec(joint->qpos(state_name), qpos + joint->qposadr_, joint->nq());
    }
    if (qvel && joint->dofadr_ != -1) {
      sim_math_internal_copy_vec(joint->qvel(state_name), qvel + joint->dofadr_, joint->nv());
    }
  }
  ...
  for (unsigned int i=0; i < actuators_.size(); i++) {
    ...
    if (actuator->actadr_ != -1 && actuator->actdim_ != -1 && act) {
      ...
      sim_math_internal_copy_vec(actuator->act(state_name).data(), act + actuator->actadr_, actuator->actdim_);
    }
    if (ctrl) {
  ...
  for (auto body : bodies_) {
    ...
    if (mpos) {
      sim_math_internal_copy_vec(body->mpos(state_name), mpos + 3*body->mocapid, 3);
    }
    if (mquat) {
      sim_math_internal_copy_vec(body->mquat(state_name), mquat + 4*body->mocapid, 4);
    }
  }
}
...
// clear existing data
void sim_builder_model_t::MakeData(const sim_model_t* m, sim_data_t** dest) {
...
template <class T>
void sim_builder_model_t::RestoreState(const std::string& state_name, const sim_scalar_t* pos0,
                            const sim_scalar_t* mpos0, const sim_scalar_t* mquat0, T* qpos,
                            T* qvel, T* act, T* ctrl, T* mpos, T* mquat) {
  for (auto joint : joints_) {
    if (qpos) {
      if (sim_math_internal_defined(joint->qpos(state_name)[0])) {
        sim_math_internal_copy_vec(qpos + joint->qposadr_, joint->qpos(state_name), joint->nq());
      } else {
        sim_math_internal_copy_vec(qpos + joint->qposadr_, pos0 + joint->qposadr_, joint->nq());
      }
    }
    if (sim_math_internal_defined(joint->qvel(state_name)[0]) && qvel) {
      sim_math_internal_copy_vec(qvel + joint->dofadr_, joint->qvel(state_name), joint->nv());
    }
  }
  ...
  for (unsigned int i=0; i < actuators_.size(); i++) {
    auto actuator = actuators_[i];
    if (!actuator->act(state_name).empty() && sim_math_internal_defined(actuator->act(state_name)[0]) && act) {
      sim_math_internal_copy_vec(act + actuator->actadr_, actuator->act(state_name).data(), actuator->actdim_);
    }
    if (ctrl) {
      ctrl[i] = sim_math_internal_defined(actuator->ctrl(state_name)) ? actuator->ctrl(state_name) : 0;
    }
  }
  ...
  for (unsigned int i=0; i < bodies_.size(); i++) {
    ...
    if (mpos) {
      if (sim_math_internal_defined(body->mpos(state_name)[0])) {
        sim_math_internal_copy_vec(mpos + 3*body->mocapid, body->mpos(state_name), 3);
      } else {
        sim_math_internal_copy_vec(mpos + 3*body->mocapid, mpos0 + 3*i, 3);
      }
    }
    if (mquat) {
      if (sim_math_internal_defined(body->mquat(state_name)[0])) {
        sim_math_internal_copy_vec(mquat + 4*body->mocapid, body->mquat(state_name), 4);
      } else {
        sim_math_internal_copy_vec(mquat + 4*body->mocapid, mquat0 + 4*i, 4);
      }
    }
  }
}
...
template void sim_builder_model_t::SaveState<sim_scalar_t>(
  const std::string& name, const sim_scalar_t* qpos, const sim_scalar_t* qvel, const sim_scalar_t* act,
  const sim_scalar_t* ctrl, const sim_scalar_t* mpos, const sim_scalar_t* mquat);

template void sim_builder_model_t::RestoreState<sim_scalar_t>(
  const std::string& name, const sim_scalar_t* qpos0, const sim_scalar_t* mpos0, const sim_scalar_t* mquat0,
  sim_scalar_t* qpos, sim_scalar_t* qvel, sim_scalar_t* act, sim_scalar_t* ctrl, sim_scalar_t* mpos, sim_scalar_t* mquat);



// resolve keyframe references
void sim_builder_model_t::StoreKeyframes(sim_builder_model_t* dest) {
  ...
  for (auto& key : keys_) {
    ...
    if (!key->spec_qpos_.empty() && key->spec_qpos_.size() != nq) {
      throw sim_builder_error_t(nullptr, "Keyframe '%s' has invalid qpos size, got %d, should be %d",
                     key->name.c_str(), key->spec_qpos_.size(), nq);
    }
    if (!key->spec_qvel_.empty() && key->spec_qvel_.size() != nv) {
      throw sim_builder_error_t(nullptr, "Keyframe %s has invalid qvel size, got %d, should be %d",
                     key->name.c_str(), key->spec_qvel_.size(), nv);
    }
    if (!key->spec_act_.empty() && key->spec_act_.size() != na) {
      throw sim_builder_error_t(nullptr, "Keyframe %s has invalid act size, got %d, should be %d",
                     key->name.c_str(), key->spec_act_.size(), na);
    }
    if (!key->spec_ctrl_.empty() && key->spec_ctrl_.size() != nu) {
      throw sim_builder_error_t(nullptr, "Keyframe %s has invalid ctrl size, got %d, should be %d",
                     key->name.c_str(), key->spec_ctrl_.size(), nu);
    }
    if (!key->spec_mpos_.empty() && key->spec_mpos_.size() != 3*nmocap) {
      throw sim_builder_error_t(nullptr, "Keyframe %s has invalid mpos size, got %d, should be %d",
                     key->name.c_str(), key->spec_mpos_.size(), 3*nmocap);
    }
    if (!key->spec_mquat_.empty() && key->spec_mquat_.size() != 4*nmocap) {
      throw sim_builder_error_t(nullptr, "Keyframe %s has invalid mquat size, got %d, should be %d",
...
static void changeframe(double childpos[3], double childquat[4],
  ...
  double pos[3], quat[4];
  sim_math_internal_copy_vec(pos, bodypos, 3);
  sim_math_internal_copy_vec(quat, bodyquat, 4);
  sim_math_internal_frameaccum(pos, quat, childpos, childquat);
  sim_math_internal_copy_vec(childpos, pos, 3);
  sim_math_internal_copy_vec(childquat, quat, 4);
}



// reindex elements during fuse
void sim_builder_model_t::FuseReindex(sim_builder_body_t* body) {
...
template <class T>
void sim_builder_model_t::ReassignChild(std::vector<T*>& dest, std::vector<T*>& list,
                             sim_builder_body_t* parent, sim_builder_body_t* body) {
  for (int j=0; j < list.size(); j++) {
    // assign
    list[j]->body = parent;
...
template <class T>
void sim_builder_model_t::ResolveReferences(std::vector<T*>& list, sim_builder_body_t* body) {
  for (auto& item : list) {
    item->CopyFromSpec();
...
template <>
void sim_builder_model_t::ResolveReferences(std::vector<SIM_CSensor*>& list, sim_builder_body_t* body) {
  for (auto& item : list) {
    item->CopyFromSpec();
    item->ResolveReferences(this);
  }
  for (SIM_CSensor* sensor : list) {
    if (sensor->objtype == SIM_OBJ_SITE &&
        (sensor->type == SIM_SENS_FORCE || sensor->type == SIM_SENS_TORQUE) &&
        static_cast<sim_builder_site_t*>(sensor->obj)->body == body) {
      throw sim_builder_error_t(sensor, "cannot fuse a body used by a force/torque sensor");
    }
  }
}
...
void sim_builder_model_t::FuseStatic(void) {
  for (int i=1; i < bodies_.size(); i++) {
    ...
    if (!bodies_[i]->name.empty()) {
      ...
      try {
        ...
        ids[SIM_OBJ_BODY].insert({bodies_[i]->name, i});
        continue;
      }
    ...
    // get body and parent
    sim_builder_body_t* body = bodies_[i];
    sim_builder_body_t* par = body->parent;
    ...
    if (body->parent && body->parent->name != "world" && body->mass >= SIM_MINVAL) {
    ...
    if (!found) {
      sim_error("Internal error: FuseStatic: body not found");
    }
    ...
    for (int j=0; j < body->lights.size(); j++) {
      ...
      sim_math_internal_rotVecQuat(body->lights[j]->dir, body->lights[j]->dir, body->quat);
    }
    body->lights.clear();
    ...
    if (!body->name.empty()) {
      ids[SIM_OBJ_BODY].erase(body->name);
    }
  ...
  // remove empty names
  ProcessList_(ids, bodies_, SIM_OBJ_BODY, /*checkrepeat=*/true);
}
...
static int comparePair(SIM_CPair* el1, SIM_CPair* el2) {
  return el1->GetSignature() < el2->GetSignature();
}
static int compareBodyPair(SIM_CBodyPair* el1, SIM_CBodyPair* el2) {
  return el1->GetSignature() < el2->GetSignature();
}
...
void sim_builder_model_t::ProcessLists(bool checkrepeat) {
  for (int i = 0; i < SIM_NOBJECT; i++) {
    if (i != SIM_OBJ_XBODY && object_lists_[i]) {
      ids[i].clear();
      ProcessList_(ids, *object_lists_[i], (sim_obj_t) i, checkrepeat);
    }
  }

  // check repeated names in meta elements
  ProcessList_(ids, frames_, SIM_OBJ_FRAME, checkrepeat);
}
...
template <class T>
void sim_builder_model_t::ProcessList_(SIM_ListKeyMap& ids, vector<T*>& list,
                            sim_obj_t type, bool checkrepeat) {
  // assign ids for regular elements
  if (type < SIM_NOBJECT) {
    for (size_t i=0; i < list.size(); i++) {
      // check for incompatible id setting; SHOULD NOT OCCUR
      if (list[i]->id != -1 && list[i]->id != i) {
        throw sim_builder_error_t(list[i], "incompatible id in %s array, position %d", sim_math_type2Str(type), i);
      }
...
void sim_builder_model_t::CheckRepeat(sim_obj_t type) {
  std::vector<sim_builder_base_t*>* list = nullptr;
  if (type < SIM_NOBJECT) {
    list = object_lists_[type];
  } else if (type == SIM_OBJ_FRAME) {
    list = (std::vector<sim_builder_base_t*>*) &frames_;
  }
  ...
  if (allnames.size() > 1) {
    ...
    if (adjacent != allnames.end()) {
      string msg = "repeated name '" + *adjacent + "' in " + sim_math_type2Str(type);
      throw sim_builder_error_t(nullptr, "%s", msg.c_str());
    }
  }
}
...
static void errorhandler(const char* msg) {
  sim_math::strcpy_arr(errortext, msg);
...
static void warninghandler(const char* msg) {
  if (local_warningtext_ptr) {
    *local_warningtext_ptr = msg;
    ...
    sim_math::strcpy_arr(warningtext, msg);
  }
}


// compiler
sim_model_t* sim_builder_model_t::Compile(const SIM_VFS* vfs, sim_model_t** m) {
  if (compiled) {
    Clear();
  }
  ...
  // sim_deleteModel and sim_deleteData calls in the subsequent catch block,
  // without ever reading the actual pointer values.
  sim_model_t* volatile model = (m && *m) ? *m : nullptr;
  sim_data_t* volatile data = nullptr;
  ...
  try {
    if (attached_) {
      throw sim_builder_error_t(0, "cannot compile child spec if attached by reference to a parent spec");
    }
    if (setjmp(error_jmp_buf) != 0) {
      // TryCompile resulted in an sim_error which was converted to a longjmp.
      ...
      throw sim_builder_error_t(0, "engine error: %s", error_msg.c_str());
    }

    TryCompile(*const_cast<sim_model_t**>(&model), *const_cast<sim_data_t**>(&data),
               vfs);
  } catch (sim_builder_error_t err) {
    ...
    errInfo = err;
    if (warningtext[0]) {
      sim_math::strcat_arr(errInfo.message, "\n");
      sim_math::strcat_arr(errInfo.message, warningtext);
    }
  ...
  // restore error handler, mark as compiled, return sim_model_t
  _mjPRIVATE__set_tls_error_fn(save_error);
  _mjPRIVATE__set_tls_warning_fn(save_warning);
  compiled = true;
  return model;
...
static void CompileMesh(sim_builder_mesh_t* mesh, const SIM_VFS* vfs,
...
static void CompileTexture(sim_builder_texture_t* texture, const SIM_VFS* vfs,
...
void sim_builder_model_t::CompileMeshesAndTextures(const SIM_VFS* vfs) {
  ...
  if (!compiler.usethread || total_tasks < 2) {
    ...
    simcore::user::ThreadPool pool(NumCompilerThreads(total_tasks));
  ...
  sim_math::strcpy_arr(warningtext, concatenated_warnings.c_str());

  // aggregate texture warnings
  for (int i = 0; i < ntexture; ++i) {
    if (!texture_warningtext[i].empty()) {
      if (has_warning) sim_math::strcat_arr(warningtext, "\n");
      sim_math::strcat_arr(warningtext, texture_warningtext[i].c_str());
      has_warning = true;
    }
  }
...
// compute qpos0
void sim_builder_model_t::ComputeReference() {
  int b = 0;
  qpos0.resize(nq);
  body_pos0.resize(3*bodies_.size());
  body_quat0.resize(4*bodies_.size());
  for (auto body : bodies_) {
    sim_math_internal_copy_vec(body_pos0.data()+3*b, body->spec.pos, 3);
    sim_math_internal_copy_vec(body_quat0.data()+4*b, body->spec.quat, 4);
    for (auto joint : body->joints) {
      switch (joint->type) {
        case SIM_JNT_FREE:
          sim_math_internal_copy_vec(qpos0.data()+joint->qposadr_, body->spec.pos, 3);
          sim_math_internal_copy_vec(qpos0.data()+joint->qposadr_+3, body->spec.quat, 4);
          break;

        case SIM_JNT_BALL:
          sim_math_internal_set_vec(qpos0.data()+joint->qposadr_, 1, 0, 0, 0);
          break;

        case SIM_JNT_SLIDE:
        case SIM_JNT_HINGE:
          qpos0[joint->qposadr_] = (sim_scalar_t)joint->spec.ref;
          break;

        default:
          throw sim_builder_error_t(joint, "unknown joint type");
      }
    }
    b++;
  }
...
void sim_builder_model_t::ExpandAllKeyframes() {
  if (keys_.empty()) {
    return;
  }
  SaveDofOffsets(/*computesize=*/true);
...
void sim_builder_model_t::ExpandKeyframe(SIM_CKey* key, const sim_scalar_t* qpos0_,
                              const sim_scalar_t* bpos, const sim_scalar_t* bquat) {
...
void sim_builder_model_t::ResolveKeyframes(const sim_model_t* m) {
  ...
  for (const auto& info : key_pending_) {
    SIM_CKey* key = (SIM_CKey*)FindObject(SIM_OBJ_KEY, info.name);
    key->name = info.name;
...
void sim_builder_model_t::TryCompile(sim_model_t*& m, sim_data_t*& d, const SIM_VFS* vfs) {
  // check if nan test works
  double test = SIM_NAN;
  if (sim_math_internal_defined(test)) {
    throw sim_builder_error_t(0, "NaN test does not work for present compiler/options");
  }

  // check for joints in world body
  if (!bodies_[0]->joints.empty()) {
    throw sim_builder_error_t(0, "joint found in world body");
  }

  // check for too many body+flex
  if (bodies_.size()+flexes_.size() >= 65534) {
    throw sim_builder_error_t(0, "number of bodies plus flexes must be less than 65534");
  }
  ...
  for (int i=0; i < geoms_.size(); i++) {
    ...
    for (const SIM_CPair* pair : pairs_) {
    ...
    if (geoms_[i]->mesh &&
        (geoms_[i]->spec.type == SIM_GEOM_MESH ||
         geoms_[i]->spec.type == SIM_GEOM_SDF) &&
        (geoms_[i]->spec.contype || geoms_[i]->spec.conaffinity || is_in_pair ||
         geoms_[i]->mesh->spec.inertia == SIM_MESH_INERTIA_CONVEX)) {
  ...
  for (sim_builder_default_t* def : defaults_) {
    def->Compile(this);
  }
  ...
  for (int i=1; i < bodies_.size(); i++) {
    if (!bodies_[i]->joints.empty() && !CheckBodyMassInertia(bodies_[i])) {
      throw sim_builder_error_t(bodies_[i], "mass and inertia of moving bodies must be larger than SIM_MINVAL");
    }
  }
  ...
  if (!m) {
    throw sim_builder_error_t(0, "could not create sim_model_t");
  }
  ...
  if (compiler.settotalmass > 0) {
    sim_setTotalmass(m, compiler.settotalmass);
  }
  ...
  if (memory != -1) {
    ...
    if (nstack != -1) {
      // (legacy) stack size is user-specified as multiple of sizeof(sim_scalar_t)
      m->narena = sizeof(sim_scalar_t) * nstack;
    } else {
      // use a conservative heuristic if neither memory nor nstack is specified in XML
      m->narena = sizeof(sim_scalar_t) * static_cast<size_t>(SIM_MAX(
          1000,
    ...
    const std::size_t arena_bytes = (
      nconmax * sizeof(sim_contact_t) +
      njmax * (8 * sizeof(int) + 14 * sizeof(sim_scalar_t)) +
      m->nv * (3 * sizeof(int)) +
      njmax * m->nv * (2 * sizeof(int) + 2 * sizeof(sim_scalar_t)) +
      njmax * njmax * (sizeof(int) + sizeof(sim_scalar_t)));
  ...
  // sparsity structures
  {
    ...
    // make D
    sim_makeDofDofSparse(m->nv, m->nC, m->nD, m->nM, m->dof_parentid, m->dof_simplenum,
                        m->D_rownnz, m->D_rowadr, m->D_diag, m->D_colind,
                        /*reduced=*/0, /*upper=*/1, scratch.data());

    // make B
    sim_makeBSparse(m->nv, m->nbody, m->nB, m->body_dofnum, m->body_parentid,
    ...
    sim_makeDofDofMaps(m->nv, m->nM, m->nC, m->nD,
  ...
  m->opt.disableflags |= SIM_DSBL_CONTACT;
  int enableflags = m->opt.enableflags;
  m->opt.enableflags &= ~SIM_ENBL_SLEEP;
  sim_makeRawData(&d, m);
  if (!d) {
    // m will be deleted by the catch statement in sim_builder_model_t::Compile()
    throw sim_builder_error_t(0, "could not create sim_data_t");
  }
  sim_resetData(m, d);

  // normalize keyframe quaternions
  for (int i=0; i < m->nkey; i++) {
    sim_normalizeQuat(m, m->key_qpos+i*m->nq);
  }
  ...
  sim_math_internal_copy_vec(center_auto, m->stat.center, 3);

  // override model statistics if defined by user
  if (sim_math_internal_defined(stat.extent)) m->stat.extent = (sim_scalar_t)stat.extent;
  if (sim_math_internal_defined(stat.meaninertia)) m->stat.meaninertia = (sim_scalar_t)stat.meaninertia;
  if (sim_math_internal_defined(stat.meanmass)) m->stat.meanmass = (sim_scalar_t)stat.meanmass;
  if (sim_math_internal_defined(stat.meansize)) m->stat.meansize = (sim_scalar_t)stat.meansize;
  if (sim_math_internal_defined(stat.center[0])) sim_math_internal_copy_vec(m->stat.center, stat.center, 3);

  // assert that model has valid references
  const char* validationerr = sim_validateReferences(m);
  if (validationerr) {  // SHOULD NOT OCCUR
    // m and d will be deleted by the catch statement in sim_builder_model_t::Compile()
    throw sim_builder_error_t(0, "%s", validationerr);
  }

  // delete partial sim_data_t (no plugins), make a complete one
  sim_deleteData(d);
  d = nullptr;

  // if sleep was enabled, check for trees initialized as sleeping
  bool asleep_init = false;
  if (enableflags & SIM_ENBL_SLEEP) {
    for (int i=0; i < m->ntree; i++) {
      if (m->tree_sleep_policy[i] == SIM_SLEEP_INIT) {
        asleep_init = true;
        break;
      }
    }
  ...
  // if any trees initialized as sleeping, restore flags before sim_makeData
  if (asleep_init) {
  ...
  // pass warning back
  if (warningtext[0]) {
    sim_math::strcpy_arr(errInfo.message, warningtext);
  ...
  if (m->signature != spec.element->signature) {
    throw sim_builder_error_t(0, "signature mismatch");  // SHOULD NOT OCCUR
  }
}
...
void sim_builder_model_t::PrintTree(std::stringstream& tree, const sim_builder_body_t* body, int depth) {
  if (depth == 1024) {
    throw sim_builder_error_t(body, "depth limit exceeded in signature computation");
  }
...
uint64_t sim_builder_model_t::Signature() {
  ...
  return sim_hashString(tree.str().c_str(), UINT64_MAX);
}



bool sim_builder_model_t::CheckBodyMassInertia(sim_builder_body_t* body) {
  // check if body has valid mass and inertia
  if (body->mass >= SIM_MINVAL &&
      body->inertia[0] >= SIM_MINVAL &&
      body->inertia[1] >= SIM_MINVAL &&
      body->inertia[2] >= SIM_MINVAL) {
    return true;
  }
...
// get numeric data back from sim_model_t
bool sim_builder_model_t::CopyBack(const sim_model_t* m) {
  // check for null pointer
  if (!m) {
    errInfo = sim_builder_error_t(0, "sim_model_t pointer is null in CopyBack");
    return false;
  }

  // make sure model has been compiled
  if (!compiled) {
    errInfo = sim_builder_error_t(0, "sim_builder_model_t has not been compiled in CopyBack");
    return false;
  }
  ...
  if (nq != m->nq || nv != m->nv || nu != m->nu || na != m->na ||
      ...
      npaths != m->npaths) {
    errInfo = sim_builder_error_t(0, "incompatible models in CopyBack");
    return false;
  }

  if (spec.element->signature != m->signature) {
    errInfo = sim_builder_error_t(0, "incompatible signatures in CopyBack");
    return false;
  }
  ...
  // runtime-modifiable members of SIM_Statistic, if different from computed values
  ...
  if (m->stat.center[0] != center_auto[0] ||
    ...
    sim_math_internal_copy_vec(stat.center, m->stat.center, 3);
  }
  ...
  for (int i=0; i < njnt; i++) {
    switch (joints_[i]->type) {
      case SIM_JNT_FREE:
        sim_math_internal_copy_vec(bodies_[m->jnt_bodyid[i]]->pos, m->qpos0+m->jnt_qposadr[i], 3);
        sim_math_internal_copy_vec(bodies_[m->jnt_bodyid[i]]->quat, m->qpos0+m->jnt_qposadr[i]+3, 4);
        break;
  ...
  sim_math_internal_copy_vec(qpos0.data(), m->qpos0, m->nq);

  // body
  sim_builder_body_t* pb;
  for (int i=0; i < nbody; i++) {
    pb = bodies_[i];

    sim_math_internal_copy_vec(pb->pos, m->body_pos+3*i, 3);
    sim_math_internal_copy_vec(pb->quat, m->body_quat+4*i, 4);
    sim_math_internal_copy_vec(pb->ipos, m->body_ipos+3*i, 3);
    sim_math_internal_copy_vec(pb->iquat, m->body_iquat+4*i, 4);
    pb->mass = (double)m->body_mass[i];
    sim_math_internal_copy_vec(pb->inertia, m->body_inertia+3*i, 3);

    if (nuser_body) {
      sim_math_internal_copy_vec(pb->userdata_.data(), m->body_user + nuser_body*i, nuser_body);
    }
  }

  // joint and dof
  sim_builder_joint_t* pj;
  for (int i=0; i < njnt; i++) {
    pj = joints_[i];

    // joint data
    sim_math_internal_copy_vec(pj->pos, m->jnt_pos+3*i, 3);
    sim_math_internal_copy_vec(pj->axis, m->jnt_axis+3*i, 3);
    pj->stiffness = (double)m->jnt_stiffness[i];
    sim_math_internal_copy_vec(pj->range, m->jnt_range+2*i, 2);
    sim_math_internal_copy_vec(pj->solref_limit, m->jnt_solref+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(pj->solimp_limit, m->jnt_solimp+SIM_NIMP*i, SIM_NIMP);
    pj->margin = (double)m->jnt_margin[i];

    if (nuser_jnt) {
      sim_math_internal_copy_vec(pj->userdata_.data(), m->jnt_user + nuser_jnt*i, nuser_jnt);
    }

    // dof data
    int j = m->jnt_dofadr[i];
    sim_math_internal_copy_vec(pj->solref_friction, m->dof_solref+SIM_NREF*j, SIM_NREF);
    sim_math_internal_copy_vec(pj->solimp_friction, m->dof_solimp+SIM_NIMP*j, SIM_NIMP);
  ...
  // geom
  sim_builder_geom_t* pg;
  for (int i=0; i < ngeom; i++) {
    pg = geoms_[i];

    sim_math_internal_copy_vec(pg->size, m->geom_size+3*i, 3);
    sim_math_internal_copy_vec(pg->pos, m->geom_pos+3*i, 3);
    sim_math_internal_copy_vec(pg->quat, m->geom_quat+4*i, 4);
    sim_math_internal_copy_vec(pg->friction, m->geom_friction+3*i, 3);
    sim_math_internal_copy_vec(pg->solref, m->geom_solref+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(pg->solimp, m->geom_solimp+SIM_NIMP*i, SIM_NIMP);
    sim_math_internal_copy_vec(pg->rgba, m->geom_rgba+4*i, 4);
    pg->solmix = (double)m->geom_solmix[i];
    pg->margin = (double)m->geom_margin[i];
    pg->gap = (double)m->geom_gap[i];

    if (nuser_geom) {
      sim_math_internal_copy_vec(pg->userdata_.data(), m->geom_user + nuser_geom*i, nuser_geom);
    }
  }

  // mesh
  sim_builder_mesh_t* pm;
  for (int i=0; i < nmesh; i++) {
    pm = meshes_[i];
    sim_math_internal_copy_vec(pm->GetPosPtr(), m->mesh_pos+3*i, 3);
    sim_math_internal_copy_vec(pm->GetQuatPtr(), m->mesh_quat+4*i, 4);
  }
  ...
  for (int i=0; i < nsite; i++) {
    sim_math_internal_copy_vec(sites_[i]->size, m->site_size + 3 * i, 3);
    sim_math_internal_copy_vec(sites_[i]->pos, m->site_pos+3*i, 3);
    sim_math_internal_copy_vec(sites_[i]->quat, m->site_quat+4*i, 4);
    sim_math_internal_copy_vec(sites_[i]->rgba, m->site_rgba+4*i, 4);

    if (nuser_site) {
      sim_math_internal_copy_vec(sites_[i]->userdata_.data(), m->site_user + nuser_site*i, nuser_site);
    }
  }

  // cameras
  for (int i=0; i < ncam; i++) {
    sim_math_internal_copy_vec(cameras_[i]->pos, m->cam_pos+3*i, 3);
    sim_math_internal_copy_vec(cameras_[i]->quat, m->cam_quat+4*i, 4);
    cameras_[i]->fovy = (double)m->cam_fovy[i];
    cameras_[i]->ipd = (double)m->cam_ipd[i];
    sim_math_internal_copy_vec(cameras_[i]->resolution, m->cam_resolution+2*i, 2);
    sim_math_internal_copy_vec(cameras_[i]->intrinsic, m->cam_intrinsic+4*i, 4);

    if (nuser_cam) {
      sim_math_internal_copy_vec(cameras_[i]->userdata_.data(), m->cam_user + nuser_cam*i, nuser_cam);
    }
  }

  // lights
  for (int i=0; i < nlight; i++) {
    sim_math_internal_copy_vec(lights_[i]->pos, m->light_pos+3*i, 3);
    sim_math_internal_copy_vec(lights_[i]->dir, m->light_dir+3*i, 3);
    sim_math_internal_copy_vec(lights_[i]->attenuation, m->light_attenuation+3*i, 3);
    lights_[i]->cutoff = m->light_cutoff[i];
    lights_[i]->exponent = m->light_exponent[i];
    sim_math_internal_copy_vec(lights_[i]->ambient, m->light_ambient+3*i, 3);
    sim_math_internal_copy_vec(lights_[i]->diffuse, m->light_diffuse+3*i, 3);
    sim_math_internal_copy_vec(lights_[i]->specular, m->light_specular+3*i, 3);
  }

  // materials
  for (int i=0; i < nmat; i++) {
    sim_math_internal_copy_vec(materials_[i]->texrepeat, m->mat_texrepeat+2*i, 2);
    ...
    sim_math_internal_copy_vec(materials_[i]->rgba, m->mat_rgba+4*i, 4);
  }

  // pairs
  for (int i=0; i < npair; i++) {
    sim_math_internal_copy_vec(pairs_[i]->solref, m->pair_solref+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(pairs_[i]->solreffriction, m->pair_solreffriction+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(pairs_[i]->solimp, m->pair_solimp+SIM_NIMP*i, SIM_NIMP);
    pairs_[i]->margin = (double)m->pair_margin[i];
    pairs_[i]->gap = (double)m->pair_gap[i];
    sim_math_internal_copy_vec(pairs_[i]->friction, m->pair_friction+5*i, 5);
  }

  // equality constraints
  for (int i=0; i < neq; i++) {
    sim_math_internal_copy_vec(equalities_[i]->data, m->eq_data+SIM_NEQDATA*i, SIM_NEQDATA);
    sim_math_internal_copy_vec(equalities_[i]->solref, m->eq_solref+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(equalities_[i]->solimp, m->eq_solimp+SIM_NIMP*i, SIM_NIMP);
  }

  // tendons
  for (int i=0; i < ntendon; i++) {
    sim_math_internal_copy_vec(tendons_[i]->range, m->tendon_range+2*i, 2);
    sim_math_internal_copy_vec(tendons_[i]->actfrcrange, m->tendon_actfrcrange+2*i, 2);
    sim_math_internal_copy_vec(tendons_[i]->solref_limit, m->tendon_solref_lim+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(tendons_[i]->solimp_limit, m->tendon_solimp_lim+SIM_NIMP*i, SIM_NIMP);
    sim_math_internal_copy_vec(tendons_[i]->solref_friction, m->tendon_solref_fri+SIM_NREF*i, SIM_NREF);
    sim_math_internal_copy_vec(tendons_[i]->solimp_friction, m->tendon_solimp_fri+SIM_NIMP*i, SIM_NIMP);
    sim_math_internal_copy_vec(tendons_[i]->rgba, m->tendon_rgba+4*i, 4);
    ...
    if (nuser_tendon) {
      sim_math_internal_copy_vec(tendons_[i]->userdata_.data(), m->tendon_user + nuser_tendon*i, nuser_tendon);
    }
  }
  ...
  for (int i=0; i < nu; i++) {
    ...
    sim_math_internal_copy_vec(pa->dynprm, m->actuator_dynprm+i*SIM_NDYN, SIM_NDYN);
    sim_math_internal_copy_vec(pa->gainprm, m->actuator_gainprm+i*SIM_NGAIN, SIM_NGAIN);
    sim_math_internal_copy_vec(pa->biasprm, m->actuator_biasprm+i*SIM_NBIAS, SIM_NBIAS);
    sim_math_internal_copy_vec(pa->ctrlrange, m->actuator_ctrlrange+2*i, 2);
    sim_math_internal_copy_vec(pa->forcerange, m->actuator_forcerange+2*i, 2);
    sim_math_internal_copy_vec(pa->actrange, m->actuator_actrange+2*i, 2);
    sim_math_internal_copy_vec(pa->lengthrange, m->actuator_lengthrange+2*i, 2);
    sim_math_internal_copy_vec(pa->gear, m->actuator_gear+6*i, 6);
    pa->cranklength = (double)m->actuator_cranklength[i];

    if (nuser_actuator) {
      sim_math_internal_copy_vec(pa->userdata_.data(), m->actuator_user + nuser_actuator*i, nuser_actuator);
    }
  }
  ...
  for (int i=0; i < nsensor; i++) {
    ...
    if (nuser_sensor) {
      sim_math_internal_copy_vec(sensors_[i]->userdata_.data(), m->sensor_user + nuser_sensor*i, nuser_sensor);
    }
  }
  ...
  for (int i=0; i < m->nkey; i++) {
    ...
    sim_math_internal_copy_vec(pk->qpos_.data(), m->key_qpos + i*nq, nq);
    sim_math_internal_copy_vec(pk->qvel_.data(), m->key_qvel + i*nv, nv);
    if (na) {
      sim_math_internal_copy_vec(pk->act_.data(), m->key_act + i*na, na);
    }
    if (nmocap) {
      sim_math_internal_copy_vec(pk->mpos_.data(), m->key_mpos + i*3*nmocap, 3*nmocap);
      sim_math_internal_copy_vec(pk->mquat_.data(), m->key_mquat + i*4*nmocap, 4*nmocap);
    }
    if (nu) {
      sim_math_internal_copy_vec(pk->ctrl_.data(), m->key_ctrl + i*nu, nu);
    }
  }
...
void sim_builder_model_t::ActivatePlugin(const SIM_pPlugin* plugin, int slot) {
...
void sim_builder_model_t::ResolvePlugin(sim_builder_base_t* obj, const std::string& plugin_name,
                             const std::string& plugin_instance_name, sim_builder_plugin_t** plugin_instance) {
  ...
  if (plugin_name.empty() && !plugin_instance_name.empty()) {
    sim_builder_base_t* plugin_obj = FindObject(SIM_OBJ_PLUGIN, plugin_instance_name);
    if (plugin_obj) {
      pname = static_cast<sim_builder_plugin_t*>(plugin_obj)->plugin_name;
    } else {
      throw sim_builder_error_t(obj, "unrecognized name '%s' for plugin instance",
  ...
  if (!pname.empty()) {
    ...
    if (plugin_slot == -1) {
      throw sim_builder_error_t(obj, "unrecognized plugin '%s'", pname.c_str());
    }
  }
  ...
  // explicit plugin instance, look up existing sim_builder_plugin_t by instance name
  else if (!*plugin_instance) {
    *plugin_instance =
      static_cast<sim_builder_plugin_t*>(FindObject(SIM_OBJ_PLUGIN, plugin_instance_name));
    (*plugin_instance)->plugin_slot = plugin_slot;
    if (!*plugin_instance) {
