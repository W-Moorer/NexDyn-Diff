...
bool sim_math_internal_defined(double num) {
  return !std::isnan(num);
}


// compute address of M[g1][g2] where M is triangular n-by-n
int sim_math_internal_matadr(int g1, int g2, int n) {
...
// set 4D vector
void sim_math_internal_set_vec(double* dest, double x, double y, double z, double w) {
  dest[0] = x;
  dest[1] = y;
  dest[2] = z;
  dest[3] = w;
}
void sim_math_internal_set_vec(float* dest, double x, double y, double z, double w) {
  dest[0] = (float)x;
  dest[1] = (float)y;
  dest[2] = (float)z;
  dest[3] = (float)w;
...
// set 3D vector
void sim_math_internal_set_vec(double* dest, double x, double y, double z) {
  dest[0] = x;
  dest[1] = y;
  dest[2] = z;
}
void sim_math_internal_set_vec(float* dest, double x, double y, double z) {
  dest[0] = (float)x;
  dest[1] = (float)y;
  dest[2] = (float)z;
}
...
// set 2D vector
void sim_math_internal_set_vec(double* dest, double x, double y) {
  dest[0] = x;
  dest[1] = y;
}
...
// add to double array
void sim_math_internal_addtovec(double* dest, const double* src, int n) {
  for (int i=0; i < n; i++) {
    dest[i] += src[i];
  }
}
...
// zero double array
void sim_math_internal_zerovec(double* dest, int n) {
  for (int i=0; i < n; i++) {
    dest[i] = 0;
  }
}
...
// zero float array
void sim_math_internal_zerovec(float* dest, int n) {
  for (int i=0; i < n; i++) {
    dest[i] = 0;
  }
}
...
// dot-product in 3D
double sim_math_internal_dot3(const double* a, const double* b) {
  return a[0]*b[0] + a[1]*b[1] + a[2]*b[2];
}


// distance between 3D points
double sim_math_internal_dist3(const double* a, const double* b) {
  return sqrt((a[0]-b[0])*(a[0]-b[0]) + (a[1]-b[1])*(a[1]-b[1]) + (a[2]-b[2])*(a[2]-b[2]));
}


// L1 norm between vectors
double sim_math_internal_L1(const double* a, const double* b, int n) {
  double res = 0;
  for (int i=0; i < n; i++) {
    res += std::abs(a[i]-b[i]);
  }
...
double sim_math_internal_normvec(double* vec, const int n) {
  double nrm = 0;
  ...
  // don't normalize if nrm is within SIM_EPS of 1
  if (std::abs(nrm - 1) > SIM_EPS) {
    for (int i=0; i < n; i++) {
      vec[i] /= nrm;
    }
  }
...
float sim_math_internal_normvec(float* vec, const int n) {
  float nrm = 0;
...
void sim_math_internal_scalevec(double* res, const double* vec, double s, int n) {
  for (int i = 0; i < n; i++) {
    res[i] = s * vec[i];
  }
}
...
void sim_math_internal_quat2mat(double* res, const double* quat) {
...
void sim_math_internal_mulquat(double* res, const double* qa, const double* qb) {
  double tmp[4];
  ...
  sim_math_internal_normvec(tmp, 4);
  sim_math_internal_copy_vec(res, tmp, 4);
}


// multiply matrix by vector, 3-by-3
void sim_math_internal_mulvecmat(double* res, const double* vec, const double* mat) {
  double tmp[3] = {
...
void sim_math_internal_mulvecmatT(double* res, const double* vec, const double* mat) {
  double tmp[3] = {
...
// compute res = R * M * R'
void sim_math_internal_mulRMRT(double* res, const double* R, const double* M) {
  double tmp[9];
...
void sim_math_internal_mulmat(double* res, const double* A, const double* B) {
  double tmp[9];
  ...
  sim_math_internal_copy_vec(res, tmp, 9);
}


// transpose 3-by-3 matrix
void sim_math_internal_transposemat(double* res, const double* mat) {
...
void sim_math_internal_localaxis(double* al, const double* ag, const double* quat) {
  double mat[9];
  double qneg[4] = {quat[0], -quat[1], -quat[2], -quat[3]};
  sim_math_internal_quat2mat(mat, qneg);
  sim_math_internal_mulvecmat(al, ag, mat);
}


// convert global to local position relative to given frame
void sim_math_internal_localpos(double* pl, const double* pg, const double* pos, const double* quat) {
  double a[3] = {pg[0]-pos[0], pg[1]-pos[1], pg[2]-pos[2]};
  sim_math_internal_localaxis(pl, a, quat);
}


// compute quaternion rotation from parent to child
void sim_math_internal_localquat(double* local, const double* child, const double* parent) {
  double pneg[4] = {parent[0], -parent[1], -parent[2], -parent[3]};
  sim_math_internal_mulquat(local, pneg, child);
}


// compute vector cross-product a = b x c
void sim_math_internal_crossvec(double* a, const double* b, const double* c) {
  a[0] = b[1]*c[2] - b[2]*c[1];
  a[1] = b[2]*c[0] - b[0]*c[2];
  a[2] = b[0]*c[1] - b[1]*c[0];
}
...
template<typename T> double sim_math_internal_makenormal(double* normal, const T a[3],
  ...
  sim_math_internal_crossvec(normal, diffAB, diffAC);
  double nrm = std::sqrt(sim_math_internal_dot3(normal, normal));
  if (nrm < SIM_EPS) {
    normal[0] = 1;
    normal[1] = 0;
    normal[2] = 0;
...
template double sim_math_internal_makenormal(double* normal, const double a[3],
                                const double b[3], const double c[3]);
template double sim_math_internal_makenormal(double* normal, const float a[3],
                                const float b[3], const float c[3]);

// compute quaternion as minimal rotation from [0;0;1] to vec
void sim_math_internal_z2quat(double* quat, const double* vec) {
  double z[3] = {0, 0, 1};
  sim_math_internal_crossvec(quat+1, z, vec);
  double s = sim_math_internal_normvec(quat+1, 3);
  if (s < 1E-10) {
    quat[1] = 1;
    quat[2] = quat[3] = 0;
  }
...
void sim_math_internal_frame2quat(double* quat, const double* x, const double* y, const double* z) {
...
void sim_math_internal_frameinvert(double newpos[3], double newquat[4],
                      const double oldpos[3], const double oldquat[4]) {
  // position
  sim_math_internal_localaxis(newpos, oldpos, oldquat);
  newpos[0] = -newpos[0];
  newpos[1] = -newpos[1];
  newpos[2] = -newpos[2];
...
void sim_math_internal_frameaccum(double pos[3], double quat[4],
                     const double childpos[3], const double childquat[4]) {
  double mat[9], vec[3], qtmp[4];
  sim_math_internal_quat2mat(mat, quat);
  sim_math_internal_mulvecmat(vec, childpos, mat);
  pos[0] += vec[0];
  pos[1] += vec[1];
  pos[2] += vec[2];
  sim_math_internal_mulquat(qtmp, quat, childquat);
  sim_math_internal_copy_vec(quat, qtmp, 4);
}


// accumulate frame transformation in second frame
void sim_math_internal_frameaccumChild(const double pos[3], const double quat[4],
                          double childpos[3], double childquat[4]) {
  double p[] = {pos[0], pos[1], pos[2]};
  double q[] = {quat[0], quat[1], quat[2], quat[3]};
  sim_math_internal_frameaccum(p, q, childpos, childquat);
  sim_math_internal_copy_vec(childpos, p, 3);
  sim_math_internal_copy_vec(childquat, q, 4);
}


// invert frame accumulation
void sim_math_internal_frameaccuminv(double pos[3], double quat[4],
                        const double childpos[3], const double childquat[4]) {
  double mat[9], vec[3], qtmp[4];
  double qneg[4] = {childquat[0], -childquat[1], -childquat[2], -childquat[3]};
  sim_math_internal_mulquat(qtmp, quat, qneg);
  sim_math_internal_copy_vec(quat, qtmp, 4);
  sim_math_internal_quat2mat(mat, quat);
  sim_math_internal_mulvecmat(vec, childpos, mat);
  pos[0] -= vec[0];
...
void sim_math_internal_globalinertia(double* global, const double* local, const double* quat) {
  double mat[9];
  sim_math_internal_quat2mat(mat, quat);
...
void sim_math_internal_offcenter(double* res, const double mass, const double* vec) {
...
void sim_math_internal_visccoef(double* visccoef, double mass, const double* inertia, double scl) {
  // compute equivalent box
  double ebox[3];
  ebox[0] = sqrt(std::max(SIM_EPS, (inertia[1] + inertia[2] - inertia[0])) / mass * 6.0);
  ebox[1] = sqrt(std::max(SIM_EPS, (inertia[0] + inertia[2] - inertia[1])) / mass * 6.0);
  ebox[2] = sqrt(std::max(SIM_EPS, (inertia[0] + inertia[1] - inertia[2])) / mass * 6.0);
...
static void sim_math_internal_axisAngle2Quat(double res[4], const double axis[3], double angle) {
...
void sim_math_internal_rotVecQuat(double res[3], const double vec[3], const double quat[4]) {
  // zero vec: zero res
  ...
  // null quat: copy vec
    ...
    sim_math_internal_copy_vec(res, vec, 3);
  }
...
double sim_math_internal_updateFrame(double quat[4], double normal[3], const double edge[3],
                        const double tprv[3], const double tnxt[3], int first) {
  double tangent[3], binormal[3];

  // normalize tangent
  sim_math_internal_copy_vec(tangent, edge, 3);
  sim_math_internal_normvec(tangent, 3);

  // compute moving frame
  if (first) {
    // use the first vertex binormal for the first edge
    sim_math_internal_crossvec(binormal, tangent, tnxt);
    sim_math_internal_normvec(binormal, 3);

    // compute edge normal given tangent and binormal
    sim_math_internal_crossvec(normal, binormal, tangent);
    sim_math_internal_normvec(normal, 3);
  } else {
    double darboux[4];

    // rotate edge normal about the vertex binormal
    sim_math_internal_crossvec(binormal, tprv, tangent);
    double angle = atan2(sim_math_internal_normvec(binormal, 3), sim_math_internal_dot3(tprv, tangent));
    sim_math_internal_axisAngle2Quat(darboux, binormal, angle);
    sim_math_internal_rotVecQuat(normal, normal, darboux);
    sim_math_internal_normvec(normal, 3);

    // compute edge binormal given tangent and normal
    sim_math_internal_crossvec(binormal, tangent, normal);
    sim_math_internal_normvec(binormal, 3);
  }
  // global orientation of the frame
  sim_math_internal_frame2quat(quat, tangent, normal, binormal);

  // return edge length
  return sqrt(sim_math_internal_dot3(edge, edge));
}


// eigenvalue decomposition of symmetric 3x3 matrix
static const double kEigEPS = 1E-12;
int sim_math_internal_eig3(double eigval[3], double eigvec[9], double quat[4], const double mat[9]) {
  ...
  for (iter=0; iter < 500; iter++) {
    ...
    sim_math_internal_quat2mat(eigvec, quat);
    sim_math_internal_transposemat(tmp2, eigvec);
    sim_math_internal_mulmat(tmp, tmp2, mat);
    sim_math_internal_mulmat(D, tmp, eigvec);
    ...
    sim_math_internal_mulquat(quat, quat, tmp);
    sim_math_internal_normvec(quat, 4);
  }
...
void sim_math_internal_trnVecPose(double res[3], const double pos[3], const double quat[4],
                     const double vec[3]) {
  // res = quat*vec + pos
...
std::string sim_math_internal_strippath(std::string filename) {
...
const char* sim_math_internal_fullInertia(double quat[4], double inertia[3], const double fullinertia[6]) {
  ...
  // copy
  if (quat) {
    sim_math_internal_copy_vec(quat, quattmp, 4);
  }

  if (inertia) {
    sim_math_internal_copy_vec(inertia, eigval, 3);
  }
...
// strip extension
std::string sim_math_internal_stripext(std::string filename) {
  // find last dot
...
std::string sim_math_internal_getext(std::string_view filename) {
...
bool sim_math_internal_isabspath(std::string path) {
  // empty: not absolute
  if (path.empty()) {
    return false;
  }
  ...
  if (sim_plugin_getResourceProvider(path.c_str()) != nullptr) {
    return true;
  }
...
std::string sim_math_internal_combinePaths(const std::string& path1, const std::string& path2) {
  // path2 has absolute path
  if (sim_math_internal_isabspath(path2)) {
    return path2;
  }
...
std::string sim_math_internal_combinePaths(const std::string& path1, const std::string& path2,
  ...
  return sim_math_internal_combinePaths(path1, sim_math_internal_combinePaths(path2, path3));
}
...
static bool sim_math_internal_isValidContentType(std::string_view text) {
...
std::optional<std::string_view> sim_math_internal_parseContentTypeAttrType(std::string_view text) {
  if (!sim_math_internal_isValidContentType(text)) {
    return std::nullopt;
  }
...
// return empty string on invalid format
std::optional<std::string_view> sim_math_internal_parseContentTypeAttrSubtype(std::string_view text) {
  if (!sim_math_internal_isValidContentType(text)) {
    return std::nullopt;
  }
...
std::string sim_math_internal_extToContentType(std::string_view filename) {
  std::string ext = sim_math_internal_getext(filename);
...
int sim_math_internal_dirnamelen(const char* path) {
  if (!path) {
    return 0;
  }
...
std::string FilePath::AbsPrefix(const std::string& str) {
  ...
  // path is scheme:filename which we consider an absolute path
  // e.g. file URI's are always absolute paths
  const SIM_pResourceProvider* provider = sim_plugin_getResourceProvider(str.c_str());
  if (provider != nullptr) {
...
std::vector<uint8_t> FileToMemory(const char* filename) {
  ...
  if (fseek(fp, 0, SEEK_END) != 0) {
    fclose(fp);
    sim_warning("Failed to calculate size for '%s'", filename);
    return {};
  }
  ...
  if (long_filesize > INT_MAX) {
    fclose(fp);
    sim_warning("File size over 2GB is not supported. File: '%s'", filename);
    return {};
  ...
  if (fseek(fp, 0, SEEK_SET) != 0) {
    fclose(fp);
    sim_warning("Read error while reading '%s'", filename);
    return {};
  }
