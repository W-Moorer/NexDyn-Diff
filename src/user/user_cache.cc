...
SIM_CAsset SIM_CAsset::Copy(const SIM_CAsset& other) {
  SIM_CAsset asset;
  asset.id_ = other.Id();
...
// requirement
void SIM_CCache::SetCapacity(std::size_t size) {
...
const std::string* SIM_CCache::HasAsset(const std::string& id) {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool SIM_CCache::Insert(const std::string& modelname, const std::string& id, const SIM_Resource *resource,
  ...
  SIM_CAsset asset(modelname, id, resource, data, size);
  auto [it, inserted] = lookup_.insert({id, asset});
  SIM_CAsset* asset_ptr = &(it->second);
...
// copied
bool SIM_CCache::PopulateData(const std::string& id, const SIM_Resource* resource, SIM_CDataFunc fn) {
  std::lock_guard<std::mutex> lock(mutex_);
  auto it = lookup_.find(id);
  if (it == lookup_.end()) {
    return false;
  ...
  if (sim_math_isModifiedResource(resource, it->second.Timestamp().c_str())) {
    return false;
  }

  SIM_CAsset* asset = &(it->second);
...
void SIM_CCache::RemoveModel(const std::string& filename) {
  std::lock_guard<std::mutex> lock(mutex_);
  for (SIM_CAsset* asset : models_[filename]) {
...
void SIM_CCache::Reset(const std::string& filename) {
  std::lock_guard<std::mutex> lock(mutex_);
...
std::size_t SIM_CCache::Capacity() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return capacity_;
}
...
std::size_t SIM_CCache::Size() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return size_;
}
...
void SIM_CCache::DeleteAsset(const std::string& id) {
  std::lock_guard<std::mutex> lock(mutex_);
  auto it = lookup_.find(id);
  if (it != lookup_.end()) {
    Delete(&(it->second));
...
void SIM_CCache::Delete(SIM_CAsset* asset) {
...
// Deletes a single asset (internal)
void SIM_CCache::Delete(SIM_CAsset* asset, const std::string& skip) {
  size_ -= asset->BytesCount();
  entries_.erase(asset);
