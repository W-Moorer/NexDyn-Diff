...
#ifndef SIMCORE_SRC_USER_USER_FLEXCOMP_H_
#define SIMCORE_SRC_USER_USER_FLEXCOMP_H_
...
typedef enum _mjtFcompType {
  SIM_FCOMPTYPE_GRID = 0,
  SIM_FCOMPTYPE_BOX,
  SIM_FCOMPTYPE_CYLINDER,
  SIM_FCOMPTYPE_ELLIPSOID,
  SIM_FCOMPTYPE_SQUARE,
  SIM_FCOMPTYPE_DISC,
  SIM_FCOMPTYPE_CIRCLE,
  SIM_FCOMPTYPE_MESH,
  SIM_FCOMPTYPE_GMSH,
  SIM_FCOMPTYPE_DIRECT,
...
typedef enum _mjtDof {
  SIM_FCOMPDOF_FULL = 0,
  SIM_FCOMPDOF_RADIAL,
  SIM_FCOMPDOF_TRILINEAR,
  SIM_FCOMPDOF_QUADRATIC,
...
class SIM_CFlexcomp {
 public:
  SIM_CFlexcomp(void);
  bool Make(sim_spec_body_t* body, char* error, int error_sz);
  ...
  bool MakeMesh(sim_builder_model_t* model, SIM_sCompiler* compiler, char* error, int error_sz);
  bool MakeGMSH(sim_builder_model_t* model, SIM_sCompiler* compiler, char* error, int error_sz);
  void LoadGMSH(sim_builder_model_t* model, SIM_Resource* resource);
  ...
  SIM_tFcompType type;              // flexcomp type
  ...
  SIM_tDof doftype;                 // dof type, all vertices or trilinear interpolation
  ...
  sim_builder_default_t def;                     // local copy, parsed parameters stored here
...
#endif  // SIMCORE_SRC_USER_USER_FLEXCOMP_H_
