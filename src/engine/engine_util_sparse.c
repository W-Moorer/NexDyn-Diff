...
void sim_math_dotSparseX3(sim_scalar_t* res0, sim_scalar_t* res1, sim_scalar_t* res2,
                     const sim_scalar_t* vec10, const sim_scalar_t* vec11, const sim_scalar_t* vec12,
                     const sim_scalar_t* vec2, int nnz1, const int* ind1) {
#ifdef SIM_USEAVX
  sim_math_dotSparseX3_avx(res0, res1, res2, vec10, vec11, vec12, vec2, nnz1, ind1);
#else
  int i = 0;
...
sim_scalar_t sim_math_dotSparse2(const sim_scalar_t* vec1, const int* ind1, int nnz1,
...
int sim_math_dense2sparse(sim_scalar_t* res, const sim_scalar_t* mat, int nr, int nc,
...
void sim_math_sparse2dense(sim_scalar_t* res, const sim_scalar_t* mat, int nr, int nc,
                      const int* rownnz, const int* rowadr, const int* colind) {
  // clear
  sim_math_zero(res, nr*nc);
...
void sim_math_copySparse(sim_scalar_t* res, const sim_scalar_t* mat, const int* rownnz, const int* rowadr,
  ...
  for (int i=0; i < nrow; i++) {
    int r = row[i];
    sim_math_copy(res + rowadr[r], mat + rowadr[r], rownnz[r]);
  }
}
...
void sim_math_zeroSparse(sim_scalar_t* res, const int* rownnz, const int* rowadr, const int* row, int nrow) {
  for (int i=0; i < nrow; i++) {
    int r = row[i];
    sim_math_zero(res + rowadr[r], rownnz[r]);
  }
}


// multiply sparse matrix and dense vector:  res = mat * vec.
void sim_math_mulMatVecSparse(sim_scalar_t* res, const sim_scalar_t* mat, const sim_scalar_t* vec,
                         int nr, const int* rownnz, const int* rowadr,
                         const int* colind, const int* rowsuper) {
#ifdef SIM_USEAVX
  sim_math_mulMatVecSparse_avx(res, mat, vec, nr, rownnz, rowadr, colind, rowsuper);
#else
  // regular sparse dot-product
  for (int r=0; r < nr; r++) {
    res[r] = sim_math_dotSparse(mat+rowadr[r], vec, rownnz[r], colind+rowadr[r]);
  }
#endif  // SIM_USEAVX
}
...
void sim_math_mulMatTVecSparse(sim_scalar_t* res, const sim_scalar_t* mat, const sim_scalar_t* vec, int nr, int nc,
                          const int* rownnz, const int* rowadr, const int* colind) {
  // clear res
  sim_math_zero(res, nc);

  for (int i=0; i < nr; i++) {
    sim_scalar_t scl = vec[i];
    ...
    const sim_scalar_t* row = mat + adr;
    for (int j=0; j < nnz; j++) {
      res[ind[j]] += row[j] * scl;
    }
  }
...
void sim_math_addToMatSparse(sim_scalar_t* dst, int* rownnz, int* rowadr, int* colind, int nr,
                        const sim_scalar_t* M, const int* M_rownnz, const int* M_rowadr,
                        const int* M_colind,
                        sim_scalar_t* buf_val, int* buf_ind) {
  for (int i=0; i < nr; i++) {
    rownnz[i] = sim_math_combineSparse(dst + rowadr[i], M + M_rowadr[i], 1, 1,
...
void sim_math_mulSymVecSparse(sim_scalar_t* restrict res, const sim_scalar_t* restrict mat,
                         const sim_scalar_t* restrict vec, int n,
...
int sim_math_combineSparseCount(int a_nnz, int b_nnz, const int* a_ind, const int* b_ind) {
  int a = 0, b = 0, c_nnz = 0;
...
void sim_math_combineSparseInc(sim_scalar_t* dst, const sim_scalar_t* src, int n, sim_scalar_t a, sim_scalar_t b,
  ...
  // scale dst by a
  if (a != 1) {
    sim_math_scl(dst, dst, a, dst_nnz);
  }
...
int sim_math_addToSparseMat(sim_scalar_t* dst, const sim_scalar_t* src, int n, int nrow, sim_scalar_t scl,
  ...
  if (dst_nnz == src_nnz) {
    ...
    if (sim_math_compare(dst_ind, src_ind, dst_nnz)) {
      // combine sim_scalar_t data directly
      sim_math_addToScl(dst, src, scl, nrow*dst_nnz);
      return dst_nnz;
    }
  }
  ...
  sim_math_transpose(dst, buf, nnz, nrow);
  sim_math_copyInt(dst_ind, buf_ind, nnz);
...
int sim_math_addChains(int* res, int n, int NV1, int NV2,
  ...
  if (NV1 == NV2) {
    ...
    if (sim_math_compare(chain1, chain2, NV1)) {
      sim_math_copyInt(res, chain1, NV1);
      return NV1;
    }
  }
...
int sim_math_compressSparse(sim_scalar_t* mat, int nr, int nc, int* rownnz, int* rowadr, int* colind,
                       sim_scalar_t minval) {
  ...
  for (int r=0; r < nr; r++) {
    ...
    for (int adr_old=rowadr_old; adr_old < end; adr_old++) {
      if (remove_small && sim_math_abs(mat[adr_old]) <= minval) {
        continue;
      }
      if (adr != adr_old) {
        mat[adr] = mat[adr_old];
...
void sim_math_transposeSparse(sim_scalar_t* res, const sim_scalar_t* mat, int nr, int nc,
                         int* res_rownnz, int* res_rowadr, int* res_colind, int* res_rowsuper,
                         const int* rownnz, const int* rowadr, const int* colind) {
  if (!nr || !nc) return;

  // clear number of non-zeros for each row of transposed
  sim_math_zeroInt(res_rownnz, nc);
...
void sim_math_superSparse(int nr, int* rowsuper,
                     const int* rownnz, const int* rowadr, const int* colind) {
  ...
  for (int r=0; r < nr-1; r++) {
    ...
    // same number of nonzeros: compare colind vectors
      ...
      rowsuper[r] = sim_math_compare(colind+rowadr[r], colind+rowadr[r+1], rownnz[r]);
    }
  }
...
int sim_math_sqrMatTDSparseCount(int* res_rownnz, int* res_rowadr, int nr,
                            const int* rownnz, const int* rowadr, const int* colind,
                            const int* rownnzT, const int* rowadrT, const int* colindT,
                            const int* rowsuperT, sim_data_t* d, int flg_upper) {
  sim_markStack(d);
  int* chain = SIM_STACK_ALLOC(d, 2*nr, int);
  int nchain = 0;
  int* res_colind = NULL;
...
void sim_math_sqrMatTDUncompressedInit(int* res_rowadr, int nc) {
  for (int r=0; r < nc; r++) {
    res_rowadr[r] = r*nc;
  }
}
...
#define SIM_MAXSUPER 8

// compute sparse M'*diag*M (diag=NULL: compute M'*M), res_rowadr must be precomputed
void sim_math_sqrMatTDSparse(sim_scalar_t* res, const sim_scalar_t* mat, const sim_scalar_t* matT,
  ...
  int* marker = SIM_STACK_ALLOC(d, nc, int);
  sim_math_zeroInt(marker, nc);

  // dense buffer (considered column-major) containing up to SIM_MAXSUPER columns
  sim_scalar_t* buffer = SIM_STACK_ALLOC(d, nc*SIM_MAXSUPER, sim_scalar_t);

  // dense index vector of the current column (unsorted)
  int* buffer_idx = SIM_STACK_ALLOC(d, nc, int);

  // rowstart[i]: address of first row in column mat'[:, i] with index > current column
  int* rowstart = SIM_STACK_ALLOC(d, nr, int);
  sim_math_zeroInt(rowstart, nr);
  ...
  for (int c=0; c < nc; c++) {
    ...
    const sim_scalar_t* val[SIM_MAXSUPER];
    ...
    sim_scalar_t diag_c[SIM_MAXSUPER];
    if (diag) {
      for (int s=0; s < ns; s++) {
        sim_scalar_t ds = 0;
        for (int k=0; k < nnz; k++) {
      ...
      for (int s=0; s < ns; s++) {
        diag_c[s] = sim_math_dot(val[s], val[s], nnz);
      }
    }
    ...
    for (int i=0; i < nnz; i++) {
      ...
      const sim_scalar_t* valT = matT_csc + adrT;

      // get v[s] = diag[r] * mat[r, c + s] for s in [0, ns)
      sim_scalar_t v[SIM_MAXSUPER];
      if (diag) {
        sim_scalar_t diag_r = diag[r];
        for (int s=0; s < ns; s++) {
          v[s] = diag_r * val[s][i];
        }
      } else {
...
void sim_math_sqrMatTDSparse_row(sim_scalar_t* res, const sim_scalar_t* mat, const sim_scalar_t* matT,
                            const sim_scalar_t* diag, int nr, int nc,
                            int* res_rownnz, const int* res_rowadr, int* res_colind,
                            const int* rownnz, const int* rowadr,
                            const int* colind, const int* rowsuper,
  ...
  for (int i=0; i < nc; i++) {
    ...
    if (rowsuperT && i > 0 && rowsuperT[i-1]) {
      ...
      sim_math_copyInt(cols, res_colind+res_rowadr[i-1], res_rownnz[i]);
    }
...
void sim_math_blockDiag(sim_scalar_t* restrict res, const sim_scalar_t* restrict mat,
...
void sim_math_blockDiagSparse(sim_scalar_t* restrict res, int* restrict res_rownnz,
                         int* restrict res_rowadr, int* restrict res_colind,
                         const sim_scalar_t* restrict mat, const int* restrict rownnz,
                         const int* restrict rowadr, const int* restrict colind,
                         int nr, int nb,
                         const int* restrict perm_r, const int* restrict perm_c,
                         const int* restrict block_r, const int* restrict block_c,
                         sim_scalar_t* restrict res2, const sim_scalar_t* restrict mat2) {
  int block = 0;
