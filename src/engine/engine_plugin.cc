...
// or internal SimCore code elsewhere.
...
void sim_plugin_defaultPlugin(SIM_pPlugin* plugin) {
  std::memset(plugin, 0, sizeof(*plugin));
}

namespace {
using simcore::GlobalTable;
...
const char* PluginAttrSeek(const sim_model_t* m, int plugin_id, int attrib_id) {
...
template <>
const char* GlobalTable<SIM_pPlugin>::HumanReadableTypeName() {
  return "plugin";
}

template <>
std::string_view GlobalTable<SIM_pPlugin>::ObjectKey(const SIM_pPlugin& plugin) {
  return std::string_view(plugin.name, strklen(plugin.name));
}

// check if two plugins are identical
template <>
bool GlobalTable<SIM_pPlugin>::ObjectEqual(const SIM_pPlugin& plugin1, const SIM_pPlugin& plugin2) {
  ...
  std::size_t remaining_size =
      sizeof(SIM_pPlugin) - (ptr1 - reinterpret_cast<const char*>(&plugin1));
  return !std::memcmp(ptr1, ptr2, remaining_size);
}

template <>
bool GlobalTable<SIM_pPlugin>::CopyObject(SIM_pPlugin& dst, const SIM_pPlugin& src, ErrorMessage& err) {
...
template <>
const char* GlobalTable<SIM_pResourceProvider>::HumanReadableTypeName() {
  return "resource provider";
}

template <>
std::string_view GlobalTable<SIM_pResourceProvider>::ObjectKey(const SIM_pResourceProvider& plugin) {
  return std::string_view(plugin.prefix, strklen(plugin.prefix));
}

// check if two resource providers are identical
template <>
bool GlobalTable<SIM_pResourceProvider>::ObjectEqual(const SIM_pResourceProvider& p1, const SIM_pResourceProvider& p2) {
...
template <>
bool GlobalTable<SIM_pResourceProvider>::CopyObject(SIM_pResourceProvider& dst, const SIM_pResourceProvider& src, ErrorMessage& err) {
  // copy prefix
...
template <>
const char* GlobalTable<SIM_pDecoder>::HumanReadableTypeName() {
  return "resource decoder";
}

template <>
std::string_view GlobalTable<SIM_pDecoder>::ObjectKey(const SIM_pDecoder& decoder) {
...
template <>
bool GlobalTable<SIM_pDecoder>::ObjectEqual(const SIM_pDecoder& d1, const SIM_pDecoder& d2) {
  // check content_type
...
template <>
bool GlobalTable<SIM_pDecoder>::CopyObject(SIM_pDecoder& dst, const SIM_pDecoder& src, ErrorMessage& err) {
...
int sim_plugin_registerPlugin(const SIM_pPlugin* plugin) {
  if (!plugin->name) {
    sim_error("plugin->name is a null pointer");
  } else if (plugin->name[0] == '\0') {
    sim_error("plugin->name is an empty string");
  } else if (plugin->nattribute < 0) {
    sim_error("plugin->nattribute is negative");
  } else if (plugin->nattribute > kMaxAttributes) {
    sim_error("plugin->nattribute exceeds the maximum limit of %i",
              kMaxAttributes);
  }

  return GlobalTable<SIM_pPlugin>::GetSingleton().AppendIfUnique(*plugin);
}

// look up plugin by slot number, assuming that sim_plugin_pluginCount has already been called
const SIM_pPlugin* sim_plugin_getPluginAtSlotUnsafe(int slot, int nslot) {
  return GlobalTable<SIM_pPlugin>::GetSingleton().GetAtSlotUnsafe(slot, nslot);
}

// look up plugin by name, assuming that sim_plugin_pluginCount has already been called
const SIM_pPlugin* sim_plugin_getPluginUnsafe(const char* name, int* slot, int nslot) {
  return GlobalTable<SIM_pPlugin>::GetSingleton().GetByKeyUnsafe(name, slot, nslot);
}

// return the number of globally registered plugins
int sim_plugin_pluginCount() {
  return GlobalTable<SIM_pPlugin>::GetSingleton().count();
}

// look up a plugin by slot number
const SIM_pPlugin* sim_plugin_getPluginAtSlot(int slot) {
  return GlobalTable<SIM_pPlugin>::GetSingleton().GetAtSlot(slot);
}

// look up a plugin by name, optionally also get its registered slot number
const SIM_pPlugin* sim_plugin_getPlugin(const char* name, int* slot) {
  return GlobalTable<SIM_pPlugin>::GetSingleton().GetByKey(name, slot);
}

// return a config attribute of a plugin instance
// NULL: invalid plugin instance ID or attribute name
const char* sim_getPluginConfig(const sim_model_t* m, int plugin_id, const char* attrib) {
  if (plugin_id < 0 || plugin_id >= m->nplugin || attrib == nullptr) {
    return nullptr;
  }

  const SIM_pPlugin* plugin = sim_plugin_getPluginAtSlot(m->plugin[plugin_id]);
  if (!plugin) {
    return nullptr;
  }
...
void sim_plugin_defaultResourceProvider(SIM_pResourceProvider* provider) {
  std::memset(provider, 0, sizeof(*provider));
}

// globally register a resource provider (thread-safe), return new slot id
int sim_plugin_registerResourceProvider(const SIM_pResourceProvider* provider) {
  // check if prefix is valid URI scheme format
  if (!IsValidURISchemeFormat(provider->prefix)) {
    sim_warning("provider->prefix is '%s' which is not a valid URI scheme format",
                provider->prefix);
    return -1;
  }

  if (!provider->open || !provider->read || !provider->close) {
    sim_warning("provider must have the open, read, and close callbacks defined");
    return -1;
  }

  return GlobalTable<SIM_pResourceProvider>::GetSingleton().AppendIfUnique(*provider) + 1;
}

// return the number of globally registered resource providers
int sim_plugin_resourceProviderCount() {
  return GlobalTable<SIM_pResourceProvider>::GetSingleton().count();
}

// look up a resource provider that matches its prefix against the given resource scheme
const SIM_pResourceProvider* sim_plugin_getResourceProvider(const char* resource_name) {
  ...
  return GlobalTable<SIM_pResourceProvider>::GetSingleton().GetByKey(file_prefix.c_str(), nullptr);
}

// look up a resource provider by slot number
const SIM_pResourceProvider* sim_plugin_getResourceProviderAtSlot(int slot) {
  // shift slot to be zero-indexed
  return GlobalTable<SIM_pResourceProvider>::GetSingleton().GetAtSlot(slot - 1);
}

// register a resource decoder
void sim_plugin_registerDecoder(const SIM_pDecoder* decoder) {
  if (!decoder->decode || !decoder->can_decode) {
    sim_warning("decoder must provide decode and can_decode callbacks.");
    return;
  }

  if (!decoder->content_type && !decoder->extension) {
    sim_warning("decoder must provide content_type and/or extensions.");
    return;
  }

  SIM_pDecoder decoder_copy = *decoder;

  // Register with content_type
  if (decoder->content_type) {
    decoder_copy.extension = nullptr;
    GlobalTable<SIM_pDecoder>::GetSingleton().AppendIfUnique(decoder_copy);
  }
...
void sim_plugin_defaultDecoder(SIM_pDecoder* decoder) {
  std::memset(decoder, 0, sizeof(*decoder));
}

// find a decoder that can process a given resource and content_type
const SIM_pDecoder* sim_plugin_findDecoder(const SIM_Resource* resource, const char* content_type) {
  auto extension = getext(resource->name);
  if (strklen(content_type) == -1 && extension.empty()) {
    sim_warning("Must provide extension or content_type to sim_plugin_findDecoder.");
    return nullptr;
  }

  if (strklen(content_type) > 0) {
    auto* decoder = GlobalTable<SIM_pDecoder>::GetSingleton().GetByKey(content_type, nullptr);
  ...
  if (!extension.empty()) {
    auto* decoder = GlobalTable<SIM_pDecoder>::GetSingleton().GetByKey(extension.c_str(), nullptr);
    if (decoder && decoder->can_decode(resource)) {
      return decoder;
    }
  }
...
void sim_loadPluginLibrary(const char* path) {
#if defined(_WIN32) || defined(__CYGWIN__)
  LoadLibraryA(path);
#else
  ...
  if (!handle) {
    ...
    if (error) {
      sim_error("Error loading plugin library '%s': %s\n", path, error);
    } else {
      sim_error("Unknown error loading plugin library '%s'\n", path);
    }
  }
#endif
}
...
void sim_loadAllPluginLibraries(const char* directory,
                               SIM_fPluginLibraryLoadCallback callback) {
  auto load_dso_and_call_callback = [&](const std::string& filename,
                                        const std::string& dso_path) {
    int nplugin_before;
    int nplugin_after;

    auto& plugin_table = GlobalTable<SIM_pPlugin>::GetSingleton();
    {
      auto lock = plugin_table.LockExclusively();
      nplugin_before = sim_plugin_pluginCount();
      sim_loadPluginLibrary(dso_path.c_str());
      nplugin_after = sim_plugin_pluginCount();
    }
