...
// print 2D array of sim_scalar_t into file
static void printArray2d(const char* str, int nr, int nc, const sim_scalar_t* data, FILE* fp,
...
static void printDelayBuffer(const char* name, const sim_scalar_t* buf, int nhistory, int dim,
  ...
  // cursor (second slot, stored as sim_scalar_t but is an integer)
  fprintf(fp, "    cursor =  %d\n", (int)buf[1]);
  ...
  // values
  const sim_scalar_t* values = times + nhistory;
  if (dim == 1) {
    fprintf(fp, "    values = ");
    for (int i = 0; i < nhistory; i++) {
...
// print sparse matrix
static void printSparse(const char* str, const sim_scalar_t* mat, int nr,
...
static void printBlockArray(const char* str, const sim_scalar_t* data, int nr, int nc,
  ...
  for (int b = 0; b < nisland; b++) {
    ...
    const sim_scalar_t* data_ptr = data + r_start * nc;
...
static void printInertia(const char* str, const sim_scalar_t* mat, const sim_model_t* m,
                         FILE* fp, const char* float_format) {
  int nv = m->nv;
  // if no data, or too many rows to be visually useful, return
  if (!mat || !nv || nv > 300) {
...
void sim_printSparsity(const char* str, int nr, int nc, const int* rowadr, const int* diag,
...
void sim_printBlockSparsity(const char* str, int nr, int nc, int nisland,
...
// print vector
static void printVector(const char* str, const sim_scalar_t* data, int n, FILE* fp,
                        const char* float_format) {
  if (!data || !n) {
    return;
  }
...
static const char* memorySize(size_t nbytes) {
  static SIM_THREADLOCAL char message[32];
  int k = 1024;
...
static size_t sizeMesh(const sim_model_t* m) {
  size_t nbytes = 0;
  ...
  nbytes += sizeof(sim_scalar_t) * 3*m->nmeshpoly;      // mesh_polynormal
...
static size_t sizeSkin(const sim_model_t* m) {
  size_t nbytes = 0;
...
static size_t sizeBVH(const sim_model_t* m) {
  size_t nbytes = 0;
  nbytes += sizeof(int)    * m->nbvh;               // bvh_depth
  nbytes += sizeof(int)    * 2*m->nbvh;             // bvh_child
  nbytes += sizeof(int)    * m->nbvh;               // bvh_nodeid
  nbytes += sizeof(sim_scalar_t) * 6*m->nbvhstatic;       // bvh_aabb
  nbytes += sizeof(int)    * m->noct;               // oct_depth
  nbytes += sizeof(int)    * 8*m->noct;             // oct_child
  nbytes += sizeof(sim_scalar_t) * 6*m->noct;             // oct_aabb
  nbytes += sizeof(sim_scalar_t) * 8*m->noct;             // oct_coeff
  return nbytes;
}
...
static bool validateFloatFormat(const char* float_format) {
  ...
  if (strnlen(float_format, FLOAT_FORMAT_MAX_LEN + 1) > FLOAT_FORMAT_MAX_LEN) {
    sim_warning("Format string longer than limit of %d.", FLOAT_FORMAT_MAX_LEN);
    return false;
  }

  int cur_idx = 0;
  if (float_format[cur_idx] != '%') {
    sim_warning("Format string must start with '%%'.");
    return false;
  }
  cur_idx++;
  ...
  while (c = strchr(flag_characters, float_format[cur_idx]), c != NULL) {
    ...
    if (flag_character_counts[flag_idx] > 1) {
      sim_warning("Format string contains repeated flag.");
      return false;
    }
    cur_idx++;
  }
  ...
  if (strchr("fgGeE", float_format[cur_idx]) == NULL) {
    sim_warning("Format string specifier must be one of \"fgGeE\".");
    return false;
  }
  cur_idx++;
  ...
  if (float_format[cur_idx] == '\0') {
    ...
    sim_warning("Unable to match format string %s with expected pattern for a single float.",
                float_format);
    return false;
  }
}
...
// print sim_model_t to text file, specifying format. float_format must be a
// valid printf-style format string for a single float value
void sim_printFormattedModel(const sim_model_t* m, const char* filename, const char* float_format) {
  // get file
  FILE* fp;
  if (filename) {
  ...
  // check for nullptr
  if (!fp) {
    sim_warning("Could not open file '%s' for writing sim_model_t", filename);
    return;
  }

  // validate format string
  if (!validateFloatFormat(float_format)) {
    sim_warning("WARNING: Received invalid float_format. Using default instead.");
    float_format = FLOAT_FORMAT;
  }

  // compute total body mass
  sim_scalar_t totalmass = 0;
  for (int i=0; i < m->nbody; i++) {
    totalmass += m->body_mass[i];
  }

  // software version and model name
  fprintf(fp, "SimCore version %s\n", sim_versionString());
  fprintf(fp, "model name     %s\n\n", m->names);

  // memory footprint
  fprintf(fp, "MEMORY\n");
  size_t sz_total = sim_sizeModel(m);
...
#define X(type, name, sz)                                             \
  ...
  {                                                                   \
    const char* format =                                              \
        _Generic(m->opt.name, sim_scalar_t: float_format, int: INT_FORMAT); \
...
#define XVEC(type, name, sz)                                             \
  ...
  {                                                                      \
    const char* format =                                                 \
        _Generic(m->opt.name[0], sim_scalar_t: float_format, int: INT_FORMAT); \
    for (int i = 0; i < sz; i++) {                                       \
...
#define X(name, sz)                                             \
  ...
  {                                                             \
    for (int i = 0; i < sz; i++) {                              \
      fprintf(fp, float_format, ((sim_scalar_t*)(&m->stat.name))[i]); \
    }                                                           \
  }                                                             \
  fprintf(fp, "\n");
#define XVEC X
...
#undef X
  ...
  // values used by SIMMODEL_POINTERS macro
...
#define XNV X
  ...
  for (int i=0; i < m->ntuple; i++) {
    ...
    for (int j=m->tuple_adr[i]; j < m->tuple_adr[i]+m->tuple_size[i]; j++) {
      fprintf(fp, "       %s %d, prm = ",
              sim_math_type2Str(m->tuple_objtype[j]), m->tuple_objid[j]);
...
#undef X
  ...
  // B sparse structure
  sim_printSparsity("B: body-dof matrix", m->nbody, m->nv, m->B_rowadr, NULL, m->B_rownnz, NULL,
                   m->B_colind, fp);
  ...
  // M sparse structure
  sim_printSparsity("M: reduced inertia matrix", m->nv, m->nv, m->M_rowadr, NULL, m->M_rownnz,
                   NULL, m->M_colind, fp);
  ...
  // D sparse structure
  sim_printSparsity("D: dof-dof matrix", m->nv, m->nv,
...
// print sim_model_t to text file
void sim_printModel(const sim_model_t* m, const char* filename) {
  sim_printFormattedModel(m, filename, FLOAT_FORMAT);
}


// print sim_model_t to text file, specifying format. float_format must be a
// valid printf-style format string for a single float value
void sim_printFormattedData(const sim_model_t* m, const sim_data_t* d, const char* filename,
                           const char* float_format) {
  // stack in use, SHOULD NOT OCCUR
  if (d->pstack) {
    SIM_ERROR("attempting to print sim_data_t when stack is in use");
  }
  ...
  void* shadow = sim_malloc(d->nbuffer);
  __msan_copy_shadow(shadow, d->buffer, d->nbuffer);
  __msan_unpoison(d->buffer, d->nbuffer);
#endif

  fprintf(fp, "MEMORY\n");
  fprintf(fp, "  total         %s\n",   memorySize(sizeof(sim_data_t) + d->nbuffer + d->narena));
  fprintf(fp, "  struct        %s\n",   memorySize(sizeof(sim_data_t)));
...
#undef X
  ...
  // TIMER
  sim_scalar_t active_timers = 0;
  for (int i=0; i < SIM_NTIMER; i++) {
  ...
  if (d->nefc) {
    ...
    int nisland_stat = SIM_MAX(1, SIM_MIN(d->nisland, SIM_NISLAND));
    for (int island=0; island < nisland_stat; island++) {
      int niter_stat = SIM_MIN(SIM_NSOLVER, d->solver_niter[island]);
      if (niter_stat) {
        fprintf(fp, "  ISLAND %d\n", island);
        fprintf(fp, "    solver_niter = %d\n", d->solver_niter[island]);
        fprintf(fp, "    solver_nnz = %d\n",  d->solver_nnz[island]);
        for (int i=0; i < niter_stat; i++) {
          const SIM_SolverStat* stat = d->solver + island*SIM_NSOLVER + i;
  ...
  if (m->nhistory) {
    ...
    for (int i = 0; i < m->nu; i++) {
      ...
      if (adr >= 0) {
        char name[100];
        const char* actuator_name = sim_id2name(m, SIM_OBJ_ACTUATOR, i);
    ...
    for (int i = 0; i < m->nsensor; i++) {
      ...
      if (adr >= 0) {
        char name[100];
        const char* sensor_name = sim_id2name(m, SIM_OBJ_SENSOR, i);
  ...
  sim_printSparsity("FLEXEDGE_J: flex edge connectivity", m->nflexedge, m->nv,
  ...
  sim_printSparsity("TEN_J: tendon moments", m->ntendon, m->nv, d->ten_J_rowadr, NULL,
  ...
  sim_printSparsity("actuator_moment", m->nu, m->nv,
  ...
  if (d->nisland) {
    ...
    sim_printSparsity("M: inertia structure", m->nv, m->nv, m->M_rowadr, NULL,
                     m->M_rownnz, NULL, m->M_colind, fp);
    sim_printBlockSparsity("iM: block-diagonal inertia (nnzs are island ids)",
  ...
  if (!sim_math_isZero(d->qHDiagInv, m->nv)) {
  ...
  for (int i=0; i < d->ncon; i++) {
    ...
    if (g1 > -1 && g2 > -1) {
      fprintf(fp, "     geoms         ");
      const char* geom1 = sim_id2name(m, SIM_OBJ_GEOM, g1);
      const char* geom2 = sim_id2name(m, SIM_OBJ_GEOM, g2);
      if (geom1) {
        fprintf(fp, "%s : ", geom1);
      } else {
        fprintf(fp, "%d : ", g1);
    ...
    printVector("     solref       ", d->contact[i].solref, SIM_NREF, fp, float_format);
    printVector("     solimp       ", d->contact[i].solimp, SIM_NIMP, fp, float_format);
    ...
    sim_scalar_t force[6] = {0};
    sim_contactForce(m, d, i, force);
  ...
  if (!sim_isSparse(m)) {
    ...
    sim_printSparsity("J: constraint Jacobian", d->nefc, m->nv, d->efc_J_rowadr, NULL,
                     d->efc_J_rownnz, d->efc_J_rowsuper, d->efc_J_colind, fp);
    if (d->nisland) {
      sim_printBlockSparsity("IEFC_J: block-diagonalized constraint Jacobian (nnzs are island ids)",
    ...
    if (sim_isDual(m)) {
      ...
      sim_printSparsity("efc_AR: inverse constraint inertia", d->nefc, d->nefc, d->efc_AR_rowadr,
...
// print sim_data_t to text file
void sim_printData(const sim_model_t* m, const sim_data_t* d, const char* filename) {
  sim_printFormattedData(m, d, filename, FLOAT_FORMAT);
}
