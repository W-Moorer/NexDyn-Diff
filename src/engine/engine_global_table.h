...
#ifndef SIMCORE_SRC_ENGINE_ENGINE_GLOBAL_TABLE_H_
#define SIMCORE_SRC_ENGINE_ENGINE_GLOBAL_TABLE_H_
...
class GlobalTable {
 public:
  ...
  int AppendIfUnique(const T& obj) {
    ...
    // Do not call sim_error inside this lambda.
    int slot = [&]() {
    ...
    // registration failed, throw an sim_error
    if (slot < 0) {
      err[sizeof(err) - 1] = '\0';
      sim_error("%s", err);
    }
...
#endif  // SIMCORE_SRC_ENGINE_ENGINE_GLOBAL_TABLE_H_
