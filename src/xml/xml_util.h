...
#ifndef SIMCORE_SRC_XML_XML_UTIL_H_
#define SIMCORE_SRC_XML_XML_UTIL_H_
...
// XML Error info
class [[nodiscard]] sim_xml_error_t {
 public:
  sim_xml_error_t(const tinyxml2::XMLElement* elem = 0,
           const char* msg = 0,
           const char* str = 0,
           int pos = 0);
  ~sim_xml_error_t() = default;
...
class SIM_XSchema {
 public:
  SIM_XSchema(std::vector<const char*> schema[], unsigned nrow);
 ...
 private:
  ...
  std::vector<SIM_XSchema> subschema_;  // allowed child elements
...
typedef struct _mjMap sim_map_t;


// XML read and write utility functions
class sim_xml_util_t {
 public:
  sim_xml_util_t() = default;
  virtual ~sim_xml_util_t() = default;
  ...
  static int FindKey(const sim_map_t* map, int mapsz, std::string key);

  // find value in map, return key ("": not found)
  static std::string FindValue(const sim_map_t* map, int mapsz, int value);
  ...
  static std::optional<simcore::user::FilePath>
      ReadAttrFile(tinyxml2::XMLElement* elem, const char* attr,
                   const SIM_VFS* vfs,
                   const simcore::user::FilePath& dir = simcore::user::FilePath(),
                   bool required = false);
  ...
  static bool MapValue(tinyxml2::XMLElement* elem, const char* attr, int* data,
                       const sim_map_t* map, int mapSz, bool required = false);
  ...
  static void WriteAttrKey(tinyxml2::XMLElement* elem, std::string name,
                           const sim_map_t* map, int mapsz, int data, int def = -12345);

  // write attribute- space-separated keywords
  static void WriteAttrKeys(XMLElement* elem, std::string name, const sim_map_t* map,
...
#endif  // SIMCORE_SRC_XML_XML_UTIL_H_
