...
FilePath ResolveFilePath(XMLElement* e, const FilePath& filename,
  ...
  SIM_Resource *resource = sim_math_openResource("", fullname.c_str(), vfs,
                                          nullptr, 0);
  if (resource != nullptr) {
  ...
  for (; parent; parent = parent->Parent()->ToElement()) {
    if (!std::strcmp(parent->Value(), "include")) {
      auto file_attr = sim_xml_util_t::ReadAttrStr(parent, "dir", false);
...
void AccumulateFiles(std::unordered_set<std::string> &files,
  ...
  while (!elements.empty()) {
    ...
    if (!std::strcmp(elem->Value(), "include") ||
      ...
      auto file_attr = sim_xml_util_t::ReadAttrFile(elem, "file", nullptr);
      if (file_attr.has_value()) {
      ...
      auto assetdir_str = sim_xml_util_t::ReadAttrStr(elem, "assetdir", false);
      if (assetdir_str.has_value()) asset_dir = FilePath(assetdir_str.value());
      auto meshdir_str = sim_xml_util_t::ReadAttrStr(elem, "meshdir", false);
      if (meshdir_str.has_value()) mesh_dir = FilePath(meshdir_str.value());
      auto texturedir_str = sim_xml_util_t::ReadAttrStr(elem, "texturedir", false);
      ...
      for (const auto &attribute : attributes) {
        auto file_attr = sim_xml_util_t::ReadAttrFile(elem, attribute, nullptr);
        if (file_attr.has_value()) {
  ...
  for (const auto &file : include_and_model_files) {
    SIM_StringVec subdeps;
    FilePath full_path = model_dir + FilePath(file);
    sim_math_getXMLDependencies(full_path.Str().c_str(), &subdeps);
    for (const auto &subdep : subdeps) {
      files.insert(subdep);
    }
  }
  // Then for each non SIMCF resource file, add them to the set of files using their respective
...
// error string copy
void SIM_CopyError(char* dst, const char* src, int maxlen) {
  if (dst && maxlen > 0) {
...
void sim_math_getXMLDependencies(const char* filename, SIM_StringVec* dependencies) {
  ...
  // error checking
  if (doc.Error()) {
    sim_error("Problem reading XML file '%s': %s", filename, doc.ErrorStr());
  }

  // get top-level element
  tinyxml2::XMLElement *root = doc.RootElement();
  if (!root) {
    sim_error("XML root element not found");
  }
  std::unordered_set<std::string> files = {filename};

  std::optional<FilePath> model_dir = std::nullopt;
  SIM_Resource *resource = sim_math_openResource("", filename, nullptr,
                                          nullptr, 0);
  if (resource != nullptr) {
    const char* dir;
    int ndir;
...
// error constructor
sim_xml_error_t::sim_xml_error_t(const XMLElement* elem, const char* msg, const char* str, int pos) {
  char temp[500];

  // construct error message
  sim_math::sprintf_arr(message, "XML Error");
  if (msg) {
    sim_math::sprintf_arr(temp, msg, str, pos);
...
//---------------------------------- class SIM_XSchema implementation --------------------------------
...
// constructor
SIM_XSchema::SIM_XSchema(std::vector<const char*> schema[], unsigned nrow) {
  // set name and type
  name_ = schema[0][0];
  type_ = schema[0][1][0];
...
std::string SIM_XSchema::GetError() {
  return error;
}
...
void SIM_XSchema::Print(std::stringstream& str, int level) const {
  ...
  // children
  for (const SIM_XSchema& subschema : subschema_) {
...
void SIM_XSchema::PrintHTML(std::stringstream& str, int level, bool pad) const {
  // replace body with (world)body
  std::string name1 = (name_ == "body" ? "(world)body" : name_);
...
bool SIM_XSchema::NameMatch(XMLElement* elem, int level) {
...
// validator
XMLElement* SIM_XSchema::Check(XMLElement* elem, int level) {
  bool missing;
  char msg[100];
  XMLElement *bad, *sub;
  ...
  for (SIM_XSchema& subschema : subschema_) {
    subschema.refcnt_ = 0;
  }
    ...
    switch (subschema.type_) {
      case '!':
        if (subschema.refcnt_ > 1)
          sim_math::sprintf_arr(msg, "unique element '%s' found %d times",
                           subschema.name_.c_str(), subschema.refcnt_);
        else if (subschema.refcnt_ < 1)
          sim_math::sprintf_arr(msg, "element '%s' is required",
...
//---------------------------------- class sim_xml_util_t implementation ----------------------------------
...
template<typename T>
bool sim_xml_util_t::ReadAttrValues(XMLElement* elem, const char* attr,
  ...
  for (int i = 0; (max < 0 || i < max) && !strm.eof(); ++i) {
    ...
    if (token_strm.fail() || !token_strm.eof()) {
      ...
      if (maybe_result.has_value()) {
        ...
        throw sim_xml_error_t(elem, "problem reading attribute '%s'", attr);
      }
    }
    ...
    if constexpr (std::is_floating_point_v<T>) {
      if (std::isnan(item)) {
        sim_warning("XML contains a 'NaN'. Please check it carefully.");
      }
    }
...
template bool sim_xml_util_t::ReadAttrValues(XMLElement* elem, const char* attr,
...
// compare two vectors
template<typename T>
bool sim_xml_util_t::SameVector(const T* vec1, const T* vec2, int n) {
  if (!vec1 || !vec2) {
    return false;
  }
...
template bool sim_xml_util_t::SameVector(const double* vec1, const double* vec2, int n);
template bool sim_xml_util_t::SameVector(const float* vec1, const float* vec2, int n);
template bool sim_xml_util_t::SameVector(const int* vec1, const int* vec2, int n);
template bool sim_xml_util_t::SameVector(const unsigned char* vec1, const unsigned char* vec2, int n);


// find string in map, return corresponding integer (-1: not found)
int sim_xml_util_t::FindKey(const sim_map_t* map, int mapsz, std::string key) {
...
std::string sim_xml_util_t::FindValue(const sim_map_t* map, int mapsz, int value) {
  for (int i=0; i < mapsz; i++) {
...
template<typename T>
std::optional<std::vector<T> > sim_xml_util_t::ReadAttrVec(XMLElement* elem, const char* attr,
                                                    bool required) {
  std::vector<T> v;
  const char* raw_cstr = elem->Attribute(attr);
  if (raw_cstr) {
    v = simcore::user::StringToVector<T>(raw_cstr);
    if (errno == EDOM) {
      sim_warning("XML contains a 'NaN'. Please check it carefully.");
    } else if (errno == ERANGE) {
      throw sim_xml_error_t(elem, "number is too large in attribute '%s'", attr);
    } else if (errno == EINVAL) {
      throw sim_xml_error_t(elem, "bad format in attribute '%s'", attr);
    } else if (errno != 0) {
      throw sim_xml_error_t(elem, "unknown error in attribute '%s'", attr);
    }
  }

  if (!v.size()) {
    if (required) {
      throw sim_xml_error_t(elem, "required attribute missing: '%s'", attr);
    } else {
      return std::nullopt;
    }
  }
...
sim_xml_util_t::ReadAttrVec(XMLElement* elem, const char* attr, bool required);
...
sim_xml_util_t::ReadAttrStr(XMLElement* elem, const char* attr, bool required) {
  const char* pstr = elem->Attribute(attr);
...
sim_xml_util_t::ReadAttrFile(XMLElement* elem, const char* attr, const SIM_VFS* vfs,
...
template<typename T>
std::optional<T> sim_xml_util_t::ReadAttrNum(XMLElement* elem, const char* attr,
                                      bool required) {
...
sim_xml_util_t::ReadAttrNum(XMLElement* elem, const char* attr, bool required);
...
template<typename T>
int sim_xml_util_t::ReadAttr(XMLElement* elem, const char* attr, const int len,
  ...
  if (exact && maybe_vec->size() < len) {
    throw sim_xml_error_t(elem, "attribute '%s' does not have enough data", attr);
  }

  // check if there is too much data
  if (maybe_vec->size() > len) {
    throw sim_xml_error_t(elem, "attribute '%s' has too much data", attr);
  }
...
template int sim_xml_util_t::ReadAttr(XMLElement* elem, const char* attr, int len,
...
int sim_xml_util_t::ReadQuat(XMLElement* elem, const char* attr, double* data, std::string& text,
                      bool required) {
  ReadAttr(elem, attr, /*len=*/4, data, text, required, /*exact=*/true);

  // check for 0 quaternion
  if (data[0] == 0 && data[1] == 0 && data[2] == 0 && data[3] == 0) {
    throw sim_xml_error_t(elem, "zero quaternion is not allowed");
  }
...
int sim_xml_util_t::ReadVector(XMLElement* elem, const char* attr,
...
// read text field
bool sim_xml_util_t::ReadAttrTxt(tinyxml2::XMLElement* elem, const char* attr,
...
// read single int
bool sim_xml_util_t::ReadAttrInt(XMLElement* elem, const char* attr, int* data, bool required) {
...
void sim_xml_util_t::Vector2String(std::string& txt, const std::vector<float>& vec, int ncol) {
...
XMLElement* sim_xml_util_t::FindSubElem(XMLElement* elem, std::string name, bool required) {
  ...
  while (iter) {
    ...
    if (name == iter->Value()) {
      ...
      if (subelem) {
        throw sim_xml_error_t(subelem, "repeated element: '%s'", name.c_str());
      }
  ...
  if (required && !subelem) {
    throw sim_xml_error_t(elem, "missing element: '%s'", name.c_str());
  }
...
bool sim_xml_util_t::MapValue(XMLElement* elem, const char* attr, int* data,
                       const sim_map_t* map, int mapSz, bool required) {
  // get attribute text
  ...
  if (value < 0) {
    throw sim_xml_error_t(elem, "invalid keyword: '%s'", maybe_text->c_str());
  }
...
int sim_xml_util_t::MapValues(XMLElement* elem, const char* attr, int* data,
                       const sim_map_t* map, int mapSz, bool required) {
  // get attribute text
  auto maybe_text = ReadAttrStr(elem, attr, required);
  if (!maybe_text.has_value()) {
  ...
  while (strm >> key) {
    if (found_keys.count(key)) {
      throw sim_xml_error_t(elem, "duplicate keyword: '%s'");
      return 0;
    }

    int value = FindKey(map, mapSz, key);
    if (value == -1) {
      throw sim_xml_error_t(elem, "invalid keyword: '%s'");
      return 0;
    }
...
// write attribute
template<typename T>
void sim_xml_util_t::WriteAttr(XMLElement* elem, std::string name, int n, const T* data, const T* def,
                        bool trim) {
  ...
  stream.precision(simcore::_simPRIVATE__get_xml_precision());
...
template void sim_xml_util_t::WriteAttr(XMLElement* elem, std::string name, int n,
...
void sim_xml_util_t::WriteVector(XMLElement* elem, std::string name, const std::vector<double>& vec) {
...
void sim_xml_util_t::WriteAttrTxt(XMLElement* elem, std::string name, std::string value) {
...
void sim_xml_util_t::WriteAttrInt(XMLElement* elem, std::string name, int data, int def) {
  // skip default
  if (data == def) {
    return;
  }
...
void sim_xml_util_t::WriteAttrKey(XMLElement* elem, std::string name,
                           const sim_map_t* map, int mapsz, int data, int def) {
  // skip default
  if (data == def) {
    return;
  }
...
void sim_xml_util_t::WriteAttrKeys(XMLElement* elem, std::string name, const sim_map_t* map,
