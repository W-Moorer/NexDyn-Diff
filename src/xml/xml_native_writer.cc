...
using simcore::user::VectorToString;
...
class sim_XMLPrinter : public tinyxml2::XMLPrinter {
...
static string WriteDoc(XMLDocument& doc, char *error, size_t error_sz) {
  doc.ClearError();
  sim_XMLPrinter stream(nullptr, /*compact=*/false);
  doc.Print(&stream);
  if (doc.ErrorID()) {
    SIM_CopyError(error, doc.ErrorStr(), error_sz);
    return "";
  }
  string str = string(stream.CStr());

  // top level sections
  std::array<string, 16> sections = {
...
XMLElement* sim_xml_writer_t::InsertEnd(XMLElement* parent, const char* name) {
...
//---------------------------------- class sim_xml_writer_t: one-element writers --------------------------

// write flex
void sim_xml_writer_t::OneFlex(XMLElement* elem, const SIM_CFlex* flex) {
  string text;
  SIM_CFlex defflex;
  ...
  WriteAttr(cont, "solref", SIM_NREF, flex->solref, defflex.solref);
  WriteAttr(cont, "solimp", SIM_NIMP, flex->solimp, defflex.solimp);
...
// write mesh
void sim_xml_writer_t::OneMesh(XMLElement* elem, const sim_builder_mesh_t* mesh, sim_builder_default_t* def) {
  string text;
...
// write skin
void sim_xml_writer_t::OneSkin(XMLElement* elem, const SIM_CSkin* skin) {
  string text;
  sim_builder_default_t mydef;
  float zero = 0;
...
// write material
void sim_xml_writer_t::OneMaterial(XMLElement* elem, const SIM_CMaterial* material, sim_builder_default_t* def) {
  // regular
  if (!writingdefaults) {
  ...
  for (int i=1; i < SIM_NTEXROLE; i++) {
    if (!material->textures_[i].empty()) {
      if (i != SIM_TEXROLE_RGB) {
        has_non_rgb = true;
      }
    }
  }
  ...
  if (has_non_rgb) {
    ...
    if (material->textures_[SIM_TEXROLE_RGB] != def->Material().textures_[SIM_TEXROLE_RGB]) {
      WriteAttrTxt(elem, "texture", material->get_texture(SIM_TEXROLE_RGB));
    }
  }
...
// write joint
void sim_xml_writer_t::OneJoint(XMLElement* elem, const sim_builder_joint_t* joint, sim_builder_default_t* def,
  ...
  if (!writingdefaults) {
    ...
    if (joint->type != SIM_JNT_FREE) {
      WriteAttr(elem, "pos", 3, joint->pos);
    }
    if (joint->type != SIM_JNT_FREE && joint->type != SIM_JNT_BALL) {
  ...
  WriteAttr(elem, "solreflimit", SIM_NREF, joint->solref_limit, def->Joint().solref_limit, true);
  WriteAttr(elem, "solimplimit", SIM_NIMP, joint->solimp_limit, def->Joint().solimp_limit, true);
  WriteAttr(elem, "solreffriction", SIM_NREF, joint->solref_friction, def->Joint().solref_friction,
            true);
  WriteAttr(elem, "solimpfriction", SIM_NIMP, joint->solimp_friction, def->Joint().solimp_friction,
            true);
...
// write geom
void sim_xml_writer_t::OneGeom(XMLElement* elem, const sim_builder_geom_t* geom, sim_builder_default_t* def, string_view classname) {
  ...
  if (!writingdefaults) {
    ...
    if (SIM_GEOMINFO[geom->type]) {
      WriteAttr(elem, "size", SIM_GEOMINFO[geom->type], geom->size, def->Geom().size);
    }
    if (sim_math_internal_defined(geom->mass)) {
      mass = geom->GetVolume() * def->Geom().density;
    }

    // mesh geom
    if (geom->type == SIM_GEOM_MESH || geom->type == SIM_GEOM_SDF) {
      sim_builder_mesh_t* mesh = geom->mesh;
      ...
      if (!SameVector(geom->pos, mesh->GetPosPtr(), 3) ||
        ...
        double p[3], q[4];
        sim_math_internal_copy_vec(p, geom->pos, 3);
        sim_math_internal_copy_vec(q, geom->quat, 4);
        sim_math_internal_frameaccuminv(p, q, mesh->GetPosPtr(),
                           mesh->GetQuatPtr());
  ...
  // defaults and regular
  WriteAttrKey(elem, "type", geom_map, SIM_NGEOMTYPES, geom->type, def->Geom().type);
  ...
  WriteAttr(elem, "solref", SIM_NREF, geom->solref, def->Geom().solref, true);
  WriteAttr(elem, "solimp", SIM_NIMP, geom->solimp, def->Geom().solimp, true);
...
// write site
void sim_xml_writer_t::OneSite(XMLElement* elem, const sim_builder_site_t* site, sim_builder_default_t* def, string_view classname) {
  double unitq[4] = {1, 0, 0, 0};
  ...
  if (!writingdefaults) {
    ...
    if (SIM_GEOMINFO[site->type]) {
      WriteAttr(elem, "size", SIM_GEOMINFO[site->type], site->size, def->Site().size);
    }
  } else {
  ...
  WriteAttrKey(elem, "type", geom_map, SIM_NGEOMTYPES, site->type, def->Site().type);
...
// write camera
void sim_xml_writer_t::OneCamera(XMLElement* elem, const SIM_CCamera* camera, sim_builder_default_t* def,
                          string_view classname) {
  double unitq[4] = {1, 0, 0, 0};
  ...
  if (camera->output != def->Camera().output) {
    int data[SIM_NCAMOUT];
    int ndata = 0;
    for (int i = 0; i < SIM_NCAMOUT; i++) {
...
// write light
void sim_xml_writer_t::OneLight(XMLElement* elem, const SIM_CLight* light, sim_builder_default_t* def,
                         string_view classname) {
  // regular
  if (!writingdefaults) {
...
// write pair
void sim_xml_writer_t::OnePair(XMLElement* elem, const SIM_CPair* pair, sim_builder_default_t* def) {
  // regular
  if (!writingdefaults) {
  ...
  WriteAttr(elem, "solref", SIM_NREF, pair->solref, def->Pair().spec.solref, true);
  WriteAttr(elem, "solreffriction", SIM_NREF, pair->solreffriction, def->Pair().spec.solreffriction,
            true);
  WriteAttr(elem, "solimp", SIM_NIMP, pair->solimp, def->Pair().spec.solimp, true);
...
// write equality
void sim_xml_writer_t::OneEquality(XMLElement* elem, const SIM_CEquality* equality, sim_builder_default_t* def) {
  // regular
  if (!writingdefaults) {
    ...
    switch (equality->type) {
      case SIM_EQ_CONNECT:
        if (equality->objtype == SIM_OBJ_BODY) {
          WriteAttrTxt(elem, "body1", sim_spec_getString(equality->name1));
          WriteAttrTxt(elem, "body2", sim_spec_getString(equality->name2));
          WriteAttr(elem, "anchor", 3, equality->data);
        } else {
          WriteAttrTxt(elem, "site1", sim_spec_getString(equality->name1));
          WriteAttrTxt(elem, "site2", sim_spec_getString(equality->name2));
        }
        break;
      ...
      case SIM_EQ_JOINT:
        WriteAttrTxt(elem, "joint1", sim_spec_getString(equality->name1));
        WriteAttrTxt(elem, "joint2", sim_spec_getString(equality->name2));
        WriteAttr(elem, "polycoef", 5, equality->data);
        break;

      case SIM_EQ_TENDON:
        WriteAttrTxt(elem, "tendon1", sim_spec_getString(equality->name1));
        WriteAttrTxt(elem, "tendon2", sim_spec_getString(equality->name2));
        WriteAttr(elem, "polycoef", 5, equality->data);
        break;

      case SIM_EQ_FLEX:
      case SIM_EQ_FLEXVERT:
        WriteAttrTxt(elem, "flex", sim_spec_getString(equality->name1));
        break;

      default:
        sim_error("sim_xml_writer_t: unknown equality type.");
    }
  }

  // defaults and regular
  WriteAttrKey(elem, "active", bool_map, 2, equality->active, def->Equality().active);
  WriteAttr(elem, "solref", SIM_NREF, equality->solref, def->Equality().solref, true);
  WriteAttr(elem, "solimp", SIM_NIMP, equality->solimp, def->Equality().solimp, true);
}



// write tendon
void sim_xml_writer_t::OneTendon(XMLElement* elem, const sim_builder_tendon_t* tendon, sim_builder_default_t* def) {
  bool fixed = (tendon->GetWrap(0) && tendon->GetWrap(0)->Type() == SIM_WRAP_JOINT);
  ...
  WriteAttr(elem, "solreflimit", SIM_NREF, tendon->solref_limit, def->Tendon().solref_limit, true);
  WriteAttr(elem, "solimplimit", SIM_NIMP, tendon->solimp_limit, def->Tendon().solimp_limit, true);
  WriteAttr(elem, "solreffriction", SIM_NREF, tendon->solref_friction, def->Tendon().solref_friction,
            true);
  WriteAttr(elem, "solimpfriction", SIM_NIMP, tendon->solimp_friction, def->Tendon().solimp_friction,
            true);
...
// write actuator
void sim_xml_writer_t::OneActuator(XMLElement* elem, const SIM_CActuator* actuator, sim_builder_default_t* def) {
  // regular
  if (!writingdefaults) {
  ...
  if (writingdefaults) {
    ...
    int default_actdim = actuator->dyntype == SIM_DYN_NONE ? 0 : 1;
  ...
  WriteAttr(elem, "dynprm", SIM_NDYN, actuator->dynprm, def->Actuator().dynprm);
  ...
  // non-plugins: write actuator parameters
    ...
    WriteAttr(elem, "gainprm", SIM_NGAIN, actuator->gainprm, def->Actuator().gainprm, true);
    WriteAttr(elem, "biasprm", SIM_NBIAS, actuator->biasprm, def->Actuator().biasprm, true);
  }
...
// write plugin
void sim_xml_writer_t::OnePlugin(XMLElement* elem, const SIM_sPlugin* plugin) {
  const string instance_name = string(sim_spec_getString(plugin->name));
  const string plugin_name = string(sim_spec_getString(plugin->plugin_name));
  if (!instance_name.empty()) {
    WriteAttrTxt(elem, "instance", instance_name);
  } else {
    WriteAttrTxt(elem, "plugin", plugin_name);
    const SIM_pPlugin* pplugin = sim_plugin_getPluginAtSlot(
      static_cast<sim_builder_plugin_t*>(plugin->element)->plugin_slot);
    const char* c = &(static_cast<sim_builder_plugin_t*>(plugin->element)->flattened_attributes[0]);
...
//---------------------------------- class sim_xml_writer_t: top-level API --------------------------------

// constructor
sim_xml_writer_t::sim_xml_writer_t(void) {
  writingdefaults = false;
}


// cast model
void sim_xml_writer_t::SetModel(sim_spec_t* _spec, const sim_model_t* m) {
  if (_spec) {
    model = static_cast<sim_builder_model_t*>(_spec->element);
  }
  if (m) {
    sim_copyBack(&model->spec, m);
  }
}


// save existing model in SIMCF canonical format, must be compiled
string sim_xml_writer_t::Write(char *error, size_t error_sz) {
  // check model
  if (!model || !model->IsCompiled()) {
    SIM_CopyError(error, "XML Write error: Only compiled model can be written", error_sz);
    return "";
  }

  // create document and root
  XMLDocument doc;
  XMLElement* root = doc.NewElement("simcore");
  root->SetAttribute("model", sim_spec_getString(model->modelname));

  // insert root
  doc.InsertFirstChild(root);

  // write comment if present
  string text = sim_spec_getString(model->comment);
  if (!text.empty()) {
...
// compiler section
void sim_xml_writer_t::Compiler(XMLElement* root) {
...
// option section
void sim_xml_writer_t::Option(XMLElement* root) {
  SIM_Option opt;
  sim_defaultOption(&opt);
  ...
  WriteAttr(section, "o_solref", SIM_NREF, model->option.o_solref, opt.o_solref);
  WriteAttr(section, "o_solimp", SIM_NIMP, model->option.o_solimp, opt.o_solimp);
...
#define WRITEDSBL(NAME, MASK) \
    ...
    WRITEDSBL("constraint",     SIM_DSBL_CONSTRAINT)
    WRITEDSBL("equality",       SIM_DSBL_EQUALITY)
    WRITEDSBL("frictionloss",   SIM_DSBL_FRICTIONLOSS)
    WRITEDSBL("limit",          SIM_DSBL_LIMIT)
    WRITEDSBL("contact",        SIM_DSBL_CONTACT)
    WRITEDSBL("spring",         SIM_DSBL_SPRING)
    WRITEDSBL("damper",         SIM_DSBL_DAMPER)
    WRITEDSBL("gravity",        SIM_DSBL_GRAVITY)
    WRITEDSBL("clampctrl",      SIM_DSBL_CLAMPCTRL)
    WRITEDSBL("warmstart",      SIM_DSBL_WARMSTART)
    WRITEDSBL("filterparent",   SIM_DSBL_FILTERPARENT)
    WRITEDSBL("actuation",      SIM_DSBL_ACTUATION)
    WRITEDSBL("refsafe",        SIM_DSBL_REFSAFE)
    WRITEDSBL("sensor",         SIM_DSBL_SENSOR)
    WRITEDSBL("midphase",       SIM_DSBL_MIDPHASE)
    WRITEDSBL("eulerdamp",      SIM_DSBL_EULERDAMP)
    WRITEDSBL("autoreset",      SIM_DSBL_AUTORESET)
    WRITEDSBL("nativeccd",      SIM_DSBL_NATIVECCD)
    WRITEDSBL("island",         SIM_DSBL_ISLAND)
#undef WRITEDSBL

#define WRITEENBL(NAME, MASK) \
    if( model->option.enableflags & MASK ) \
      WriteAttrKey(sub, NAME, enable_map, 2, 1);
    WRITEENBL("override",       SIM_ENBL_OVERRIDE)
    WRITEENBL("energy",         SIM_ENBL_ENERGY)
    WRITEENBL("fwdinv",         SIM_ENBL_FWDINV)
    WRITEENBL("invdiscrete",    SIM_ENBL_INVDISCRETE)
    WRITEENBL("multiccd",       SIM_ENBL_MULTICCD)
    WRITEENBL("sleep",          SIM_ENBL_SLEEP)
#undef WRITEENBL
  }
...
// size section
void sim_xml_writer_t::Size(XMLElement* root) {
  XMLElement* section = InsertEnd(root, "size");

  // write memory
  if (model->memory != -1) {
    WriteAttrTxt(section, "memory", sim_math_writeNumBytes(model->memory));
  }
...
// statistic section
void sim_xml_writer_t::Statistic(XMLElement* root) {
  XMLElement* section = InsertEnd(root, "statistic");
  SIM_Statistic* s = &model->stat;

  if (sim_math_internal_defined(s->meaninertia)) WriteAttr(section, "meaninertia", 1, &s->meaninertia);
  if (sim_math_internal_defined(s->meanmass)) WriteAttr(section, "meanmass", 1, &s->meanmass);
  if (sim_math_internal_defined(s->meansize)) WriteAttr(section, "meansize", 1, &s->meansize);
  if (sim_math_internal_defined(s->extent)) WriteAttr(section, "extent", 1, &s->extent);
  if (sim_math_internal_defined(s->center[0])) WriteAttr(section, "center", 3, s->center);
...
// default section
void sim_xml_writer_t::Default(XMLElement* root, sim_builder_default_t* def) {
  XMLElement* elem;
  XMLElement* section;

  // pointer to parent defaults
  sim_builder_default_t* parent;
  if (def->parent) {
    parent = def->parent;
  } else {
    parent = new sim_builder_default_t;
  }
...
// extension section
void sim_xml_writer_t::Extension(XMLElement* root) {
  ...
  std::unordered_set<const SIM_pPlugin*> seen_plugins;

  // write all plugins
  const SIM_pPlugin* last_plugin = nullptr;
  XMLElement* plugin_elem = nullptr;
  for (int i = 0; i < model->Plugins().size(); ++i) {
    sim_builder_plugin_t* pp = static_cast<sim_builder_plugin_t*>(model->GetObject(SIM_OBJ_PLUGIN, i));
    ...
    const SIM_pPlugin* plugin = sim_plugin_getPluginAtSlot(pp->plugin_slot);
...
// custom section
void sim_xml_writer_t::Custom(XMLElement* root) {
  XMLElement* elem;

  // get sizes, skip section if empty
  int nnum = model->NumObjects(SIM_OBJ_NUMERIC);
  int ntxt = model->NumObjects(SIM_OBJ_TEXT);
  int ntup = model->NumObjects(SIM_OBJ_TUPLE);
  ...
  for (int i=0; i < nnum; i++) {
    SIM_CNumeric* numeric = (SIM_CNumeric*)model->GetObject(SIM_OBJ_NUMERIC, i);
  ...
  for (int i=0; i < ntxt; i++) {
    SIM_CText* text = (SIM_CText*)model->GetObject(SIM_OBJ_TEXT, i);
  ...
  for (int i=0; i < ntup; i++) {
    SIM_CTuple* tuple = (SIM_CTuple*)model->GetObject(SIM_OBJ_TUPLE, i);
    ...
    for (int j=0; j < (int)tuple->objtype_.size(); j++) {
      ...
      WriteAttrTxt(obj, "objtype", sim_math_type2Str((int)tuple->objtype_[j]));
...
// asset section
void sim_xml_writer_t::Asset(XMLElement* root) {
  XMLElement* elem;

  // get sizes
  int nmesh = model->NumObjects(SIM_OBJ_MESH);
  int nhfield = model->NumObjects(SIM_OBJ_HFIELD);

  // return if empty
  if (nmesh == 0 && nhfield == 0) {
    return;
  }
  ...
  for (int i=0; i < nmesh; i++) {
    ...
    sim_builder_mesh_t* mesh = (sim_builder_mesh_t*)model->GetObject(SIM_OBJ_MESH, i);
  ...
  for (int i=0; i < nhfield; i++) {
    // create element
    SIM_CHField* hfield = (SIM_CHField*)model->GetObject(SIM_OBJ_HFIELD, i);
...
XMLElement* sim_xml_writer_t::OneFrame(XMLElement* elem, sim_builder_frame_t* frame) {
  if (!frame) {
    return elem;
  }
...
void sim_xml_writer_t::Body(XMLElement* elem, sim_builder_body_t* body, sim_builder_frame_t* frame, string_view childclass) {
  double unitq[4] = {1, 0, 0, 0};

  if (!body) {
    throw sim_xml_error_t(0, "missing body in XML write");  // SHOULD NOT OCCUR
  }
  ...
  // write body attributes and inertial
    ...
    // sleep policy
    if (body->sleep != SIM_SLEEP_AUTO &&
        body->sleep != SIM_SLEEP_AUTO_NEVER &&
        body->sleep != SIM_SLEEP_AUTO_ALLOWED) {
    ...
    if (model->compiler.saveinertial ||
        (body->explicitinertial && model->compiler.inertiafromgeom != SIM_INERTIAFROMGEOM_TRUE)) {
  ...
  while (i < body->bodies.size() || body->bodies.empty()) {
    sim_builder_frame_t* bframe = body->bodies.empty() ? nullptr : body->bodies[i]->frame;
    ...
    while (j < body->frames.size()) {
      sim_builder_frame_t* fframe = body->frames[j++];
...
// collision section
void sim_xml_writer_t::Contact(XMLElement* root) {
  XMLElement* elem;

  // get number of pairs of each type
  int npair = model->NumObjects(SIM_OBJ_PAIR);
  int nexclude = model->NumObjects(SIM_OBJ_EXCLUDE);
  ...
  for (int i=0; i < npair; i++) {
    // create element and write
    SIM_CPair* pair = (SIM_CPair*)model->GetObject(SIM_OBJ_PAIR, i);
    elem = InsertEnd(section, "pair");
  ...
  for (int i=0; i < nexclude; i++) {
    // create element
    SIM_CBodyPair* exclude = (SIM_CBodyPair*)model->GetObject(SIM_OBJ_EXCLUDE, i);
...
// constraint section
void sim_xml_writer_t::Equality(XMLElement* root) {
  // skip section if empty
  int num;
  if ((num=model->NumObjects(SIM_OBJ_EQUALITY)) == 0) {
    return;
  }
  ...
  for (int i=0; i < num; i++) {
    SIM_CEquality* equality = (SIM_CEquality*)model->GetObject(SIM_OBJ_EQUALITY, i);
...
// deformable section
void sim_xml_writer_t::Deformable(XMLElement* root) {
  XMLElement* elem;

  // get sizes
  int nflex = model->NumObjects(SIM_OBJ_FLEX);
  int nskin = model->NumObjects(SIM_OBJ_SKIN);
  ...
  for (int i=0; i < nflex; i++) {
    // create element and write
    SIM_CFlex* flex = (SIM_CFlex*)model->GetObject(SIM_OBJ_FLEX, i);
  ...
  for (int i=0; i < nskin; i++) {
    // create element and write
    SIM_CSkin* skin = (SIM_CSkin*)model->GetObject(SIM_OBJ_SKIN, i);
...
// tendon section
void sim_xml_writer_t::Tendon(XMLElement* root) {
  // skip section if empty
  int num;
  if ((num=model->NumObjects(SIM_OBJ_TENDON)) == 0) {
    return;
  }
  ...
  for (int i=0; i < num; i++) {
    ...
    sim_builder_tendon_t* tendon = (sim_builder_tendon_t*)model->GetObject(SIM_OBJ_TENDON, i);
    if (!tendon->NumWraps()) {        // SHOULD NOT OCCUR
      continue;
    }
    XMLElement* elem = InsertEnd(section,
                                 tendon->GetWrap(0)->Type() == SIM_WRAP_JOINT ? "fixed" : "spatial");
    OneTendon(elem, tendon, model->def_map[tendon->classname]);

    // write wraps
    XMLElement* wrapelem;
    for (int j=0; j < tendon->NumWraps(); j++) {
      const SIM_CWrap* wrap = tendon->GetWrap(j);
      switch (wrap->Type()) {
        case SIM_WRAP_JOINT:
...
// actuator section
void sim_xml_writer_t::Actuator(XMLElement* root) {
  // skip section if empty
  int num;
  if ((num=model->NumObjects(SIM_OBJ_ACTUATOR)) == 0) {
    return;
  }
  ...
  // write all actuators
  for (int i=0; i < num; i++) {
    SIM_CActuator* actuator = (SIM_CActuator*)model->GetObject(SIM_OBJ_ACTUATOR, i);
    XMLElement* elem;
    if (actuator->plugin.active) {
...
// sensor section
void sim_xml_writer_t::Sensor(XMLElement* root) {
  double zero = 0;

  // skip section if empty
  int num;
  if ((num=model->NumObjects(SIM_OBJ_SENSOR)) == 0) {
    return;
  }
  ...
  for (int i=0; i < num; i++) {
    XMLElement* elem = 0;
    SIM_CSensor* sensor = model->Sensors()[i];
    if (sensor->type == SIM_SENS_CAMPROJECTION) {
      continue;
    }
    if (sensor->type == SIM_SENS_RANGEFINDER && sensor->objtype != SIM_OBJ_SITE) {
      continue;
    }
    ...
    switch (sensor->type) {
      ...
      case SIM_SENS_RANGEFINDER:
        {
          ...
          WriteAttrKeys(elem, "data", raydata_map, SIM_NRAYDATA, data, ndata, 0);
        }
        break;
      ...
      case SIM_SENS_FRAMEPOS:
        ...
        WriteAttrTxt(elem, "objtype", sim_math_type2Str(sensor->objtype));
        WriteAttrTxt(elem, "objname", sensor->get_objname());
        if (sensor->reftype != SIM_OBJ_UNKNOWN) {
          WriteAttrTxt(elem, "reftype", sim_math_type2Str(sensor->reftype));
      ...
      case SIM_SENS_GEOMDIST:
        ...
        WriteAttrTxt(elem, sensor->objtype == SIM_OBJ_BODY ? "body1" : "geom1", sensor->get_objname());
        WriteAttrTxt(elem, sensor->reftype == SIM_OBJ_BODY ? "body2" : "geom2", sensor->get_refname());
        break;
      ...
      case SIM_SENS_CONTACT:
        {
          ...
          if (sensor->objtype == SIM_OBJ_BODY) {
            WriteAttrTxt(elem, "body1", sensor->get_objname());
          } else if (sensor->objtype == SIM_OBJ_XBODY) {
            WriteAttrTxt(elem, "subtree1", sensor->get_objname());
          } else if (sensor->objtype == SIM_OBJ_GEOM) {
            WriteAttrTxt(elem, "geom1", sensor->get_objname());
          } else if (sensor->objtype == SIM_OBJ_SITE) {
            WriteAttrTxt(elem, "site", sensor->get_objname());
          }
          if (sensor->reftype == SIM_OBJ_BODY) {
            WriteAttrTxt(elem, "body2", sensor->get_refname());
          } else if (sensor->reftype == SIM_OBJ_XBODY) {
            WriteAttrTxt(elem, "subtree2", sensor->get_refname());
          } else if (sensor->reftype == SIM_OBJ_GEOM) {
            WriteAttrTxt(elem, "geom2", sensor->get_refname());
          }
          int dataspec = sensor->intprm[0];
          WriteAttrInt(elem, "num", sensor->dim / sim_math_condataSize(dataspec), 1);
          int data[SIM_NCONDATA];
          int ndata = 0;
          ...
          WriteAttrKeys(elem, "data", condata_map, SIM_NCONDATA, data, ndata, 0);
      ...
      case SIM_SENS_PLUGIN:
        elem = InsertEnd(section, "plugin");
        if (sensor->objtype != SIM_OBJ_UNKNOWN) {
          WriteAttrTxt(elem, "objtype", sim_math_type2Str(sensor->objtype));
          WriteAttrTxt(elem, "objname", sensor->get_objname());
        }
      ...
      case SIM_SENS_USER:
        ...
        if (sim_math_type2Str(sensor->objtype)) {
          WriteAttrTxt(elem, "objtype", sim_math_type2Str(sensor->objtype));
        }
        WriteAttrTxt(elem, "objname", sensor->get_objname());
...
// keyframe section
void sim_xml_writer_t::Keyframe(XMLElement* root) {
  // create section
  XMLElement* section = InsertEnd(root, "keyframe");

  if (!model->key_pending_.empty()) {
    throw sim_xml_error_t(0, "Model has pending keyframes. It must be (re)compiled before writing XML.");
  }
    ...
    if (model->nmocap) {
      for (int j=0; j < model->nbody; j++) {
        if (model->Bodies()[j]->mocap) {
          sim_builder_body_t* body = model->Bodies()[j];
          int id = body->mocapid;
