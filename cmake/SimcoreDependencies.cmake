...
# Build configuration for third party libraries used in simcore.

set(SIMCORE_DEP_VERSION_lodepng
    17d08dd26cac4d63f43af217ebd70318bfb8189c
    CACHE STRING "Version of `lodepng` to be fetched."
)
set(SIMCORE_DEP_VERSION_tinyxml2
    e6caeae85799003f4ca74ff26ee16a789bc2af48
    CACHE STRING "Version of `tinyxml2` to be fetched."
)
set(SIMCORE_DEP_VERSION_tinyobjloader
    1421a10d6ed9742f5b2c1766d22faa6cfbc56248
    CACHE STRING "Version of `tinyobjloader` to be fetched."
)
set(SIMCORE_DEP_VERSION_MarchingCubeCpp
    f03a1b3ec29b1d7d865691ca8aea4f1eb2c2873d
    CACHE STRING "Version of `MarchingCubeCpp` to be fetched."
)
set(SIMCORE_DEP_VERSION_ccd
    7931e764a19ef6b21b443376c699bbc9c6d4fba8 # v2.1
    CACHE STRING "Version of `ccd` to be fetched."
)
set(SIMCORE_DEP_VERSION_qhull
    62ccc56af071eaa478bef6ed41fd7a55d3bb2d80
    CACHE STRING "Version of `qhull` to be fetched."
)
set(SIMCORE_DEP_VERSION_Eigen3
    75bcd155c40cb48e647c87c3f29052360255bc9e
    CACHE STRING "Version of `Eigen3` to be fetched."
)

set(SIMCORE_DEP_VERSION_abseil
    255c84dadd029fd8ad25c5efb5933e47beaa00c7 # LTS 20250814.1
    CACHE STRING "Version of `abseil` to be fetched."
)

set(SIMCORE_DEP_VERSION_gtest
    52eb8108c5bdec04579160ae17225d66034bd723 # v1.17.0
    CACHE STRING "Version of `gtest` to be fetched."
)

set(SIMCORE_DEP_VERSION_benchmark
    5c55f5d4f45a1b09c5d98aa63a671993ebd42c69
    CACHE STRING "Version of `benchmark` to be fetched."
)

set(SIMCORE_DEP_VERSION_TriangleMeshDistance
    2cb643de1436e1ba8e2be49b07ec5491ac604457
    CACHE STRING "Version of `TriangleMeshDistance` to be fetched."
)

mark_as_advanced(SIMCORE_DEP_VERSION_lodepng)
mark_as_advanced(SIMCORE_DEP_VERSION_MarchingCubeCpp)
mark_as_advanced(SIMCORE_DEP_VERSION_tinyxml2)
mark_as_advanced(SIMCORE_DEP_VERSION_tinyobjloader)
mark_as_advanced(SIMCORE_DEP_VERSION_ccd)
mark_as_advanced(SIMCORE_DEP_VERSION_qhull)
mark_as_advanced(SIMCORE_DEP_VERSION_Eigen3)
mark_as_advanced(SIMCORE_DEP_VERSION_abseil)
mark_as_advanced(SIMCORE_DEP_VERSION_gtest)
mark_as_advanced(SIMCORE_DEP_VERSION_benchmark)
mark_as_advanced(SIMCORE_DEP_VERSION_TriangleMeshDistance)
...
if(NOT TARGET lodepng)
  FetchContent_Declare(
    ...
    GIT_TAG ${SIMCORE_DEP_VERSION_lodepng}
  )
  ...
  if(NOT lodepng_POPULATED)
    ...
    target_compile_options(lodepng PRIVATE ${SIMCORE_MACOS_COMPILE_OPTIONS})
    target_link_options(lodepng PRIVATE ${SIMCORE_MACOS_LINK_OPTIONS})
    if(NOT EMSCRIPTEN)
...
if(NOT TARGET marchingcubecpp)
  FetchContent_Declare(
    ...
    GIT_TAG ${SIMCORE_DEP_VERSION_MarchingCubeCpp}
  )
...
set(QHULL_FETCH_PATCH_ARGS "")
if(EMSCRIPTEN)
  # Patch changes in https://github.com/qhull/qhull/pull/173.patch
  set(QHULL_FETCH_PATCH_ARGS
      PATCH_COMMAND git apply --reject --whitespace=fix
                    ${PROJECT_SOURCE_DIR}/cmake/qhull-support-emscripten.patch
  )
endif()
...
findorfetch(
  ...
  GIT_TAG
  ${SIMCORE_DEP_VERSION_qhull}
  TARGETS
  qhull
  EXCLUDE_FROM_ALL
  ${QHULL_FETCH_PATCH_ARGS}
)
# simcore includes a file from libqhull_r which is not exported by the qhull include directories.
# Add it to the target.
target_include_directories(
  qhullstatic_r INTERFACE $<BUILD_INTERFACE:${qhull_SOURCE_DIR}/src/libqhull_r>
)
target_compile_options(qhullstatic_r PRIVATE ${SIMCORE_MACOS_COMPILE_OPTIONS})
target_link_options(qhullstatic_r PRIVATE ${SIMCORE_MACOS_LINK_OPTIONS})
...
findorfetch(
  ...
  GIT_TAG
  ${SIMCORE_DEP_VERSION_tinyxml2}
  TARGETS
  tinyxml2
  EXCLUDE_FROM_ALL
)
target_compile_options(tinyxml2 PRIVATE ${SIMCORE_MACOS_COMPILE_OPTIONS})
target_link_options(tinyxml2 PRIVATE ${SIMCORE_MACOS_LINK_OPTIONS})

# update cmake_minimum_required version for compatibility with newer version of cmake
if(NOT DEFINED CMAKE_POLICY_VERSION_MINIMUM)
  set(CMAKE_POLICY_VERSION_MINIMUM ${SIMCORE_CMAKE_MIN_REQ})
...
findorfetch(
  ...
  GIT_TAG
  ${SIMCORE_DEP_VERSION_tinyobjloader}
  TARGETS
  tinyobjloader
  EXCLUDE_FROM_ALL
)
...
if(NOT TARGET trianglemeshdistance)
  FetchContent_Declare(
    ...
    GIT_TAG ${SIMCORE_DEP_VERSION_TriangleMeshDistance}
  )
...
set(CCD_FETCH_PATCH_ARGS "")
if(EMSCRIPTEN)
  # Patch changes in https://github.com/danfis/libccd/pull/83.patch
  set(CCD_FETCH_PATCH_ARGS
      PATCH_COMMAND git apply --reject --whitespace=fix
                    ${PROJECT_SOURCE_DIR}/cmake/ccd-support-emscripten.patch
  )
endif()
...
findorfetch(
  ...
  GIT_TAG
  ${SIMCORE_DEP_VERSION_ccd}
  TARGETS
  ccd
  EXCLUDE_FROM_ALL
  ${CCD_FETCH_PATCH_ARGS}
)
if(CMAKE_POLICY_VERSION_MINIMUM_LOCALLY_DEFINED)
  unset(CMAKE_POLICY_VERSION_MINIMUM)
  unset(CMAKE_POLICY_VERSION_MINIMUM_LOCALLY_DEFINED)
endif()
target_compile_options(ccd PRIVATE ${SIMCORE_MACOS_COMPILE_OPTIONS})
target_link_options(ccd PRIVATE ${SIMCORE_MACOS_LINK_OPTIONS})
...
if(SIMCORE_BUILD_TESTS OR SIMCORE_BUILD_STUDIO OR SIMCORE_USE_FILAMENT)
...
if(SIMCORE_TEST_PYTHON_UTIL)
  ...
  if(NOT TARGET eigen)
    ...
    FetchContent_Declare(
      ...
      GIT_TAG ${SIMCORE_DEP_VERSION_Eigen3}
    )
...
set(BUILD_SHARED_LIBS
    ...
    CACHE BOOL "Build simcore as a shared library" FORCE
)
